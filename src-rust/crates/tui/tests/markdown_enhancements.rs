...
use mangocode_tui::messages::{
    detect_table, parse_inline_formatting, render_markdown, render_table,
};
...
fn table_alignment_detection() {
    ...
    assert!(matches!(
        table.alignments[0],
        mangocode_tui::messages::TableAlignment::Left
    ));
    // Second column should be center-aligned
    assert!(matches!(
        table.alignments[1],
        mangocode_tui::messages::TableAlignment::Center
    ));
    // Third column should be right-aligned
    assert!(matches!(
        table.alignments[2],
        mangocode_tui::messages::TableAlignment::Right
    ));
}
...
#[test]
fn markdown_preserves_url_and_email_text() {
    let lines = render_markdown("See https://example.com or mail help@example.com", 120);
    let content = flatten(&lines);
