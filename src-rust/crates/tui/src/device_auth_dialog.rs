...
// Copilot (RFC 8628), browser-based OAuth for Anthropic, and browser-based OAuth
// for OpenAI Codex (ChatGPT).  The actual network requests run in a background
// tokio task; this module only owns the display state.
...
pub enum DeviceAuthEvent {
    ...
    /// The OAuth URL could not be opened automatically; manual URL entry is still valid.
    BrowserOpenFailed { url: String, error: String },
...
pub fn render_device_auth_dialog(frame: &mut Frame, state: &DeviceAuthDialogState, area: Rect) {
    ...
    let pink = Color::Rgb(250, 114, 145); // golden mango accent
