...
pub static PROMPT_SLASH_COMMANDS: Lazy<Vec<SlashCommandSpec>> = Lazy::new(|| {
    vec![
        ...
        SlashCommandSpec {
            name: "run".into(),
            description: "Inspect work-run status, evidence, replay trace, doctor, and reliability eval".into(),
            group: "Core".into(),
        },
        SlashCommandSpec {
            name: "intelligence".into(),
            description: "Inspect and refresh source intelligence for grounded coding work".into(),
            group: "Core".into(),
        },
        SlashCommandSpec {
            name: "work-run".into(),
            description: "Alias for /run status, evidence, replay trace, doctor, and reliability eval".into(),
            group: "Core".into(),
        },
        SlashCommandSpec {
            name: "sessions".into(),
        ...
        SlashCommandSpec {
            name: "greeting".into(),
            description: "Toggle the startup greeting box".into(),
            group: "UI".into(),
        },
        SlashCommandSpec {
        ...
        SlashCommandSpec {
            name: "completion-policy".into(),
            description: "Control source, verification, and completion readiness gates".into(),
            group: "Core".into(),
        },
        SlashCommandSpec {
            name: "settings".into(),
...
mod tests {
    ...
    #[test]
    fn graphify_prompt_command_visibility_matches_feature_gate() {
        let enabled = cfg!(feature = "tool-project-graph");
        assert_eq!(
            PROMPT_SLASH_COMMANDS
                .iter()
                .any(|cmd| cmd.name == "graphify"),
            enabled
        );
        for key in ["graph", "graphify", "project-graph"] {
            assert_eq!(
                RUNTIME_SLASH_COMMAND_RESERVED_KEYS.contains(&key),
                enabled,
                "{key} reserved-key visibility should match tool-project-graph"
            );
        }
    }
