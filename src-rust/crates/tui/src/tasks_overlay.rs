...
use chrono;
use mangocode_tools::TaskStatus;

// ---------------------------------------------------------------------------
// Helper functions for TaskStatus (defined in mangocode_tools)
// ---------------------------------------------------------------------------
...
pub struct TasksOverlay {
    ...
    /// Last-rendered viewport height (rows). Updated by `render_tasks_overlay`
    /// each frame so `ensure_visible` can scroll using the actual terminal size.
    /// Uses `Cell` so the render function can write through `&TasksOverlay`.
    pub last_viewport_height: Cell<u16>,
}
...
impl TasksOverlay {
    ...
    /// Converts tasks from the mangocode_tools task store to display models.
    ///
    ...
    pub fn refresh_tasks(
        &mut self,
        task_store: &Arc<dashmap::DashMap<String, mangocode_tools::Task>>,
    ) {
        self.tasks.clear();
    ...
    fn ensure_visible(&mut self) {
        let viewport_height = (self.last_viewport_height.get() as usize).max(1);
        if self.selected_idx < self.scroll_offset as usize {
            ...
            self.scroll_offset =
                (self.selected_idx + 1).saturating_sub(viewport_height) as u16;
        }
    }
    ...
    pub fn cycle_and_persist_status(&mut self) -> Option<(String, TaskStatus)> {
        if let Some(task) = self.tasks.get(self.selected_idx) {
            ...
            if let Some(mut global_task) = mangocode_tools::TASK_STORE.get_mut(&task_id) {
...
pub fn render_tasks_overlay(frame: &mut Frame, overlay: &TasksOverlay, area: Rect) {
    ...
    // Record viewport height so ensure_visible() can compute correct scroll.
    overlay.last_viewport_height.set(inner.height);
