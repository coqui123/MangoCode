...
impl PermissionRequest {
    /// Create a standard four-option permission dialog:
    ///   `y` — Yes, allow once
    ...
    /// text produced by `mangocode_core::format_permission_reason`.
    ///
...
// ---------------------------------------------------------------------------
//
// Displays one (or a small batch) of clarification questions from the
// AskUserQuestion tool. Each question has options and an implicit
// free-form "Other" path; multi-select questions accumulate selections
// before advancing.

use mangocode_tools::{Question, QuestionAnswer, QuestionResponse};

/// Per-question dialog state — focused row, selected options, the free-form
/// "Other" text buffer, and a flag for whether the user is currently typing
/// in the "Other" field.
#[derive(Debug, Clone)]
pub struct QuestionState {
    /// Focused row index. 0..options.len() = an option; options.len() = the
    /// "Other..." entry.
    pub focused: usize,
    /// Indices of selected options (used when `multi_select` is true; for
    /// single-select questions we set/clear a single index here).
    pub selected: std::collections::BTreeSet<usize>,
    /// Free-form text the user typed when picking "Other".
    pub other_text: String,
    /// True when the user is currently typing into the "Other" text field.
...
/// State for the AskUserQuestion clarification dialog.
#[derive(Debug, Clone)]
pub struct QuestionDialog {
    ...
    pub current: usize,
    /// Vertical scroll offset for long option lists.
    pub scroll: u16,
    /// True once the user dismisses the dialog (Esc) before answering all.
    pub cancelled: bool,
    /// True once every question has an answer and the response should be
    /// sent. The CLI main loop checks this and dispatches the oneshot.
    pub completed: bool,
}
...
impl QuestionDialog {
    ...
    pub fn current_state_mut(&mut self) -> Option<&mut QuestionState> {
        self.states.get_mut(self.current)
    }

    /// True once every question has a non-empty answer.
    pub fn is_complete(&self) -> bool {
        self.questions
            .iter()
            .zip(self.states.iter())
            .all(|(_, s)| !s.selected.is_empty() || !s.other_text.is_empty())
    }

    /// Build the final response payload after all answers are collected (or
    /// after cancellation).
    pub fn build_response(&self) -> QuestionResponse {
        let answers = self
            ...
            .zip(self.states.iter())
            .map(|(q, s)| {
                if !s.other_text.trim().is_empty() {
                    QuestionAnswer::Other {
                        text: s.other_text.trim().to_string(),
                    }
                } else if !s.selected.is_empty() {
                    let selected_labels: Vec<String> = s
                        .selected
                        .iter()
...
/// Render the QuestionDialog centered over the frame.
pub fn render_question_dialog(dialog: &QuestionDialog, frame: &mut Frame) {
    use ratatui::layout::{Constraint, Direction, Layout};
    use ratatui::widgets::Wrap;

    let area = centered_dialog_area(frame.area(), 78, 26);
    frame.render_widget(Clear, area);

    let Some(question) = dialog.current_question() else {
        return;
    };
    let Some(state) = dialog.current_state() else {
        return;
    };

    let total = dialog.questions.len();
    ...
    // Layout: header + question + spacer + options + spacer + hint
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(1), // header chip
            Constraint::Length(3), // question text (wrapped)
            Constraint::Min(4),    // options
            Constraint::Length(2), // hint footer
        ])
        .split(inner);
    ...
    // Question text
    let qpara = Paragraph::new(question.question.clone())
        .wrap(Wrap { trim: true })
        .style(Style::default().fg(Color::White).add_modifier(Modifier::BOLD));
    frame.render_widget(qpara, chunks[1]);

    // Options list
    let mut option_lines: Vec<Line> = Vec::new();
    for (i, opt) in question.options.iter().enumerate() {
        let is_focused = state.focused == i && !state.typing_other;
        ...
        let cursor = if is_focused { "\u{276f} " } else { "  " };
    ...
    let is_other_focused = state.focused == other_idx;
    let other_cursor = if is_other_focused { "\u{276f} " } else { "  " };
    let other_label_style = if is_other_focused {
        Style::default()
            .fg(Color::Yellow)
            .add_modifier(Modifier::BOLD)
    } else {
    ...
    if state.typing_other {
        option_lines.push(Line::from(vec![
            Span::styled(other_cursor, Style::default().fg(Color::Cyan)),
            Span::styled("Other: ", other_label_style),
            Span::styled(
                state.other_text.clone(),
                Style::default()
                    .fg(Color::Green)
        ...
        let mut spans = vec![
            Span::styled(other_cursor, Style::default().fg(Color::Cyan)),
            Span::styled("    Other\u{2026}", other_label_style),
        ];
        if !state.other_text.is_empty() {
            spans.push(Span::raw("  "));
            spans.push(Span::styled(
                format!("\"{}\"", state.other_text),
    ...
    // Footer hint
    let hint = if state.typing_other {
        "Enter: accept text   Esc: back to options"
    } else if question.multi_select {
        "\u{2191}\u{2193} navigate   Space toggle   Enter confirm   o Other   Esc cancel"
    } else {
        "\u{2191}\u{2193} navigate   Enter pick   o Other   Esc cancel"
    };
...
/// Handle a key event for the QuestionDialog. Returns `true` if the dialog
/// transitioned into a terminal state (completed or cancelled) and the
/// caller should now dispatch the response.
pub fn handle_question_key(dialog: &mut QuestionDialog, key: KeyEvent) -> bool {
    if dialog.cancelled || dialog.completed {
        return true;
    }
    let Some(question) = dialog.questions.get(dialog.current).cloned() else {
        dialog.completed = true;
        return true;
    };
    let other_idx = question.options.len();
    let Some(state) = dialog.states.get_mut(dialog.current) else {
        dialog.completed = true;
        return true;
    };

    // When typing into the "Other" field, keys go to the text buffer.
    if state.typing_other {
        match key.code {
            KeyCode::Esc => {
                state.typing_other = false;
            }
            KeyCode::Enter => {
                state.typing_other = false;
                if !state.other_text.trim().is_empty() {
                    // Clear option selections — "Other" supersedes them.
                    state.selected.clear();
                    advance_or_finish(dialog);
                }
            }
            KeyCode::Backspace => {
            ...
            KeyCode::Char(c) => {
                state.other_text.push(c);
            }
            _ => {}
        }
        return dialog.completed || dialog.cancelled;
    }
    ...
    match key.code {
        ...
        KeyCode::Down => {
            if state.focused < other_idx {
                state.focused += 1;
            }
        }
        KeyCode::Tab => {
            // Move forward to next question if there's already an answer;
            // otherwise no-op.
            if !state.selected.is_empty() || !state.other_text.is_empty() {
                advance_or_finish(dialog);
            }
        }
        KeyCode::BackTab => {
        ...
        KeyCode::Char(' ') => {
            if state.focused < other_idx {
                if question.multi_select {
                    if !state.selected.insert(state.focused) {
                        state.selected.remove(&state.focused);
                    }
                } else {
                    state.selected.clear();
                    state.selected.insert(state.focused);
                }
                state.other_text.clear();
            } else {
                // Space on "Other" enters typing mode.
                state.typing_other = true;
            }
        }
        KeyCode::Enter => {
            if state.focused == other_idx {
                ...
                if question.multi_select {
                    // For multi-select, Enter confirms the current set
                    // and advances. If nothing is selected yet, toggle the
    ...
    dialog.completed || dialog.cancelled
}

fn advance_or_finish(dialog: &mut QuestionDialog) {
...
mod tests {
    ...
    // ----- AskUserQuestion dialog state machine -----------------------------
    ...
    fn make_question(options: &[&str], multi: bool, header: Option<&str>) -> Question {
        Question {
            ...
            options: options
                .iter()
                .map(|l| mangocode_tools::QuestionOption {
                    label: l.to_string(),
                    description: None,
                })
                .collect(),
    ...
    #[test]
    fn question_single_select_via_enter() {
        let q = make_question(&["A", "B", "C"], false, None);
        let mut d = QuestionDialog::new("tu1".into(), vec![q]);
        // Down twice to focus "C", then Enter
        assert!(!handle_question_key(&mut d, k(KeyCode::Down)));
        assert!(!handle_question_key(&mut d, k(KeyCode::Down)));
        assert!(handle_question_key(&mut d, k(KeyCode::Enter)));
        assert!(d.completed);
        let resp = d.build_response();
        assert!(!resp.cancelled);
        match &resp.answers[0] {
            QuestionAnswer::Options { selected } => assert_eq!(selected, &vec!["C".to_string()]),
            other => panic!("unexpected {:?}", other),
        }
    }

    #[test]
    fn question_digit_hotkey_single_select() {
        let q = make_question(&["one", "two", "three"], false, None);
        let mut d = QuestionDialog::new("tu1".into(), vec![q]);
        assert!(handle_question_key(&mut d, ch('2')));
        assert!(d.completed);
        match &d.build_response().answers[0] {
            QuestionAnswer::Options { selected } => assert_eq!(selected, &vec!["two".to_string()]),
            other => panic!("unexpected {:?}", other),
        }
    }

    #[test]
    fn question_multi_select_space_then_enter() {
        let q = make_question(&["A", "B", "C"], true, Some("hdr"));
        let mut d = QuestionDialog::new("tu1".into(), vec![q]);
        // Toggle A (focused), Down, toggle B
        assert!(!handle_question_key(&mut d, ch(' ')));
        assert!(!handle_question_key(&mut d, k(KeyCode::Down)));
        assert!(!handle_question_key(&mut d, ch(' ')));
        assert!(!d.completed);
        // Enter confirms multi-select set
        assert!(handle_question_key(&mut d, k(KeyCode::Enter)));
        let resp = d.build_response();
        match &resp.answers[0] {
    ...
    #[test]
    fn question_other_text_supersedes_options() {
        let q = make_question(&["A", "B"], false, None);
        let mut d = QuestionDialog::new("tu1".into(), vec![q]);
        // Hotkey 'o' enters typing-other on the Other entry
        assert!(!handle_question_key(&mut d, ch('o')));
        for c in "hello".chars() {
            assert!(!handle_question_key(&mut d, ch(c)));
        }
        // Enter accepts the Other text and completes
        assert!(handle_question_key(&mut d, k(KeyCode::Enter)));
        assert!(d.completed);
        let resp = d.build_response();
        match &resp.answers[0] {
            QuestionAnswer::Other { text } => assert_eq!(text, "hello"),
            other => panic!("unexpected {:?}", other),
        }
    }
    ...
    fn question_esc_cancels() {
        let q = make_question(&["A"], false, None);
        let mut d = QuestionDialog::new("tu1".into(), vec![q]);
        assert!(handle_question_key(&mut d, k(KeyCode::Esc)));
        assert!(d.cancelled);
        let resp = d.build_response();
        assert!(resp.cancelled);
        assert!(matches!(&resp.answers[0], QuestionAnswer::Cancelled));
    }

    #[test]
    fn question_multi_question_advance() {
        let q1 = make_question(&["a1", "a2"], false, None);
        let q2 = make_question(&["b1", "b2"], false, None);
        let mut d = QuestionDialog::new("tu1".into(), vec![q1, q2]);
        // Pick a1 — should advance to Q2 without completing.
        assert!(!handle_question_key(&mut d, ch('1')));
        assert!(!d.completed);
        ...
        match (&resp.answers[0], &resp.answers[1]) {
            (
                QuestionAnswer::Options { selected: a },
                QuestionAnswer::Options { selected: b },
            ) => {
                assert_eq!(a, &vec!["a1".to_string()]);
                assert_eq!(b, &vec!["b2".to_string()]);
            }
            other => panic!("unexpected {:?}", other),
        }
    }
    ...
    #[test]
    fn question_other_typing_backspace_then_esc_returns_to_options() {
        let q = make_question(&["A"], false, None);
        let mut d = QuestionDialog::new("tu1".into(), vec![q]);
        handle_question_key(&mut d, ch('o'));
        for c in "abc".chars() {
            handle_question_key(&mut d, ch(c));
        }
        handle_question_key(&mut d, k(KeyCode::Backspace));
        assert_eq!(d.current_state().unwrap().other_text, "ab");
        // Esc returns to option selection without completing.
        assert!(!handle_question_key(&mut d, k(KeyCode::Esc)));
        assert!(!d.current_state().unwrap().typing_other);
        assert!(!d.completed);
        assert!(!d.cancelled);
    }
}
