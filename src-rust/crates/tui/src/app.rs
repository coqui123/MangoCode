...
pub fn try_copy_to_clipboard(text: &str) -> bool {
    ...
    #[cfg(target_os = "windows")]
    {
        match std::process::Command::new("clip")
            ...
            .spawn()
        {
            Ok(child) => return finish_clipboard_writer(child, "clip", text),
            Err(err) => debug!(error = %err, tool = "clip", "clipboard helper unavailable"),
        }
    }
    // macOS
    #[cfg(target_os = "macos")]
    {
        match std::process::Command::new("pbcopy")
            ...
            .spawn()
        {
            Ok(child) => return finish_clipboard_writer(child, "pbcopy", text),
            Err(err) => debug!(error = %err, tool = "pbcopy", "clipboard helper unavailable"),
        }
    }
    // Linux / Wayland / X11
    #[cfg(target_os = "linux")]
        ...
        for cmd in &[
            ...
            if let Some((prog, args)) = parts.split_first() {
                if let Ok(child) = std::process::Command::new(prog)
                    ...
                    .spawn()
                {
                    if finish_clipboard_writer(child, prog, text) {
                        return true;
                    }
                }
            }
...
fn finish_clipboard_writer(mut child: std::process::Child, tool: &str, text: &str) -> bool {
    ...
    let write_ok = match child.stdin.take() {
        Some(mut stdin) => match stdin.write_all(text.as_bytes()) {
            Ok(()) => true,
            Err(err) => {
                warn!(error = %err, %tool, "failed to write clipboard text");
                false
            }
        },
        None => {
            warn!(%tool, "clipboard helper did not expose stdin");
            false
        }
    };
    ...
    let exit_ok = match child.wait() {
        ...
        Err(err) => {
            warn!(error = %err, %tool, "failed to wait for clipboard helper");
            false
        }
    };
...
pub enum ModalOwner {
    PermissionRequest,
    /// AskUserQuestion clarification dialog.
    QuestionDialog,
    Settings,
    Theme,
    Privacy,
...
pub struct App {
    ...
    pub show_help: bool,
    /// When true, the startup welcome/greeting box stays visible even after the
    /// transcript fills. Toggled with `/greeting`. Defaults to true.
    pub show_greeting: bool,
    /// Set by the in-app `/clear` handler when there is a turn in flight.
    /// The CLI loop checks this each tick and cancels + aborts the running
    /// query task so its TurnComplete event can't push old messages back
    /// over the just-cleared transcript.
    pub pending_query_cancel: bool,

    // Extended state
    pub tool_use_blocks: Vec<ToolUseBlock>,
    pub permission_request: Option<PermissionRequest>,
    /// Breadcrumb set when a permission dialog is dismissed.
    /// `(tool_use_id, tool_name, selected_option_key)` — '\x00' means Esc/dismiss.
    pub last_permission_response: Option<(String, String, char)>,
    /// Active clarification dialog (from the AskUserQuestion tool). When
    /// `Some`, the dialog is rendered as a modal overlay and steals keyboard
    /// input until the user answers every question or cancels.
    pub question_dialog: Option<crate::dialogs::QuestionDialog>,
    pub frame_count: u64,
    pub token_count: u32,
    ...
    /// LM Studio provider dialog.
    pub lmstudio_provider_dialog: DialogSelectState,
    ...
    /// the fetch completes.
    pub model_fetch_rx:
        Option<tokio::sync::mpsc::Receiver<Result<Vec<crate::model_picker::ModelEntry>, String>>>,
    ...
    /// Cell-row height of the rendered mascot inline image. Lets the welcome box
    /// reserve exactly the rows the image occupies instead of padding to the box
    /// height, which would leave a gap below the mascot. 0 until `init_mascot`.
    pub mascot_rows: u16,
...
impl App {
    fn env_voice_enabled() -> bool {
        std::env::var("MANGOCODE_VOICE_ENABLED")
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false)
    }

    fn voice_recorder_for_startup(
        enabled: bool,
    ) -> Option<Arc<Mutex<mangocode_core::voice::VoiceRecorder>>> {
        if !enabled {
            return None;
        }
    ...
    pub fn new(config: Config, cost_tracker: Arc<CostTracker>) -> Self {
        ...
        Self {
            ...
            show_help: false,
            show_greeting: true,
            pending_query_cancel: false,
            tool_use_blocks: Vec::new(),
            permission_request: None,
            last_permission_response: None,
            ...
            connect_dialog: {
                let items = vec![
                    ...
                    SelectItem {
                        ...
                        description: "Inference Providers router (OpenAI-compatible chat)".into(),
            ...
            lmstudio_provider_dialog: {
                let items = vec![
                    SelectItem {
                        id: "m365-copilot".into(),
                        title: "M365 Copilot".into(),
                        description: "Claude Opus via M365 Copilot".into(),
                        category: "LM Studio Providers".into(),
                        badge: Some("COPILOT".into()),
                    },
                    SelectItem {
                        id: "lmstudio-default".into(),
                        title: "Default LM Studio".into(),
                        description: "Standard local models".into(),
                        category: "LM Studio Providers".into(),
                        badge: None,
                    },
                ];
                DialogSelectState::new("LM Studio Provider", items)
            },
            ...
            voice_recorder: Self::voice_recorder_for_startup(Self::env_voice_enabled()),
    ...
    pub fn enable_voice_from_startup(&mut self) {
        self.voice_recorder = Self::voice_recorder_for_startup(true);
    }

    fn report_model_load_error(&mut self, reason: String) {
        let message = format!("Could not load models: {reason}");
        self.model_picker.set_load_error(reason);
        self.status_message = Some(message.clone());
        self.notifications
            .push(NotificationKind::Warning, message, Some(6));
    }
    ...
    fn modal_stack(&self) -> Vec<ModalOwner> {
        ...
        push_if!(self.question_dialog.is_some(), ModalOwner::QuestionDialog);
        ...
        push_if!(
            self.lmstudio_provider_dialog.visible,
            ModalOwner::LmStudioProvider
        );
        push_if!(
    ...
    fn persist_provider_and_model(&self) -> Result<(), String> {
        let mut settings =
    ...
    fn with_provider_persist_warning(message: String, result: Result<(), String>) -> String {
        match result {
            Ok(()) => message,
            Err(e) => format!(
                "{}\n\nWarning: failed to save provider/model preference: {}",
                message, e
            ),
        }
    }

    fn persist_fast_mode_setting(enabled: bool) -> Result<(), String> {
        let path = mangocode_core::config::Settings::config_dir().join("ui-settings.json");
        Self::persist_fast_mode_setting_to_path(&path, enabled)
        ...
        let mut value = if path.exists() {
            ...
            if !parsed.is_object() {
                return Err(std::io::Error::new(
                    ...
                    "ui-settings.json must contain a JSON object",
                ));
            }
            parsed
        } else {
    ...
    fn notify_persistence_warning(&mut self, context: &'static str, error: impl std::fmt::Display) {
        let message = format!("Warning: failed to save {context}: {error}");
        warn!(context, error = %error, "failed to persist TUI state");
        self.status_message = Some(message.clone());
        self.notifications
            .push(NotificationKind::Warning, message, Some(6));
    }
    ...
    fn complete_existing_codex_oauth_connect(&mut self) {
        ...
        let persist_result = self.persist_provider_and_model();
        self.has_credentials = true;
        self.provider_registry_stale = true;
        self.status_message = Some(Self::with_provider_persist_warning(
    ...
    pub fn apply_theme(&mut self, theme_name: &str) {
        ...
        match Settings::load_sync() {
            Ok(mut settings) => {
                settings.config.theme = self.config.theme.clone();
                match settings.save_sync() {
                    Ok(()) => self.status_message = Some(format!("Theme set to: {}", theme_name)),
                    Err(e) => {
                        self.status_message = Some(format!(
                            "Theme changed for this session, but failed to save settings: {}",
                            e
                        ));
                    }
                }
            ...
            Err(e) => {
                self.status_message = Some(format!(
                    "Theme changed for this session, but failed to load settings: {}",
                    e
                ));
            }
        }
    ...
    pub fn intercept_slash_command_with_args(&mut self, cmd: &str, args: &str) -> bool {
        ...
        match cmd.as_str() {
            ...
            "privacy" => {
                ...
                if let Some(message) = self.privacy_screen.take_status_message() {
                    self.status_message = Some(message);
                }
                true
            }
            ...
            "clear" => {
                // Ask the CLI loop to cancel/abort any turn currently in flight
                // so its TurnComplete sync can't repopulate `messages` after
                // this clear.
            ...
            "greeting" => {
                ...
                self.invalidate_transcript();
                self.status_message = Some(if self.show_greeting {
                    "Greeting shown.".to_string()
                } else {
                    "Greeting hidden.".to_string()
                });
                true
            }
            "vim" | "vi" => false,
    ...
    fn close_secondary_views(&mut self) {
        ...
        self.global_search.open = false;
        self.help_overlay.visible = false;
        self.show_help = false;
        self.rewind_flow.visible = false;
        self.history_search_overlay.visible = false;
        self.history_search = None;
        self.lmstudio_provider_dialog.close();
    }
        ...
        let (filename, content) = match self.export_dialog.selected {
            ExportFormat::Json => {
                ...
                let s = match serde_json::to_string_pretty(&json) {
                    Ok(s) => s,
                    Err(err) => {
                        warn!(error = %err, "failed to serialize transcript export JSON");
                        return None;
                    }
                };
    ...
    fn toggle_last_thinking_block(&mut self) {
    ...
    fn handle_key_event_internal(&mut self, key: KeyEvent, now: std::time::Instant) -> bool {
        ...
        // Go-to-line dialog (Ctrl+G in transcript mode)
            ...
            match key.code {
                ...
                KeyCode::Enter | KeyCode::Right => {
                    if self.onboarding_dialog.next_page() {
                        self.onboarding_dialog.dismiss();
                        if let Err(err) = Self::persist_onboarding_complete() {
                            self.notify_persistence_warning("onboarding completion", err);
                        }
                    }
                }
                KeyCode::Left => {
            ...
            match key.code {
                ...
                ) =>
                {
                        ...
                        self.device_auth_dialog.status
                    {
                        ...
                        let saved = if provider_id != mangocode_core::ProviderId::ANTHROPIC_MAX
                            && provider_id != "openai-codex"
                        {
                            ...
                            match self.auth_store.set_result(&provider_id, credential) {
                                Ok(()) => true,
                                Err(e) => {
                                    self.status_message = Some(format!(
            ...
            match key.code {
                ...
                KeyCode::Enter => {
                    ...
                    match validate_provider_credential(&provider_id, &pending_key) {
                        Ok(()) => {
                            match self.auth_store.set_result(
                                &provider_id,
                                mangocode_core::StoredCredential::ApiKey { key: pending_key },
                            ) {
                                Ok(()) => {
                                    let _ = self.key_input_dialog.take_key();
                                    self.set_provider_default(provider_id.clone());
                                    let persist_result = self.persist_provider_and_model();
                                    self.has_credentials = true;
                                ...
                                Err(e) => {
                                    self.status_message = Some(format!(
                                        "Step 3/3 failed [save]: failed to save {} credentials: {}",
                                        provider_name, e
                                    ));
                                }
                            }
                ...
                KeyCode::Enter => {
                    if let Some(selected) = self.connect_dialog.selected().cloned() {
                        ...
                        match selected.id.as_str() {
                            ...
                            // LM Studio: apply M365 Copilot as the default upfront so
                            // dismissing the sub-dialog still leaves it selected. Users
                            // can switch to "Default LM Studio" from the sub-dialog.
                            "lmstudio" => {
                                self.config.provider = Some("lmstudio".to_string());
                                self.set_model("lmstudio/m365-copilot".to_string());
                                let persist_result = self.persist_provider_and_model();
                                self.has_credentials = true;
                                self.provider_registry_stale = true;
                                self.status_message = Some(Self::with_provider_persist_warning(
                                    "Step 2/3: M365 Copilot applied as default. Press Enter to confirm or use arrow keys to pick Default LM Studio."
                                        .to_string(),
                                    persist_result,
                                ));
                                self.lmstudio_provider_dialog.open();
                            }
                            "ollama" | "llamacpp" => {
        ...
        if self.lmstudio_provider_dialog.visible {
            match key.code {
                KeyCode::Esc => {
                    self.lmstudio_provider_dialog.close();
                    self.connect_dialog.open();
                ...
                KeyCode::Up => {
                    self.lmstudio_provider_dialog.move_up();
                }
                KeyCode::Down => {
                    self.lmstudio_provider_dialog.move_down();
                }
                KeyCode::PageUp => {
                    self.lmstudio_provider_dialog.page_up();
                }
                KeyCode::PageDown => {
                    self.lmstudio_provider_dialog.page_down();
                }
                KeyCode::Enter => {
                    if let Some(selected) = self.lmstudio_provider_dialog.selected().cloned() {
                        self.lmstudio_provider_dialog.close();
                        if selected.id == "m365-copilot" {
                            self.config.provider = Some("lmstudio".to_string());
                            self.set_model("lmstudio/m365-copilot".to_string());
                        } else {
                            self.set_provider_default("lmstudio".to_string());
                        }
                        ...
                        self.status_message = Some(Self::with_provider_persist_warning(
                            format!(
                                "Step 3/3 complete: switched to {} via LM Studio. Ready to chat!",
                                selected.title
                            ),
                            persist_result,
                        ));
                ...
                KeyCode::Backspace => {
                    self.lmstudio_provider_dialog.filter_pop();
                }
                KeyCode::Char(c) => {
                    self.lmstudio_provider_dialog.filter_push(c);
                }
                _ => {}
            }
            return false;
            ...
            match key.code {
                ...
                KeyCode::Enter => {
                    if let Some((model_id, effort)) = self.model_picker.confirm() {
                        ...
                        let fast_mode_persist_warning = if self.fast_mode
                            && model_id != FAST_MODE_MODEL
                        {
                            self.fast_mode = false;
                            match Self::persist_fast_mode_setting(false) {
                                Ok(()) => None,
                                Err(err) => {
                                    warn!(
                                        error = %err,
                                        "failed to persist fast mode disable after model selection"
                                    );
                                    Some(format!(
                                        "Warning: failed to save fast-mode preference: {err}"
                                    ))
                                }
                            }
                        } else {
                        ...
                        let effort_hint = effort
                            ...
                            .unwrap_or_default();
                        let mut status_message = Self::with_provider_persist_warning(
                            format!("Model: {}{}", full_model, effort_hint),
                            persist_result,
                        );
                        if let Some(warning) = fast_mode_persist_warning {
                            self.notifications.push(
                                NotificationKind::Warning,
                                warning.clone(),
                                Some(6),
                            ...
                            status_message.push_str(&warning);
                        }
        ...
        // AskUserQuestion clarification dialog — owns input until the user
        // answers every question or cancels. The CLI main loop polls
        // `question_dialog.completed` / `.cancelled` each tick and dispatches
        // the response back over the oneshot.
        if let Some(dialog) = self.question_dialog.as_mut() {
            crate::dialogs::handle_question_key(dialog, key);
            return false;
        }

        // MCP elicitation dialog — highest priority modal after permission
        ...
        if self.focus == FocusTarget::Transcript {
            match key.code {
                ...
                // 't' in transcript mode: toggle last thinking block
                KeyCode::Char('t') if !self.is_streaming => {
                    self.toggle_last_thinking_block();
                    return false;
                }
                KeyCode::Char(_)
                    if !key.modifiers.contains(KeyModifiers::CONTROL)
    ...
    fn handle_permission_key(&mut self, key: KeyEvent) {
        ...
        match key.code {
            KeyCode::Char(c) => {
                if let Some(digit) = c.to_digit(10) {
                    ...
                    if let Some(idx) = matched_idx {
                        ...
                        self.dismiss_permission_with_breadcrumb(c);
                    }
                }
            }
            KeyCode::Enter => {
                ...
                let key = self
                    ...
                    .map(|o| o.key)
                    .unwrap_or('n');
                self.dismiss_permission_with_breadcrumb(key);
            }
            KeyCode::Up => {
                if let Some(pr) = self.permission_request.as_mut() {
                    if pr.selected_option > 0 {
            ...
            KeyCode::Esc => {
                self.dismiss_permission_with_breadcrumb('\x00');
            }
            _ => {}
        }
    }

    fn dismiss_permission_with_breadcrumb(&mut self, selected_key: char) {
        if let Some(pr) = self.permission_request.take() {
            self.last_permission_response = Some((pr.tool_use_id, pr.tool_name, selected_key));
        }
    }
    ...
    fn maybe_record_bash_prefix(&mut self) {
        ...
        {
            let prefix = suggested_prefix
                .as_deref()
                .and_then(Self::bash_command_prefix)
                .or_else(|| Self::bash_command_prefix(command));
            if let Some(prefix) = prefix {
                self.bash_prefix_allowlist.insert(prefix);
            }
        }
    }

    /// Returns `true` if the given bash `command` is covered by the session-local
    /// prefix allowlist (i.e. its parsed first word matches an entry in
    ...
    pub fn bash_command_allowed_by_prefix(&self, command: &str) -> bool {
        let Some(first_word) = Self::bash_command_prefix(command) else {
            return false;
        };
        self.bash_prefix_allowlist.contains(&first_word)
    }

    fn bash_command_prefix(command: &str) -> Option<String> {
        match mangocode_core::split_command_words(command) {
            Ok(words) => words.into_iter().find_map(|word| {
            ...
            Err(err) => {
                tracing::warn!(
                    error = %err,
                    "failed to parse bash command prefix for permission allowlist"
                );
                None
            }
        }
        ...
        loop {
            ...
            if let Some(ref mut rx) = self.model_fetch_rx {
                match rx.try_recv() {
                    ...
                    Ok(Err(message)) => {
                        self.report_model_load_error(message);
                        self.model_fetch_rx = None;
                    }
                    Err(tokio::sync::mpsc::error::TryRecvError::Disconnected) => {
                        self.report_model_load_error(
                            "fetch stopped before returning results".to_string(),
                        );
                        self.model_fetch_rx = None;
                    }
            ...
            if self.model_picker_fetch_pending {
                ...
                if provider_id_str == "ollama" {
                    ...
                    self.model_picker.load_error = None;
                    ...
                    if let Some(provider) = registry.get(&pid) {
                        ...
                        tokio::spawn(async move {
                            match provider.list_models().await {
                                ...
                                Err(err) => {
                                    let _ =
                                        tx.send(Err(format!("{}: {}", provider.name(), err))).await;
                                }
                            }
                        });
                    } else {
                        let message = format!(
                            "provider '{}' is not registered; connect or configure credentials first",
                            provider_id_str
                        );
                        self.report_model_load_error(message);
                    }
                } else {
                    self.report_model_load_error("provider registry is unavailable".to_string());
                }
            }
...
mod tests {
    ...
    #[test]
    fn test_fast_mode_setting_persistence_rejects_malformed_ui_settings() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("ui-settings.json");
        std::fs::write(&path, "{not-json").unwrap();

        let err = App::persist_fast_mode_setting_to_path(&path, false).unwrap_err();
    ...
    #[test]
    fn test_bash_prefix_allowlist_handles_quoted_executable_paths() {
        use crate::dialogs::PermissionRequest;
        use crossterm::event::{KeyCode, KeyEvent, KeyEventKind, KeyEventState, KeyModifiers};
        ...
        let pr = PermissionRequest::bash(
            ...
            r#""C:\Program Files\Mango Tool\tool.exe" --flag"#.to_string(),
            Some(r#""C:\Program Files\Mango Tool\tool.exe""#.to_string()),
        );
        ...
        assert!(
            app.bash_command_allowed_by_prefix(r#""C:\Program Files\Mango Tool\tool.exe" --other"#)
        );
        assert!(!app
            .bash_command_allowed_by_prefix(r#""C:\Program Files\Other Tool\tool.exe" --other"#));
    }

    #[test]
    fn test_bash_prefix_allowlist_rejects_malformed_command() {
        let mut app = make_app();
        app.bash_prefix_allowlist.insert("git".to_string());

        assert!(!app.bash_command_allowed_by_prefix(r#"git commit "unterminated"#));
    }
