...
/// Light rose accent (mascot)
const CLAUDE_ORANGE: Color = Color::Rgb(250, 114, 145);
...
pub fn parse_tool_input_json(input_json: &str) -> serde_json::Value {
    if input_json.trim().is_empty() {
        return serde_json::json!({});
    }
    serde_json::from_str(input_json).unwrap_or_else(|_| serde_json::json!({ "raw": input_json }))
}
...
pub fn render_tool_use(tool_name: &str, input_json: &str) -> Vec<Line<'static>> {
    let input = parse_tool_input_json(input_json);
...
fn render_tool_use_inner(tool_name: &str, input: &serde_json::Value) -> Vec<Line<'static>> {
    ...
    let mut header_spans = vec![
        Span::styled("  \u{2022} ".to_string(), Style::default().fg(Color::Green)),
        Span::styled(
            tool_name.to_string(),
            Style::default()
                .fg(Color::White)
...
pub fn render_tool_result_error(error: &str) -> Vec<Line<'static>> {
    ...
    if total_lines > 10 {
        lines.push(Line::from(vec![Span::styled(
            format!("  ... ({} more lines, Ctrl+O to expand)", total_lines - 10),
            Style::default()
                .fg(Color::DarkGray)
                .add_modifier(Modifier::DIM),
        )]));
...
mod tests {
    ...
    fn bash_tool_use_shows_bullet_header_and_command() {
        ...
        assert!(
            ...
            "tool name should appear in header"
        );
        assert!(
            !rendered.contains("* Bash"),
            "old generic header must not appear"
        );
        assert!(
            rendered.contains("ctrl+o"),
            "(ctrl+o to expand) hint should appear"
        );
    }
    ...
    fn test_render_user_memory_input() {
        let result = render_user_memory_input("project", "MangoCode");
        assert_eq!(result.len(), 2);
        let first = line_text(&result[0]);
        assert!(first.contains("# project: MangoCode"));
