...
static TABLE_ROW_PATTERN: Lazy<Result<Regex, regex::Error>> =
    Lazy::new(|| Regex::new(r"^\s*\|.+\|\s*$"));

/// Regex pattern to detect markdown table separator row (dashes/colons/pipes)
static TABLE_SEPARATOR_PATTERN: Lazy<Result<Regex, regex::Error>> =
    Lazy::new(|| Regex::new(r"^\s*\|\s*[:|-]+\s*(\|\s*[:|-]+\s*)*\|\s*$"));
...
pub fn detect_table(lines: &[&str], start_idx: usize) -> Option<(Table, usize)> {
    ...
    if !is_table_row(lines[start_idx]) {
        return None;
    }

    // Check if next line is a separator
    if !is_table_separator(lines[start_idx + 1]) {
        return None;
    }
    ...
    while end_idx < lines.len() && is_table_row(lines[end_idx]) {
...
fn is_table_row(line: &str) -> bool {
    if let Ok(pattern) = &*TABLE_ROW_PATTERN {
...
fn is_table_separator(line: &str) -> bool {
    if let Ok(pattern) = &*TABLE_SEPARATOR_PATTERN {
        return pattern.is_match(line);
    }
...
pub fn parse_inline_formatting(text: &str) -> Vec<Span<'static>> {
    ...
    while idx < chars.len() {
        // Check for URL (http://, https://, www.) — emit as cyan + underlined.
        if let Some(url_len) = match_url(&chars, idx) {
            if !current_text.is_empty() {
                spans.push(Span::raw(current_text.clone()));
                current_text.clear();
            }
            let url: String = chars[idx..idx + url_len].iter().collect();
        ...
        // Check for email (foo@bar.tld) — same styling as URLs.
        if let Some(email_len) = match_email(&chars, idx) {
            if !current_text.is_empty() {
                spans.push(Span::raw(current_text.clone()));
                current_text.clear();
            }
            let email: String = chars[idx..idx + email_len].iter().collect();
            spans.push(Span::styled(
                email,
                Style::default()
                    .fg(Color::Cyan)
...
/// Return the length (in chars) of a URL starting at `chars[idx]`, or None.
/// Matches `http://`, `https://`, or bare `www.` followed by a host.
fn match_url(chars: &[char], idx: usize) -> Option<usize> {
    fn starts_with(chars: &[char], idx: usize, prefix: &str) -> bool {
        let pchars: Vec<char> = prefix.chars().collect();
        if chars.len() < idx + pchars.len() {
            return false;
        }
        chars[idx..idx + pchars.len()] == pchars[..]
    }

    let prefix_len = if starts_with(chars, idx, "https://") {
        8
    } else if starts_with(chars, idx, "http://") {
        7
    } else if starts_with(chars, idx, "www.") {
        // Only treat bare www. as a URL at a word boundary so e.g.
        // "subdomain.www.foo" doesn't trigger inside another token.
        if idx > 0 && (chars[idx - 1].is_alphanumeric() || chars[idx - 1] == '.') {
            return None;
        }
        4
    } else {
    ...
    // Scan host/path body until whitespace or a clear terminator. Trim
    // trailing punctuation that's almost certainly sentence-final.
    ...
    // Trim trailing sentence punctuation so "see https://x.com." doesn't
    // include the period.
    while end > idx + prefix_len {
    ...
    // Require at least one body char and a dot somewhere in the host.
    let len = end - idx;
    if len <= prefix_len {
        return None;
    }
    if !chars[idx + prefix_len..end].contains(&'.') {
        return None;
    }
    Some(len)
}

/// Return the length (in chars) of an email starting at `chars[idx]`, or None.
/// Matches `local@host.tld` with conservative ASCII-ish character classes.
fn match_email(chars: &[char], idx: usize) -> Option<usize> {
    ...
    if !is_local(chars[idx]) {
        return None;
    }
    // Must NOT be inside a larger token (the previous char already counted as
    // local-class), to avoid eating half of `foo bar@baz`.
    if idx > 0 && is_local(chars[idx - 1]) {
        return None;
    }
    let mut local_end = idx;
    while local_end < chars.len() && is_local(chars[local_end]) {
        local_end += 1;
    }
    if local_end >= chars.len() || chars[local_end] != '@' || local_end == idx {
        return None;
    }
    let host_start = local_end + 1;
    let mut host_end = host_start;
    while host_end < chars.len() && is_host(chars[host_end]) {
        host_end += 1;
    }
    // Trim trailing dot (sentence punctuation immediately after a TLD).
    while host_end > host_start && chars[host_end - 1] == '.' {
        host_end -= 1;
    }
    if host_end == host_start {
        return None;
    }
    if !chars[host_start..host_end].contains(&'.') {
        return None;
    }
    Some(host_end - idx)
}
