...
use crate::overlays::{
    centered_rect, render_dark_overlay_buf, render_dialog_bg_buf, MANGOCODE_ACCENT, MANGOCODE_MUTED,
    MANGOCODE_PANEL_BG, MANGOCODE_PANEL_BORDER, MANGOCODE_TEXT,
};
...
pub fn render_mcp_view(state: &McpViewState, area: Rect, buf: &mut Buffer) {
    ...
    let title = Line::from(vec![
        Span::styled(" MCP", Style::default().fg(MANGOCODE_ACCENT).add_modifier(Modifier::BOLD)),
        Span::styled(
    ...
    let footer = Line::from(vec![
        ...
        Span::styled(" e ", Style::default().fg(MANGOCODE_ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("error detail  "),
        Span::styled(" r ", Style::default().fg(MANGOCODE_ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("reconnect"),
    ]);
    Paragraph::new(footer)
        .style(Style::default().bg(MANGOCODE_PANEL_BG).fg(MANGOCODE_MUTED))
        .render(layout[2], buf);
}
...
fn render_server_list(state: &McpViewState, area: Rect, buf: &mut Buffer) {
    ...
    let border_style = if focused {
        ...
        Style::default().fg(MANGOCODE_PANEL_BORDER)
    };
    Block::default()
        .title(Span::styled(
            " Servers ",
            Style::default()
                .fg(if focused { MANGOCODE_ACCENT } else { MANGOCODE_MUTED })
        ...
        *row += 1;
        for (idx, server) in group {
            ...
            let line = Line::from(vec![Span::styled(
                row_text,
                if sel {
                    Style::default().fg(Color::Black).bg(MANGOCODE_ACCENT).add_modifier(Modifier::BOLD)
                } else {
...
fn render_tool_list(state: &McpViewState, area: Rect, buf: &mut Buffer) {
    ...
    let search_line = Line::from(vec![
        Span::styled("/ ", Style::default().fg(MANGOCODE_ACCENT)),
        Span::styled(
            if state.tool_search.is_empty() { "filter tools".to_string() } else { state.tool_search.clone() },
            Style::default().fg(if state.tool_search.is_empty() { MANGOCODE_MUTED } else { MANGOCODE_TEXT }),
        ),
    ]);
