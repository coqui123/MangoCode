...
/// Compute a safe aspect ratio (h/w) from a crop rect, guarding against
/// degenerate width AND non-finite values (NaN / inf), either of which
/// would cascade into a zero-sized raster after the `as u32` cast.
fn safe_crop_aspect(crop: &SvgCropRect) -> f32 {
    if crop.w.partial_cmp(&0.001) != Some(std::cmp::Ordering::Greater) || !crop.h.is_finite() {
        return 1.0;
    }
    let ratio = crop.h / crop.w;
    if !ratio.is_finite() {
        return 1.0;
    }
    ratio.clamp(0.01, 100.0)
}
...
fn rasterize_svg_rgba_for_inline(target_w: u32) -> Option<(Vec<u8>, u32, u32)> {
    ...
    let aspect = safe_crop_aspect(&crop);
...
pub fn encode_svg_as_kitty_apc_with_width(target_w: u32) -> Option<String> {
    ...
    // Mascot uses the reserved image ID 1 (see kitty_image::MASCOT_IMAGE_ID).
    // With a stable ID, re-emissions REPLACE the stored image in the terminal's
    // graphics memory instead of accumulating a new image on every position
    // change, capping mascot memory at one copy regardless of session length.
    for (i, chunk) in b64.as_bytes().chunks(KITTY_CHUNK_SIZE).enumerate() {
        let more = if i + 1 == total { 0u8 } else { 1u8 };
        let params = if i == 0 {
            format!("a=T,f=100,i=1,m={},q=2,C=1", more)
        } else {
            format!("a=T,i=1,m={},q=2", more)
        };
...
/// Content aspect ratio (height / width) of the mascot's tight bounds.
fn mascot_content_aspect() -> Option<f32> {
    let tree = SVG_TREE.as_ref()?;
    let svg_size = tree.size();
    let crop = SVG_CONTENT_BOUNDS.unwrap_or(SvgCropRect {
        x: 0.0,
    ...
    Some(safe_crop_aspect(&crop))
}

/// Number of terminal cell rows an inline mascot rasterized at `target_w`
/// pixels wide occupies, given the terminal's cell pixel height. Used to size
/// the welcome box so it hugs the image instead of padding below it.
pub fn mascot_inline_rows(target_w: u32, cell_px_h: f32) -> u16 {
    let cell = if cell_px_h.is_finite() && cell_px_h > 1.0 {
        cell_px_h
    } else {
        16.0
    };
    let aspect = mascot_content_aspect().unwrap_or(339.0 / 282.0);
    ((target_w as f32 * aspect) / cell).round().max(1.0) as u16
}

/// Encode the mascot inline image sized to occupy approximately `rows` terminal
/// rows, using the best available inline protocol.
///
/// Returns `(escape, actual_rows)` where `actual_rows` is how many cell rows the
/// image occupies — callers that track the cursor manually (the hybrid
/// scrollback backend, which prints the greeting inline so it scrolls with the
/// transcript) use it to keep their row counter in sync with the rendered image.
/// Returns `None` on text-only terminals or if rasterization fails.
pub fn encode_mascot_inline_for_rows(rows: u16, cell_px_h: f32) -> Option<(String, u16)> {
    use crate::kitty_image::{detect_image_protocol, ImageProtocol};

    let rows = rows.max(2);
    let cell_px_h = if cell_px_h.is_finite() && cell_px_h > 1.0 {
        cell_px_h
    } else {
        16.0
    };

    let aspect = mascot_content_aspect()?;
    // Pick a raster width whose aspect-derived height matches the requested rows.
    let target_h_px = rows as f32 * cell_px_h;
    let target_w = ((target_h_px / aspect).round() as u32).clamp(128, 1024);
    // Recompute the rows the chosen width actually yields so the caller advances
    // the cursor by the true image height.
    let actual_rows = ((target_w as f32 * aspect / cell_px_h).round() as u16).max(1);

    let escape = match detect_image_protocol() {
        ImageProtocol::Kitty => encode_svg_as_kitty_apc_with_width(target_w)?,
        ImageProtocol::Iterm => encode_svg_as_iterm_osc1337_with_width(target_w)?,
        ImageProtocol::Sixel => encode_svg_as_sixel_with_width(target_w)?,
        ImageProtocol::Text => return None,
    };
    Some((escape, actual_rows))
}
...
fn fallback_lines() -> Vec<Line<'static>> {
    vec![
        Line::from(Span::styled(
            "  🥭 MangoCode  ",
            Style::default().fg(Color::Rgb(250, 114, 145)),
        )),
        Line::from(""),
    ]
}
