...
pub const MANGOCODE_ACCENT: Color = Color::Rgb(250, 114, 145); // #FA7291 light rose (mascot)
    ...
    for y in area.y..area.y + area.height {
        for x in area.x..area.x + area.width {
            if let Some(cell) = buf.cell_mut((x, y)) {
                cell.set_bg(MANGOCODE_OVERLAY_BG);
                cell.set_fg(MANGOCODE_MUTED);
            }
        }
    }
}
...
pub fn render_help_overlay(frame: &mut Frame, overlay: &HelpOverlay, area: Rect) {
    ...
    let block = Block::default()
        ...
        .style(Style::default().bg(MANGOCODE_PANEL_BG).fg(MANGOCODE_TEXT));
...
// ---------------------------------------------------------------------------
// Pinned-entry persistence  (~/.mangocode/history_pins.json)
// ---------------------------------------------------------------------------
...
/// Load the set of pinned entry texts from `~/.mangocode/history_pins.json`.
...
pub fn load_pinned_texts() -> std::collections::HashSet<String> {
    ...
    match serde_json::from_str::<std::collections::HashSet<String>>(&content) {
        Ok(pins) => pins,
        Err(err) => {
            tracing::warn!(
                path = %path.display(),
                error = %err,
                "failed to parse history pins"
            );
...
/// Failures are logged but otherwise ignored (best-effort).
    ...
    if let Some(parent) = path.parent() {
        if let Err(err) = std::fs::create_dir_all(parent) {
            tracing::warn!(
                error = %err,
                path = %parent.display(),
                "failed to create history pin directory"
            );
            return;
        }
    }
    let json = match serde_json::to_string_pretty(pinned_texts) {
        Ok(json) => json,
        Err(err) => {
            tracing::warn!(error = %err, "failed to serialize history pins");
            return;
        }
    };
...
impl HistorySearchOverlay {
    ...
    ///
    /// Pinned state is loaded from `~/.mangocode/history_pins.json` and applied
...
pub fn render_message_selector(frame: &mut Frame, overlay: &MessageSelectorOverlay, area: Rect) {
    ...
    if overlay.messages.is_empty() {
        ...
        for (display_i, msg) in overlay.messages[start..end].iter().enumerate() {
            ...
            let preview_max = (dialog_width as usize).saturating_sub(20).max(1);
