...
fn write_text_macos_w(text: &str) -> bool {
    ...
    let child = match Command::new("pbcopy")
        .stdin(Stdio::piped())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
    {
        Ok(c) => c,
        Err(err) => {
            tracing::debug!(error = %err, tool = "pbcopy", "clipboard helper unavailable");
            return false;
        }
    };
    finish_clipboard_writer_w(child, "pbcopy", text)
}
...
fn write_text_windows_w(text: &str) -> bool {
    ...
    finish_clipboard_writer_w(child, "powershell Set-Clipboard", text)
}
...
fn write_text_linux_w(text: &str) -> bool {
    ...
    for (prog, args) in &[
        ...
        if let Ok(child) = Command::new(prog)
            ...
            .spawn()
        {
            if finish_clipboard_writer_w(child, prog, text) {
                return true;
            }
        }
    }
...
#[cfg(not(test))]
fn finish_clipboard_writer_w(mut child: std::process::Child, tool: &str, text: &str) -> bool {
    use std::io::Write;
