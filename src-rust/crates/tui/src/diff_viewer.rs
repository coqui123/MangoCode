...
use mangocode_core::{parse_git_diff_new_path, parse_unified_diff_marker_path};
    ...
    for raw_line in text.lines() {
        if raw_line.starts_with("diff --git ") {
            ...
            let path = raw_line
                ...
                .and_then(parse_git_diff_new_path)
        ...
        } else if current_hunk.is_none() && raw_line.starts_with("--- ") {
            if let Some(f) = current_file.as_mut() {
                if f.path == "unknown" {
                    if let Some(path) = parse_unified_diff_marker_path(&raw_line[4..]) {
                        f.path = path;
                    }
                }
            }
...
fn parse_hunk_header(line: &str) -> (u32, u32, u32, u32) {
    ...
    let parse_range = |s: &str| -> (u32, u32) {
        ...
        if let Some((start_str, count_str)) = s.split_once(',') {
            let start = start_str.parse().unwrap_or(1);
...
mod tests {
    ...
    fn parse_unified_diff_preserves_paths_with_spaces() {
        let text = "diff --git a/docs/my file.md b/docs/my file.md\n\
                    index 1111111..2222222 100644\n\
                    --- a/docs/my file.md\n\
                    +++ b/docs/my file.md\n\
                    @@ -1,1 +1,1 @@\n\
        ...
        let files = parse_unified_diff(text);
        assert_eq!(files.len(), 1);
        assert_eq!(files[0].path, "docs/my file.md");
        assert_eq!(files[0].added, 1);
        assert_eq!(files[0].removed, 1);
    }

    #[test]
    fn parse_unified_diff_uses_marker_path_when_header_is_ambiguous() {
        let text = "diff --git a/docs/foo b/bar.md b/docs/new name.md\n\
                    index 1111111..2222222 100644\n\
                    --- a/docs/foo b/bar.md\n\
                    +++ b/docs/new name.md\n\
                    @@ -1,1 +1,1 @@\n\
        ...
        let files = parse_unified_diff(text);
        assert_eq!(files.len(), 1);
        assert_eq!(files[0].path, "docs/new name.md");
    }

    #[test]
    fn parse_unified_diff_counts_marker_like_hunk_lines() {
        let text = "diff --git a/docs/file.md b/docs/file.md\n\
                    index 1111111..2222222 100644\n\
                    --- a/docs/file.md\n\
                    +++ b/docs/file.md\n\
                    @@ -1,1 +1,1 @@\n\
        ...
        assert_eq!(files[0].added, 1);
        assert_eq!(files[0].removed, 1);
        assert_eq!(files[0].hunks[0].lines[1].content, "-- removed heading");
        assert_eq!(files[0].hunks[0].lines[2].content, "++ added heading");
    }
