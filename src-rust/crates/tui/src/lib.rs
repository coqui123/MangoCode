// mangocode-tui: Terminal UI using ratatui + crossterm for MangoCode.
//
...
/// Memory update notification banner (shown after MangoCode updates a AGENTS.md file).
...
pub fn init_mascot(app: &mut App) {
    ...
    // Record the image's cell-row height so the welcome box can reserve exactly
    // that many rows (no gap below the mascot).
    app.mascot_rows = if app.mascot_sixel.is_some() {
        let cell_px_h = crossterm::terminal::window_size()
            .ok()
            .filter(|ws| ws.rows != 0 && ws.height != 0)
            .map(|ws| ws.height as f32 / ws.rows as f32)
            .unwrap_or(16.0);
        crate::rustle::mascot_inline_rows(target_w, cell_px_h)
    } else {
        0
    };
}
...
pub fn setup_terminal_with_capabilities(
    ...
    // On failure, undo anything already enabled before returning the error.
...
mod tests {
    ...
    fn test_render_app_keeps_logo_header_after_first_message() {
        ...
        let mut app = make_app();
        app.push_message(mangocode_core::types::Message::user("hello".to_string()));
        ...
        assert!(rendered.contains("MangoCode"));
    ...
    fn test_render_transcript_search_highlights_current_distinctly() {
        ...
        for cell in &terminal.backend().buffer().content {
            ...
            if cell.style().bg == Some(Color::Rgb(250, 114, 145)) {
    ...
    fn test_message_renderer_includes_tool_use_and_thinking_blocks() {
        let msg = mangocode_core::types::Message::assistant_blocks(vec![
    ...
    fn test_message_renderer_includes_tool_result_errors() {
        let msg = mangocode_core::types::Message::user_blocks(vec![ContentBlock::ToolResult {
    ...
    fn test_handle_status_event() {
        let mut app = make_app();
        app.handle_query_event(mangocode_query::QueryEvent::Status("working".to_string()));
    ...
    fn test_handle_error_event() {
        let mut app = make_app();
        app.is_streaming = true;
        app.handle_query_event(mangocode_query::QueryEvent::Error("oops".to_string()));
        assert!(!app.is_streaming);
        assert_eq!(app.messages.len(), 1);
    ...
    fn test_handle_tool_start_and_end() {
        let mut app = make_app();
        app.handle_query_event(mangocode_query::QueryEvent::ToolStart {
    ...
    fn test_turn_complete_flushes_streaming_text() {
        ...
        app.handle_query_event(mangocode_query::QueryEvent::TurnComplete {
            turn: 1,
