// memory_file_selector.rs — Memory file selector overlay.
...
use crate::overlays::{
    centered_rect, render_dark_overlay_buf, render_dialog_bg_buf, MANGOCODE_ACCENT,
    MANGOCODE_MUTED, MANGOCODE_PANEL_BG, MANGOCODE_TEXT,
};
...
impl MemoryFileSelectorState {
    ...
    /// - User:    `~/.mangocode/AGENTS.md`
    /// - Project: `{project_root}/AGENTS.md`
    /// - Local:   `{project_root}/.mangocode/AGENTS.md`
    ///
    ...
    pub fn open(&mut self, project_root: &std::path::Path) {
        ...
        // User-level: ~/.mangocode/AGENTS.md
        let user_path = mangocode_core::config::Settings::config_dir().join("AGENTS.md");
        let user_display = {
        ...
        // Local-level: {project_root}/.mangocode/AGENTS.md
        let local_path = project_root.join(".mangocode").join("AGENTS.md");
...
pub fn render_memory_file_selector(state: &MemoryFileSelectorState, area: Rect, buf: &mut Buffer) {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(" — choose a file", Style::default().fg(MANGOCODE_MUTED)),
        Span::styled(
            format!(
                "{:>width$}",
                "Esc close",
    ...
    for (i, file) in state.files.iter().enumerate() {
        ...
        let new_tag = if !file.exists {
            Span::styled(" (new)", Style::default().fg(MANGOCODE_MUTED))
        } else {
            Span::raw("")
        };
