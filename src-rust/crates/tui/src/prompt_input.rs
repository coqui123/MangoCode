...
const CLAUDE_ORANGE: Color = Color::Rgb(226, 39, 92); // #E2275C mascot rose-red prompt accent
        ...
        VimPendingState::Indent { dir, count } => {
            if key == dir.to_string().as_str() {
                ...
                for i in 0..count.max(1) {
                    ...
                    } else if let Some(stripped) = new_lines[idx].strip_prefix(indent) {
                        new_lines[idx] = stripped.to_string();
                    } else {
...
fn vim_count(
    ...
    if key.len() == 1 && key.chars().next().is_some_and(|ch| ch.is_ascii_digit()) {
...
fn vim_normal(
    ...
    match key {
        ...
        // ---- dd/yy (multi-char operator handling) ----
        "dd" => {
            let ls = text[..*cursor].rfind('\n').map(|p| p + 1).unwrap_or(0);
            let le = text[*cursor..]
                .find('\n')
    ...
    pub fn vim_command(&mut self, key: &str) {
        ...
        match self.vim_pending.clone() {
            ...
            VimPendingState::Mark => {
                // `m<char>` — set mark
                if key.len() == 1 {
                    if let Some(name) = key.chars().next() {
                        self.set_mark(name);
                    }
                }
                self.vim_pending = VimPendingState::None;
...
/// Render the prompt input widget in the same low-chrome style as MangoCode:
...
pub fn render_prompt_input(
    ...
    let accent = match mode {
        InputMode::Readonly => CLAUDE_ORANGE, // orange = locked while MangoCode responds
    ...
    for (i, line_text) in logical_lines.iter().enumerate() {
        ...
        // Inline ghost-text completion for slash commands: when the active
        // suggestion's name extends the current input, render the suffix in
        // dim color after the cursor (fish-shell style).
        let ghost_text: Option<String> = if i == 0
            && focused
            && state.text.starts_with('/')
            && state.cursor == state.text.len()
            && !state.suggestions.is_empty()
        {
            let idx = state.suggestion_index.unwrap_or(0);
            state.suggestions.get(idx).and_then(|s| {
                if s.source == TypeaheadSource::SlashCommand
                    && s.text.len() > state.text.len()
                    && s.text
                        .to_lowercase()
                        .starts_with(&state.text.to_lowercase())
                {
                    let suffix = &s.text[state.text.len()..];
                    let used = visible.chars().count();
                    let remaining = available_width.saturating_sub(used);
                    if remaining == 0 {
                        None
                    } else {
                        Some(suffix.chars().take(remaining).collect())
                    }
                } else {
                    None
                }
        ...
        let spans: Vec<Span<'static>> = if i == 0 {
            ...
            if let Some(g) = ghost_text {
                v.push(Span::styled(g, Style::default().fg(Color::DarkGray)));
            }
            v
        } else {
