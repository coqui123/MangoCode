...
/// A single entry in ~/.mangocode/stats.jsonl
...
/// Load and aggregate stats from ~/.mangocode/stats.jsonl
pub fn load_stats() -> AggregatedStats {
    let path = dirs::home_dir()
        .map(|h| h.join(".mangocode").join("stats.jsonl"))
        .unwrap_or_default();
...
fn render_cost_heatmap(data: &AggregatedStats, area: Rect, buf: &mut Buffer) {
    ...
    // We group into chunks of 7 calendar days (by index)
