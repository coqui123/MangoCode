...
impl<T: VirtualItem> VirtualList<T> {
    ...
    /// Total height of all items at `width`. Saturates at `u16::MAX` to avoid overflow.
    pub fn total_height(&mut self, width: u16) -> u16 {
        let mut total: u32 = 0;
        for i in 0..self.items.len() {
            total = total.saturating_add(self.item_height(i, width) as u32);
    ...
    pub fn sticky_header_index(&mut self, width: u16) -> Option<usize> {
        ...
        for i in 0..self.items.len() {
            ...
            if row.saturating_add(h) > self.scroll_offset {
                break;
            }
    ...
    pub fn render(&mut self, area: Rect, buf: &mut Buffer) {
        ...
        for idx in 0..self.items.len() {
            ...
            let item_end = current_row.saturating_add(h);
            ...
            if current_row >= self.scroll_offset.saturating_add(area.height) {
                break;
            }
            ...
            let visible_rows = h
                ...
                .min(bottom.saturating_sub(screen_row));
