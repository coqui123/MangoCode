        ...
        for m in models {
            let provider = if let Some((p, _)) = m.split_once('/') {
                ...
                if m.contains("claude") {
                    ...
                    "google".to_string()
                } else if m.to_ascii_lowercase().contains("minimax") {
                    "minimax".to_string()
                } else {
                    "other".to_string()
                }
...
pub fn models_for_provider(provider_id: &str) -> Vec<ModelEntry> {
    match provider_id {
        ...
        // google-vertex: fully dynamic — list_models() queries the Vertex
        // Publisher Models API across all publishers (Google, Anthropic,
        // Mistral, Meta, etc.) and replaces this placeholder immediately.
        // Return empty so the picker shows only the "Loading models…" spinner.
        "google-vertex" => vec![],
...
pub fn default_model_for_provider(provider_id: &str) -> String {
    match provider_id {
        ...
        "google-vertex" => "google/gemini-2.5-flash".to_string(),
...
pub struct ModelPickerState {
    ...
    /// Last dynamic model-list load error, shown inline while the picker is open.
...
impl ModelPickerState {
    ...
    /// Mark the dynamic model-list load as failed and expose the reason inline.
    ...
    /// never left empty.
    pub async fn fetch_models(client: &mangocode_api::AnthropicClient) -> Vec<ModelEntry> {
...
pub fn render_model_picker(state: &ModelPickerState, area: Rect, buf: &mut Buffer) {
    ...
    let _pink = Color::Rgb(250, 114, 145); // golden mango accent
    let dim = Color::Rgb(138, 125, 115); // warm muted
    let dialog_bg = Color::Rgb(26, 20, 15); // warm dark brown
    let highlight_bg = Color::Rgb(250, 114, 145);
    let highlight_fg = Color::Rgb(26, 20, 15); // dark text on light-rose bg
    ...
    if let Some(error) = state.load_error.as_deref() {
        lines.push(Line::from(vec![Span::styled(
            format!(
                ...
                mangocode_core::truncate::truncate_bytes_prefix(error, 120)
            ),
            Style::default().fg(Color::Yellow),
        )]));
    }
...
mod tests {
    ...
    #[test]
    fn model_load_error_stops_loading_and_is_cleared_by_new_models() {
        let mut p = ModelPickerState::new();
        p.loading_models = true;

        p.set_load_error("network unavailable".to_string());

        assert!(!p.loading_models);
        assert_eq!(p.load_error.as_deref(), Some("network unavailable"));
