...
// 7. Save OAuthTokens to ~/.mangocode/oauth_tokens.json
// 8. Return the credential (API key or Bearer token)

use anyhow::{bail, Context};
use mangocode_core::oauth::{self, OAuthTokens};
...
async fn run_callback_server(
    ...
    // Show the same browser UX page regardless of callback validation result.
...
async fn read_line_from_stdin() -> anyhow::Result<String> {
    ...
    if let Err(err) = std::io::stdout().flush() {
        warn!(error = %err, "failed to flush OAuth manual-code prompt");
    }
...
async fn exchange_code_for_tokens(
    ...
    if !resp.status().is_success() {
        ...
        let text = response_error_body(resp, "token exchange").await;
...
async fn create_api_key(access_token: &str) -> anyhow::Result<String> {
    ...
    if !resp.status().is_success() {
        let status = resp.status();
        let text = response_error_body(resp, "API key creation").await;
...
async fn wait_for_auth_code_impl(
    ...
    tokio::spawn(async move {
        if let Ok(line) = read_line_from_stdin().await {
            // The manually-displayed code may carry a "#state" fragment
            // (code#state); keep only the bare authorization code before it.
            let trimmed = line
                .trim()
                .split('#')
                .next()
