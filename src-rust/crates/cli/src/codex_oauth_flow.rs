...
fn authorization_code_from_params(params: &HashMap<String, String>) -> anyhow::Result<&str> {
...
fn oauth_callback_page(title: &str, message: &str, tone: &str) -> String {
...
body {{ margin: 0; min-height: 100vh; display: grid; place-items: center; padding: 24px; font-family: system-ui, -apple-system, Segoe UI, sans-serif; background: #f4f6f5; color: #1d2528; }}
main {{ width: min(100%, 440px); border: 1px solid #d6ddd9; border-radius: 8px; background: #ffffff; padding: 22px; box-shadow: 0 10px 30px rgba(29, 37, 40, 0.08); }}
.status {{ display: inline-block; margin-bottom: 14px; padding: 4px 8px; border-radius: 6px; font-size: 12px; font-weight: 650; }}
.success {{ color: #0f5d58; background: #e5f0ec; }}
.error {{ color: #8f1d1d; background: #f8e7e7; }}
.neutral {{ color: #42534c; background: #edf1ef; }}
h1 {{ margin: 0 0 8px; font-size: 22px; line-height: 1.2; }}
p {{ margin: 0; color: #5f6f68; line-height: 1.5; }}
</style>
</head>
<body>
<main>
<div class="status {tone_class}">MangoCode authentication</div>
<h1>{title}</h1>
<p>{message}</p>
</main>
</body>
...
async fn accept_oauth_callback(
    ...
    let query = match parse_request_query_line(request_line.trim()) {
        Ok(query) => query,
        Err(err) => {
            let html =
                oauth_callback_page("Login failed", "Invalid OAuth callback request.", "error");
    ...
    if let Some(err) = params.get("error") {
        ...
        let html = oauth_callback_page("Login failed", &desc, "error");
        respond_html_ok(&mut socket, &html).await?;
        bail!(
            "OAuth error: {}",
    ...
    if params.get("state").map(|s| s.as_str()) != Some(expected_state) {
        let html = oauth_callback_page("Login failed", "OAuth state mismatch.", "error");
        respond_html_ok(&mut socket, &html).await?;
        bail!("OAuth state mismatch");
    }

    let code = match authorization_code_from_params(&params) {
        Ok(code) => code,
        Err(err) => {
            let html = oauth_callback_page("Login failed", &err.to_string(), "error");
            respond_html_ok(&mut socket, &html).await?;
            return Err(err);
        }
    };

    let html = oauth_callback_page("Login complete", "You can return to MangoCode.", "success");
    respond_html_ok(&mut socket, &html).await?;
    Ok(code.to_string())
}
...
pub async fn exchange_code_for_tokens(code: &str, verifier: &str) -> anyhow::Result<CodexTokens> {
    ...
    if !resp.status().is_success() {
        ...
        let body = response_error_body(resp, "Codex token exchange").await;
...
pub async fn run_oauth_flow(
    ...
    if let Err(err) = open::that(&auth_url) {
        ...
        warn!(
            error = %error,
            auth_url = %auth_url,
            "failed to open Codex OAuth URL in browser"
        );
        let _ = event_tx
            .send(DeviceAuthEvent::BrowserOpenFailed {
                url: auth_url.clone(),
                error,
            })
            .await;
...
mod tests {
    ...
    fn parse_query_params_roundtrip() {
        ...
        let with_form_encoding = parse_query_params("error_description=bad+request%21&=ignored");
        assert_eq!(
            with_form_encoding
                .get("error_description")
                .map(String::as_str),
        ...
        assert!(!with_form_encoding.contains_key(""));
    }

    #[test]
    fn authorization_code_requires_present_non_blank_code() {
        let good = parse_query_params("code=abc&state=xyz");
        assert_eq!(authorization_code_from_params(&good).unwrap(), "abc");

        let padded = parse_query_params("code=%20abc%20&state=xyz");
        assert_eq!(authorization_code_from_params(&padded).unwrap(), "abc");

        let missing = parse_query_params("state=xyz");
        assert!(authorization_code_from_params(&missing).is_err());

        let blank = parse_query_params("code=%20%20&state=xyz");
        assert!(authorization_code_from_params(&blank).is_err());
    }

    #[test]
    fn callback_request_line_requires_query_string() {
        assert!(parse_request_query_line("GET /callback HTTP/1.1").is_err());
        assert_eq!(
            parse_request_query_line("GET /callback?code=abc&state=xyz HTTP/1.1").unwrap(),
            "code=abc&state=xyz"
        );
    }

    #[test]
    fn oauth_callback_page_is_responsive_and_escaped() {
        let html = oauth_callback_page("Login <complete>", "Use MangoCode & continue.", "success");

        assert!(html.contains("<!doctype html>"));
        assert!(html.contains("name=\"viewport\""));
        assert!(html.contains("rel=\"icon\""));
        assert!(html.contains("Login &lt;complete&gt;"));
        assert!(html.contains("Use MangoCode &amp; continue."));
        assert!(html.contains("class=\"status success\""));
    }
