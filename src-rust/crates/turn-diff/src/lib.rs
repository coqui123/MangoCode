...
impl RollbackResult {
    /// True when the rollback could not fully revert the turn — a binary file
    /// (not recoverable from the text history), a failed delete/restore, or a
    /// missing snapshot. `rollback_turn` returns `Ok` even in this case so the
    /// `reverted_paths`/`errors` detail is preserved; callers MUST check this
    /// (or inspect `errors`) and surface the partial failure rather than
    /// assuming the turn was fully undone.
...
/// Revert the file changes made during a turn.
///
/// Returns `Ok` even when some paths could not be reverted (binary files, failed
/// writes, missing snapshots); those are collected in `RollbackResult::errors`.
/// Callers should check [`RollbackResult::is_partial_failure`] and surface the
/// partial failure — an `Ok` return does NOT guarantee the turn was fully undone.
...
mod tests {
    ...
    #[test]
    fn rollback_result_flags_partial_failure() {
        let clean = RollbackResult {
            reverted_paths: vec![PathBuf::from("a.rs")],
            errors: vec![],
        };
        ...
        let partial = RollbackResult {
            reverted_paths: vec![],
            errors: vec!["Cannot rollback binary file foo.png".to_string()],
        };
