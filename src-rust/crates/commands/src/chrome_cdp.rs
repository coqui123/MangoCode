...
async fn fetch_json(client: &reqwest::Client, url: &str) -> anyhow::Result<Value> {
    ...
    if !r.status().is_success() {
        let status = r.status();
        let body = response_error_body(r, "Chrome DevTools JSON").await;
        let body = body.trim();
        if body.is_empty() {
...
fn merge_key_event(base: &Value, typ: &str, include_text: bool) -> Value {
    let Some(base_obj) = base.as_object() else {
        tracing::warn!(
            "Input.dispatchKeyEvent base payload was not an object; using minimal event"
        );
        return json!({ "type": typ });
    };
...
fn insert_text_payload(ch: char) -> Option<Value> {
    if !ch.is_control() && !ch.is_ascii() {
        return Some(json!({ "text": ch.to_string() }));
    }
    None
}
    ...
    let result = async {
        ...
        for ch in text.chars() {
            if ch == '\n' || ch == '\r' {
                ...
                .await?;
            } else if let Some(payload) = insert_text_payload(ch) {
                cdp_call(&mut s, "Input.insertText", payload).await?;
            }
        }
    ...
    let result = async {
        ...
        if let Some(t) = prompt_text {
            if let Some(obj) = p.as_object_mut() {
                obj.insert("promptText".to_string(), json!(t));
            }
        }
...
mod tests {
    ...
    #[test]
    fn merge_key_event_sets_type_and_drops_text_when_requested() {
        let base = json!({
            "key": "a",
            "code": "KeyA",
            "text": "a",
        ...
        let merged = merge_key_event(&base, "keyDown", false);

        assert_eq!(merged["type"], "keyDown");
        assert_eq!(merged["key"], "a");
        assert!(merged.get("text").is_none());
    }

    #[test]
    fn merge_key_event_keeps_text_for_char_events() {
        let base = json!({
            "key": "a",
            "text": "a",
        });

        let merged = merge_key_event(&base, "char", true);

        assert_eq!(merged["type"], "char");
        assert_eq!(merged["text"], "a");
    }

    #[test]
    fn merge_key_event_handles_non_object_payloads_without_panicking() {
        let merged = merge_key_event(&json!("not-object"), "keyUp", false);

        assert_eq!(merged, json!({ "type": "keyUp" }));
    }

    #[test]
    fn insert_text_payload_handles_non_ascii_printable_chars() {
        assert_eq!(insert_text_payload('é'), Some(json!({ "text": "é" })));
        assert_eq!(insert_text_payload('界'), Some(json!({ "text": "界" })));
        assert_eq!(insert_text_payload('a'), None);
        assert_eq!(insert_text_payload('\n'), None);
    }
}
