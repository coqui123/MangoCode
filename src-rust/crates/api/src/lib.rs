// mangocode-api: Anthropic API client with streaming SSE support for MangoCode
// Rust port.
//
// Handles:
...
pub use provider_error::{
    diagnostic_from_claude_error, format_provider_diagnostic, ProviderDiagnostic,
    ProviderDiagnosticKind, ProviderError,
};
...
static WORKLOAD_OVERRIDE: std::sync::OnceLock<String> = std::sync::OnceLock::new();

/// Set a process-wide billing workload tag (from `--workload`). It takes effect
/// for every request, including those issued by providers that build their own
/// `ClientConfig` (e.g. Claude Max OAuth, minimax) and never see the CLI value.
pub fn set_workload_override(workload: String) {
    let _ = WORKLOAD_OVERRIDE.set(workload);
}

/// The effective billing workload tag: the process-wide override if set,
/// otherwise the per-client configured value.
fn effective_workload(config_workload: &str) -> String {
    WORKLOAD_OVERRIDE
        .get()
        .cloned()
...
pub use providers::copilot_pirate_tools::{
    copilot_agent_recovery_user_message, copilot_response_text_is_bad,
    is_copilot_tool_protocol_error, COPILOT_AGENT_RECOVERY_LIMIT,
    COPILOT_PROTOCOL_EXHAUSTED_RECOVERY_MSG,
};
pub use providers::openai_compat::is_copilot_pirate_backend;
pub use providers::{
...
pub mod client {
    ...
    pub struct ClientConfig {
        ...
        /// Billing workload tag sent in the `x-anthropic-billing-header`
        /// (`cc_workload=<tag>`). Overridable via `--workload`.
        pub workload: String,
    }
    ...
    impl Default for ClientConfig {
        fn default() -> Self {
            Self {
                api_key: String::new(),
                api_base: mangocode_core::constants::ANTHROPIC_API_BASE.to_string(),
                ...
                workload: "claude_code".to_string(),
            }
        }
    }
    ...
    impl AnthropicClient {
        /// Build a new client. Validation of the API key is deferred to the
        /// first API call so non-Anthropic setups can still initialize.
        ...
        /// Build a client, falling back to reqwest's default client if the
        /// environment-specific builder fails.
        pub fn new_lossy(config: ClientConfig) -> Self {
            match Self::new(config.clone()) {
                Ok(client) => client,
                Err(err) => {
                    tracing::warn!(
                        "failed to build configured Anthropic HTTP client; using default reqwest client: {err}"
                    );
                    Self {
        ...
        pub fn from_config(cfg: &mangocode_core::config::Config) -> anyhow::Result<Self> {
            let api_key = cfg
                .resolve_api_key()
        ...
        async fn send_with_retry(&self, body: &Value) -> Result<reqwest::Response, ClaudeError> {
            ...
            loop {
                ...
                // Honor a process-wide --workload override even for providers
                // that build their own ClientConfig and never saw the CLI value.
                let workload = crate::effective_workload(&self.config.workload);
                let billing_header = format!(
                    "cc_version=0.1; cc_entrypoint=claude_code; {}; cc_workload={};",
                    cch_hash, workload
                );
                ...
                // Required session-identity headers for Claude Max OAuth Bearer auth.
                // The OAuth path validates these headers before accepting Bearer tokens.
        ...
        fn frame_to_event(
            ...
            match event_name {
                ...
                "message_start" => {
                    let v: Value = Self::parse_sse_json(event_name, data)?;
                    let msg = v.get("message")?;
                    let id = msg.get("id")?.as_str()?.to_string();
                    let model = msg.get("model")?.as_str()?.to_string();
                    let usage = msg
                        .get("usage")
                        .and_then(|u| Self::parse_sse_field(event_name, "message.usage", u))
                        .unwrap_or_default();
                ...
                "content_block_start" => {
                    ...
                    let content_block: ContentBlock =
                        Self::parse_sse_field(event_name, "content_block", block_value)?;
                ...
                "content_block_delta" => {
                    ...
                    let delta: streaming::ContentDelta =
                        Self::parse_sse_field(event_name, "delta", delta_value)?;
                ...
                "message_delta" => {
                    ...
                    let usage = v
                        .get("usage")
                        .and_then(|u| Self::parse_sse_field(event_name, "usage", u));
        ...
        fn parse_sse_json(event_name: &str, data: &str) -> Option<Value> {
        ...
        fn parse_sse_field<T>(event_name: &str, field: &str, value: &Value) -> Option<T>
        where
            T: DeserializeOwned,
        {
            match serde_json::from_value(value.clone()) {
                ...
                Err(err) => {
                    warn!(
                        ...
                        field,
                        error = %err,
                        "failed to parse Anthropic SSE event field"
                    );
                    None
                }
            }
    ...
    mod tests {
        ...
        #[test]
        fn frame_to_event_keeps_message_start_when_usage_is_malformed() {
            let event_type = Some("message_start".to_string());
            let event = AnthropicClient::frame_to_event(
                &event_type,
                r#"{"message":{"id":"m1","model":"claude","usage":42}}"#,
            )
            .expect("message_start should still parse with default usage");

            match event {
                streaming::AnthropicStreamEvent::MessageStart { id, model, usage } => {
                    assert_eq!(id, "m1");
        ...
        #[test]
        fn frame_to_event_drops_malformed_content_block() {
            let event_type = Some("content_block_start".to_string());
            let event = AnthropicClient::frame_to_event(
                &event_type,
                r#"{"index":0,"content_block":{"type":"tool_use","id":"tool-1"}}"#,
            );
                ...
                if let Some(partial) = self.partials.remove(index) {
                    let block = match partial {
                        ...
                        PartialBlock::ToolUse { id, name, json_buf } => {
                            // Streamed tool-call arguments are accumulated as
                            // fragments; weak local models often truncate the final
                            // JSON. Try a strict parse, then a structural repair
                            // (close strings/braces, drop trailing commas) before
                            // falling back to an empty object. Repair is logged
                            // loudly: a structurally-repaired call can be valid JSON
                            // yet carry a truncated value (e.g. a clipped path), so
                            // the recovery must be visible rather than silent.
                            let input = match serde_json::from_str(&json_buf) {
                                Ok(value) => value,
                                Err(_) => {
                                    match crate::providers::openai::repair_tool_call_arguments(
                                        &json_buf,
                                    ) {
                                        Some(value) => {
                                            tracing::warn!(
                                                tool = %name,
                                                "repaired truncated streamed tool-call arguments; \
                                                 verify the call used the intended values"
                                            );
                                            value
                                        }
                                        None => {
                                            tracing::warn!(
                                                tool = %name,
                                                "streamed tool-call arguments were unparseable; \
                                                 using empty object"
                                            );
