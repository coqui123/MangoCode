...
use mangocode_core::types::{ContentBlock, MessageContent, Role, ToolResultContent, UsageInfo};
...
impl BedrockProvider {
    ...
    fn sign_request(
        ...
        let parsed = match url::Url::parse(url_str) {
            Ok(parsed) => parsed,
            Err(err) => {
                warn!("failed to parse Bedrock endpoint URL '{url_str}': {err}; using fallback host for signing");
                match url::Url::parse("https://bedrock-runtime.us-east-1.amazonaws.com/") {
                    Ok(fallback) => fallback,
                    Err(fallback_err) => {
                        warn!("failed to parse Bedrock fallback URL: {fallback_err}");
                        return headers;
                    }
                }
            }
        ...
        let hmac_sha256 = |key: &[u8], data: &[u8], label: &str| -> Option<Vec<u8>> {
            let mut mac = match HmacSha256::new_from_slice(key) {
                Ok(mac) => mac,
                Err(err) => {
                    warn!("failed to initialize Bedrock HMAC for {label}: {err}");
                    return None;
                }
            };
            mac.update(data);
        ...
        let sign_key = {
            ...
            let Some(k_region) = hmac_sha256(&k_date, region.as_bytes(), "k_region") else {
                return headers;
            };
            let Some(k_service) = hmac_sha256(&k_region, service.as_bytes(), "k_service") else {
                return headers;
            };
            let Some(sign_key) = hmac_sha256(&k_service, b"aws4_request", "request key") else {
                return headers;
            };
            sign_key
        };
        ...
        let signature = {
            let Some(signature) =
                hmac_sha256(&sign_key, string_to_sign.as_bytes(), "request signature")
            else {
                return headers;
            };
        ...
        match resp {
            ...
            Ok(r) if r.status().as_u16() == 401 || r.status().as_u16() == 403 => {
                let body = r.text().await.unwrap_or_else(|err| {
                    format!("<failed to read foundation-models error response body: {err}>")
                });
                let body = body.trim();
            ...
            Ok(r) => {
                ...
                let body = body.trim();
                Ok(ProviderStatus::Degraded {
                    reason: if body.is_empty() {
                        format!("foundation-models returned {status}")
                    } else {
                        format!("foundation-models returned {status}: {body}")
                    },
                })
            }
...
mod tests {
    ...
    fn sigv4_provider() -> BedrockProvider {
        BedrockProvider {
            id: ProviderId::new(ProviderId::AMAZON_BEDROCK),
    ...
    fn sign_request_adds_sigv4_headers() {
        let provider = sigv4_provider();
        let date = chrono::Utc
            .with_ymd_and_hms(2026, 5, 24, 12, 0, 0)
            .single()
            .unwrap();

        let headers = provider.sign_request(
            "POST",
        ...
        assert_eq!(
            ...
            Some("20260524T120000Z")
        );
        assert!(headers.contains_key("x-amz-content-sha256"));
    }

    #[test]
    fn sign_request_invalid_url_falls_back_without_panicking() {
        let provider = sigv4_provider();
        let date = chrono::Utc
            .with_ymd_and_hms(2026, 5, 24, 12, 0, 0)
            .single()
        ...
        let headers = provider.sign_request("POST", "not a url", "{}", &date);
