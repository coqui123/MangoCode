...
use super::copilot_pirate_tools::{
...
pub fn is_copilot_pirate_backend(base_url: &str) -> bool {
    let lower = base_url.to_ascii_lowercase();
    // Only a local backend can be the Copilot-pirate proxy. Requiring this for
    // *every* path (including the env override) prevents hijacking a real cloud
    // OpenAI-compat provider (openrouter/groq/xai/...) that is selected later in
    // the same process while the override env var happens to be set.
    ...
    let port = port.trim();
    // Match the port as a delimited token so ":8765" does not also match
    // ":87654" (a different server that merely shares the prefix).
    lower.ends_with(&format!(":{port}")) || lower.contains(&format!(":{port}/"))
}
...
impl OpenAiCompatProvider {
    ...
    fn build_messages(&self, request: &ProviderRequest) -> Vec<Value> {
        ...
        if is_copilot_pirate_backend(&self.base_url) {
            // Reframe native tool_calls / role:tool into the plain-text form
            // Copilot understands (it renders HTML in role:tool results and
            // replies in XML when it sees native tool_calls). Must run before
            // the tool-prompt injection so the appendix lands on the system msg.
    ...
    fn gated_tools_for_request(&self, request: &ProviderRequest, tools: Vec<Value>) -> Vec<Value> {
        ...
        // Copilot-pirate: tools are injected into the system prompt, not the HTTP body.
        if is_copilot_pirate_backend(&self.base_url) {
            return Vec::new();
        }
    ...
    async fn create_message_non_streaming(
        ...
        let copilot_tool_bridge =
            is_copilot_pirate_backend(&self.base_url) && !request.tools.is_empty();
        // Once a local tool result is in history, a plain-text Copilot reply is
        // a valid final synthesis — gate the low-precision sandbox/listing
        // heuristics off so correct answers aren't retried into a protocol error.
        let copilot_post_tool_result =
            copilot_tool_bridge && messages_contain_tool_result(&messages);
        // Whether a real Write/Edit already ran — gates the fabricated-write
        // check so a genuine post-write confirmation isn't retried in a loop.
        let copilot_had_file_mutation =
            copilot_tool_bridge && messages_contain_write_tool_call(&messages);
        let mut working_messages = messages;

        for attempt in 0..COPILOT_PROVIDER_RETRY_ATTEMPTS {
            let mut body = json!({
                "model": request.model,
            ...
            if copilot_tool_bridge {
                response = apply_tool_parse_to_response(response, &request.tools);
                let tools_available = !request.tools.is_empty();
                if copilot_response_is_bad(
                    &response,
                    tools_available,
                    copilot_post_tool_result,
                    copilot_had_file_mutation,
                ) {
                    if attempt + 1 < COPILOT_PROVIDER_RETRY_ATTEMPTS {
                        let assistant_text = response_text_content(&response);
                        append_copilot_tool_retry_turn(&mut working_messages, &assistant_text);
                        continue;
                    }
                    let response_text = response_text_content(&response);
                    let snippet = truncate_wire_debug_text(&response_text, 240);
                    return Err(ProviderError::Other {
                        provider: self.id.clone(),
                        message: format!(
                            "{COPILOT_TOOL_PROTOCOL_ERROR_PREFIX} after \
                             {COPILOT_PROVIDER_RETRY_ATTEMPTS} attempts. Last reply: {snippet}"
                        ),
                        status: None,
                        body: None,
                    });
        ...
        Err(ProviderError::Other {
            provider: self.id.clone(),
            message: format!("{COPILOT_TOOL_PROTOCOL_ERROR_PREFIX}: retry loop exhausted"),
            status: None,
            body: None,
        })
    }
    ...
    async fn do_streaming(
        ...
        if !(200..300).contains(&(status as usize)) {
            let text = resp.text().await.map_err(|e| ProviderError::Other {
                provider: self.id.clone(),
                message: format!("HTTP error response body read failed: {}", e),
                status: Some(status),
                body: None,
            })?;
            return Err(self.map_http_error(status, &text));
...
impl LlmProvider for OpenAiCompatProvider {
    ...
    {
        // Copilot returns plain text; parse tool blocks from the full reply reliably.
        if is_copilot_pirate_backend(&self.base_url) {
            let response = self.create_message_non_streaming(&request).await?;
            let events = provider_response_to_stream_events(response);
        ...
        let s = stream! {
            ...
            loop {
                ...
                for line in lines {
                    ...
                    // The final usage chunk emitted under
                    // `stream_options.include_usage` carries token counts with
                    // either no `choices` key or an empty `choices: []` array.
                    // Handle both: surface usage and move on when there's no
                    // delta to process.
                    let choice = match chunk_json
                        .get("choices")
                        ...
                        .and_then(|c| c.first())
                    {
                        ...
                        None => {
                            if let Some(usage_val) =
                                chunk_json.get("usage").filter(|u| !u.is_null())
                            {
    ...
    async fn health_check(&self) -> Result<ProviderStatus, ProviderError> {
        ...
        match builder.send().await {
            ...
            Ok(r) => {
                let status = r.status();
                let body = r.text().await.unwrap_or_else(|err| {
                    format!("<failed to read models endpoint error response body: {err}>")
                });
                let body = body.trim();
                Ok(ProviderStatus::Unavailable {
                    reason: if body.is_empty() {
                        format!("models endpoint returned {status}")
                    } else {
                        format!("models endpoint returned {status}: {body}")
                    },
                })
            }
    ...
    fn capabilities(&self) -> ProviderCapabilities {
        let copilot_pirate = is_copilot_pirate_backend(&self.base_url);
        ProviderCapabilities {
            streaming: true,
            // Copilot-pirate uses prompt-in / parse-out tool protocol.
            tool_calling: true,
            thinking: !copilot_pirate
                && (self.quirks.reasoning_field.is_some() || self.id == ProviderId::OLLAMA),
...
mod tests {
    ...
    #[test]
    fn copilot_pirate_backend_detects_default_port() {
        assert!(is_copilot_pirate_backend("http://127.0.0.1:8765/v1"));
        assert!(is_copilot_pirate_backend("http://localhost:8765"));
        assert!(!is_copilot_pirate_backend("http://127.0.0.1:1234/v1"));
        // Port must be a delimited token, not a prefix of a longer port.
        assert!(!is_copilot_pirate_backend("http://127.0.0.1:87654/v1"));
        // Cloud OpenAI-compat providers must never be hijacked, even though they
        // share is_copilot_pirate_backend (no env var set in this test).
        assert!(!is_copilot_pirate_backend("https://openrouter.ai/api/v1"));
        assert!(!is_copilot_pirate_backend("https://api.groq.com/openai/v1"));
    }

    #[test]
    fn copilot_pirate_backend_enables_text_tool_protocol() {
