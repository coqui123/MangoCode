...
impl MockProvider {
    ...
    pub fn request_count(&self) -> usize {
        lock_or_recover(&self.request_log, "request_log").len()
    }

    fn next_turn(&self, request: ProviderRequest) -> (String, Vec<ToolCall>, String, UsageInfo) {
        lock_or_recover(&self.request_log, "request_log").push(request);

        let id_num = self.next_id.fetch_add(1, Ordering::Relaxed);
        let id = format!("mock-msg-{}", id_num);

        let tools = lock_or_recover(&self.tool_calls, "tool_calls")
            .pop_front()
            .unwrap_or_default();

        let text = if tools.is_empty() {
            lock_or_recover(&self.responses, "responses")
                .pop_front()
...
fn lock_or_recover<'a, T>(mutex: &'a Mutex<T>, name: &str) -> MutexGuard<'a, T> {
    match mutex.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
            tracing::warn!("MockProvider {name} mutex was poisoned; recovering state");
            poisoned.into_inner()
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn mock_provider_lock_recovery_handles_poisoned_mutex() {
        let mutex = Mutex::new(Vec::<ProviderRequest>::new());
        let result = std::panic::catch_unwind(|| {
            let _guard = mutex.lock().unwrap();
            panic!("poison mock mutex");
        });
