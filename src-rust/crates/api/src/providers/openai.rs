...
impl OpenAiProvider {
    ...
    pub fn to_openai_tools_pub(tools: &[mangocode_core::types::ToolDefinition]) -> Vec<Value> {
    ...
    pub fn parse_non_streaming_response_pub(
        json: &Value,
        provider_id: &mangocode_core::provider_id::ProviderId,
    ...
    fn to_openai_tools(tools: &[mangocode_core::types::ToolDefinition]) -> Vec<Value> {
        tools
            .iter()
            .map(|td| {
                json!({
    ...
    fn parse_non_streaming_response(
        ...
        if let Some(tool_calls) = message.get("tool_calls").and_then(|t| t.as_array()) {
            for tc in tool_calls {
                ...
                let input = parse_tool_call_arguments(&name, &args_str);
...
fn parse_tool_call_arguments(tool_name: &str, args: &str) -> Value {
    ...
    match serde_json::from_str(args) {
        Ok(value) => value,
        Err(err) => {
            // Weak local models (7B-14B served via LM Studio / llama.cpp) routinely
            // truncate tool-call argument JSON mid-generation — an unterminated
            // string, a missing closing brace, a trailing comma, or a stray ```
            // fence. Attempt a conservative structural repair before giving up, so
            // a single dropped brace doesn't waste the whole turn. Falls back to the
            // raw-preserving behavior when the args are too broken to recover.
            if let Some(repaired) = repair_tool_call_arguments(args) {
                tracing::warn!(
                    tool = %tool_name,
                    "repaired truncated/malformed tool call arguments JSON"
                );
                return repaired;
            }
            tracing::warn!(
...
/// Best-effort structural repair of a malformed/truncated tool-call argument
/// string. Returns `Some(object)` only when the repaired text parses to a JSON
/// object; otherwise `None` so the caller can preserve the raw text.
///
/// Repairs handled (the common weak-model failures):
/// - leading prose / ```json fences before the object (skips to the first `{`/`[`)
/// - trailing prose / ``` fences after the object (stops at the matching close)
/// - an unterminated string (appends the closing quote)
/// - unbalanced `{`/`[` (appends the matching closers in order)
/// - a trailing comma before the (re)appended closer
pub(crate) fn repair_tool_call_arguments(args: &str) -> Option<Value> {
    // Skip any leading prose / code fence to the first JSON opener.
    let start = args.find(['{', '['])?;
    let body = &args[start..];
    ...
    for ch in body.chars() {
        ...
        match ch {
            ...
            '}' | ']' => {
                ...
                // Root value closed — ignore any trailing prose / ``` fence.
                if stack.is_empty() {
                    break;
                }
            }
    ...
    // Close an unterminated string left open by truncation.
    if in_string {
        out.push('"');
    }
    // Drop trailing commas/whitespace before appending the missing closers.
    while {
        let t = out.trim_end();
        t.ends_with(',')
    } {
        ...
        out.pop(); // remove the comma
    }
    // Append any closers the model dropped, innermost first.
...
mod tests {
    ...
    #[test]
    fn parse_non_streaming_response_preserves_malformed_tool_arguments() {
        let response = json!({
            "id": "chatcmpl-test",
            "model": "gpt-test",
            "choices": [{
        ...
        let parsed =
            OpenAiProvider::parse_non_streaming_response(&response, &ProviderId::new("openai"))
                .unwrap();
        ...
        match &parsed.content[0] {
            ContentBlock::ToolUse { id, name, input } => {
                ...
                // Unquoted-key garbage is unrecoverable — preserve raw, do not invent.
                assert_eq!(input, &json!({ "raw": "{bad json" }));
            }
    ...
    #[test]
    fn parse_tool_call_arguments_valid_passes_through() {
        let v = parse_tool_call_arguments("t", r#"{"path":"a.rs","n":3}"#);
        assert_eq!(v, json!({"path":"a.rs","n":3}));
    }

    #[test]
    fn repair_recovers_unterminated_string() {
        // Truncated mid-string (model stopped emitting): close string + brace.
        let v = parse_tool_call_arguments("Write", r#"{"path":"src/main.rs","content":"fn main() {"#);
        assert_eq!(v["path"], json!("src/main.rs"));
        assert_eq!(v["content"], json!("fn main() {"));
    }

    #[test]
    fn repair_recovers_missing_closing_brace_and_trailing_comma() {
        let v = parse_tool_call_arguments("t", r#"{"a":1,"b":[2,3],"#);
        assert_eq!(v, json!({"a":1,"b":[2,3]}));
    }

    #[test]
    fn repair_strips_code_fence_and_trailing_prose() {
        let v = parse_tool_call_arguments("t", "```json\n{\"q\":\"hello\"}\n```\nDone.");
        assert_eq!(v, json!({"q":"hello"}));
    }

    #[test]
    fn repair_gives_up_on_unrecoverable_returns_raw() {
        // Unquoted identifiers cannot be repaired structurally -> raw preserved.
        let v = parse_tool_call_arguments("t", "{bad json");
        assert_eq!(v, json!({"raw":"{bad json"}));
    }
}
