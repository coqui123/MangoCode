...
///
/// Strips fields not part of the Codex backend contract to keep the request
/// payload compatible with Codex/ChatGPT backends.
...
fn collect_output_item_tool_use(item: &Value) -> Option<Value> {
    ...
    let input = match args_value {
        Some(Value::String(s)) => parse_tool_arguments_value(s),
        Some(v) => v.clone(),
        None => json!({}),
    };
...
fn parse_tool_arguments_value(raw: &str) -> Value {
