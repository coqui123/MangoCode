...
impl ProviderRegistry {
    /// Create an empty registry with LM Studio as the default provider ID.
    pub fn new() -> Self {
        Self {
            providers: HashMap::new(),
            default_provider_id: ProviderId::new("lmstudio"),
        }
    }
    ...
    /// Takes the same [`ClientConfig`] that [`AnthropicClient`] takes.
    ///
    ...
    /// in the environment.  LM Studio is the default provider.
    ///
    /// This is the recommended constructor for production use.
    pub fn from_environment(anthropic_config: ClientConfig) -> Self {
        let mut registry = Self::new();
        let provider = Arc::new(AnthropicProvider::from_config(anthropic_config));
        registry.register(provider);
        registry
