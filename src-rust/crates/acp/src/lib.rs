...
impl AcpServerState {
    fn sessions(&self) -> MutexGuard<'_, HashMap<String, Arc<tokio::sync::Mutex<AcpSession>>>> {
        lock_or_recover(&self.sessions, "sessions")
    }

    fn running(&self) -> MutexGuard<'_, HashMap<String, CancellationToken>> {
        lock_or_recover(&self.running, "running")
    }

    fn pending_permissions(&self) -> MutexGuard<'_, HashMap<String, PendingPermission>> {
        lock_or_recover(&self.pending_permissions, "pending permissions")
    }
}
...
fn lock_or_recover<'a, T>(mutex: &'a Mutex<T>, name: &str) -> MutexGuard<'a, T> {
    match mutex.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
            warn!("ACP {name} mutex was poisoned; recovering state");
            poisoned.into_inner()
        }
    }
}
...
pub async fn run_acp_server() -> anyhow::Result<()> {
    ...
    if let Err(err) = writer.await {
        warn!(error = %err, "ACP writer task failed");
    }
    Ok(())
}
...
async fn create_session(
    ...
    let permission_mode = params_permission_mode(&params)?;
...
async fn prompt_session(
    ...
    let (outcome, save_error) = {
        ...
        let outcome = mangocode_query::run_query_loop(
            ...
            cancel.clone(),
            None,
            None, // ACP sessions do not expose a HITL permission prompt UI
        )
        .await;
        ...
        let save_error = match mangocode_core::history::save_session(&session_guard.session).await {
            Ok(()) => None,
            Err(err) => {
                warn!(
                    error = %err,
                    session_id = %session_id,
                    "ACP failed to save session after prompt"
                );
                Some(err.to_string())
            }
        };
    ...
    let mut outcome_json = match outcome {
    ...
    if let Some(save_error) = save_error.clone() {
        if let Some(outcome) = outcome_json.as_object_mut() {
            outcome.insert("sessionSaveError".to_string(), Value::String(save_error));
        }
    }
    send_session_update(
        &out_tx,
...
fn cancel_session(params: Option<Value>, state: Arc<AcpServerState>) -> anyhow::Result<Value> {
    ...
    if let Some(token) = state.running().remove(&session_id) {
        token.cancel();
    }
...
async fn set_session_mode(
    ...
    let mode = params_permission_mode(&params)?.unwrap_or(PermissionMode::Default);
    let session = {
        let sessions = state.sessions();
        sessions
            .get(&session_id)
...
fn request_acp_permission(
    ...
    await_permission_decision(rx)
}

/// Wait for the client's permission decision without starving the async runtime.
///
/// `request_permission` is a synchronous trait method invoked from async tool
/// execution, so a bare blocking `recv_timeout` (up to an hour) would pin a Tokio
/// worker thread for the whole wait. On a multi-threaded runtime, `block_in_place`
/// runs the blocking recv on the current thread while letting Tokio move other
/// tasks to sibling workers. On a current-thread runtime (where `block_in_place`
/// would panic) or outside any runtime, fall back to a plain blocking recv.
fn await_permission_decision(rx: std_mpsc::Receiver<PermissionDecision>) -> PermissionDecision {
    let recv = move || {
...
fn handle_client_response(resp: JsonRpcResponseMsg, state: &Arc<AcpServerState>) {
    ...
    if let Some(pending) = state.pending_permissions().remove(&id) {
...
fn cancel_pending_permissions(state: &Arc<AcpServerState>, session_id: &str) {
    let mut pending = state.pending_permissions();
...
fn params_permission_mode(params: &Value) -> anyhow::Result<Option<PermissionMode>> {
...
fn parse_permission_mode(mode: &str) -> Option<PermissionMode> {
    ...
    match normalized.as_str() {
        ...
        "accept-edits" | "code" => Some(PermissionMode::AcceptEdits),
        "bypass" | "bypass-permissions" => Some(PermissionMode::BypassPermissions),
...
mod tests {
    ...
    #[test]
    fn lock_recovery_handles_poisoned_mutex() {
    ...
    #[test]
    fn params_permission_mode_normalizes_and_rejects_unknown_values() {
        assert_eq!(
            params_permission_mode(&json!({ "modeId": " ACCEPT_EDITS " })).unwrap(),
            Some(PermissionMode::AcceptEdits)
        );
        assert_eq!(
            params_permission_mode(&json!({ "mode": "bypass_permissions" })).unwrap(),
            Some(PermissionMode::BypassPermissions)
        );
        assert_eq!(
            params_permission_mode(&json!({ "mode_id": "   " })).unwrap(),
            None
        );

        let err = params_permission_mode(&json!({ "modeId": "definitely-no" })).unwrap_err();
        assert!(err
            .to_string()
            .contains("Unknown session mode: definitely-no"));
    }
    ...
    async fn set_model_and_mode_update_session_runtime_inputs() {
        ...
        create_session(Some(json!({ "sessionId": "default-model" })), state.clone())
            ...
            .unwrap();
        {
            let session = state.sessions().get("default-model").cloned().unwrap();
        ...
        let session = state.sessions().get("s").cloned().unwrap();
    ...
    async fn create_session_rejects_unknown_mode() {
        ...
        assert!(err.to_string().contains("Unknown session mode: not-a-mode"));
        assert!(!state.sessions().contains_key("bad-mode"));
    }

    #[tokio::test]
    async fn tool_list_rejects_unknown_mode_without_session() {
        let state = Arc::new(AcpServerState::default());
        let err = tool_list_json(Some(json!({ "modeId": "not-a-mode" })), state)
            .await
            .unwrap_err();
