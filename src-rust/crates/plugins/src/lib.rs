// mangocode-plugins: Plugin runtime for the MangoCode CLI.
//
// This crate handles plugin discovery, manifest parsing, hook registration,
// and the /plugin + /reload-plugins command definitions.
//
// Dependency order: mangocode-plugins → mangocode-core only.
// mangocode-commands → mangocode-plugins (not the other way around).
...
// ---------------------------------------------------------------------------
// Top-level async API (called from mangocode-commands / mangocode-cli)
// ---------------------------------------------------------------------------

/// Discover and load all plugins from the standard locations.
///
/// Search order:
/// 1. `~/.mangocode/plugins/`  (user-global)
/// 2. `<project_dir>/.mangocode/plugins/`  (project-local)
...
pub fn parse_plugin_args(args: &str) -> PluginSubCommand {
    ...
    let words = match mangocode_core::split_command_words(args) {
        Ok(words) => words,
        Err(err) => return PluginSubCommand::Error(err.to_string()),
    };
    let arg = |words: &[String]| -> String { words.get(1).cloned().unwrap_or_default() };
    let reject_extra = |subcommand: &str, words: &[String]| -> Option<PluginSubCommand> {
        (words.len() > 2).then(|| {
            PluginSubCommand::Error(format!(
                "Unexpected argument '{}' for /plugin {}.",
                words[2], subcommand
            ))
        })
    };

    match words.first().map(|s| s.to_lowercase()).as_deref() {
        Some("list") | Some("ls") => PluginSubCommand::List,
        Some("enable") => {
            reject_extra("enable", &words).unwrap_or_else(|| PluginSubCommand::Enable(arg(&words)))
        }
        Some("disable") => reject_extra("disable", &words)
            .unwrap_or_else(|| PluginSubCommand::Disable(arg(&words))),
        Some("info") | Some("show") => {
            reject_extra("info", &words).unwrap_or_else(|| PluginSubCommand::Info(arg(&words)))
        }
        Some("install") | Some("i") => reject_extra("install", &words)
            .unwrap_or_else(|| PluginSubCommand::Install(arg(&words))),
...
///
/// Copies the plugin directory into `~/.mangocode/plugins/` and returns the
...
mod tests {
    ...
    #[test]
    fn parse_plugin_args_install_preserves_quoted_path() {
        assert_eq!(
            parse_plugin_args(r#"install "C:\Users\me\My Plugin""#),
            PluginSubCommand::Install(r#"C:\Users\me\My Plugin"#.to_string())
        );
    }

    #[test]
    fn parse_plugin_args_rejects_trailing_arguments() {
        match parse_plugin_args("enable my-plugin extra") {
            PluginSubCommand::Error(message) => {
