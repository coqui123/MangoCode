...
pub fn run_hook_sync(hook: &RegisteredHook, event_json: &str) -> HookOutcome {
    ...
    // Write event JSON to stdin. A hook may not read stdin, so preserve the
    // hook's exit-status behavior, but surface delivery failures for operators.
    if let Some(mut stdin) = child.stdin.take() {
        if let Err(e) = stdin.write_all(event_json.as_bytes()) {
            tracing::warn!(
                ...
                error = %e,
                "Failed to write plugin hook event JSON to stdin"
            );
        }
    }
...
/// (including the empty sequence) anywhere in the pattern. Supports exact
/// matches, leading/trailing stars (`File*`, `*Tool`), mid-string stars
/// (`mcp__*__read`), and multiple stars (`a*b*c`, `*foo*`). No other wildcards.
fn glob_match(pattern: &str, text: &str) -> bool {
    if !pattern.contains('*') {
        return pattern == text;
    }

    // The literal segments between stars must appear in order. A segment before
    // the first star must anchor the start; a segment after the last star must
    // anchor the end; interior segments may appear anywhere in between.
...
mod tests {
    ...
    #[test]
    fn glob_match_supports_stars_anywhere() {
        // Exact and the universal star.
        assert!(glob_match("Bash", "Bash"));
        assert!(!glob_match("Bash", "Edit"));
        assert!(glob_match("*", "anything"));
        // Leading / trailing stars (prior behavior preserved).
        assert!(glob_match("File*", "FileEdit"));
        assert!(!glob_match("File*", "Edit"));
        assert!(glob_match("*Tool", "SearchTool"));
        assert!(!glob_match("*Tool", "ToolX"));
        // Mid-string star.
        assert!(glob_match("mcp__*__read", "mcp__github__read"));
        assert!(!glob_match("mcp__*__read", "mcp__read"));
        assert!(!glob_match("mcp__*__read", "mcp__github__write"));
        // Multiple stars and contains.
