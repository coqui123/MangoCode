...
// ---------------------------------------------------------------------------
// MCP server config (simplified — full config lives in mangocode-core)
// ---------------------------------------------------------------------------
...
///
/// All kinds are dispatched at runtime except `Setup` and `TeammateIdle`,
/// which are accepted in manifests for forward compatibility but have no
/// runtime trigger yet (no plugin-install lifecycle / idle detector exists).
...
/// before deserializing so that both camelCase and snake_case variants
...
fn normalize_manifest_json(mut v: serde_json::Value) -> serde_json::Value {
    ...
    // `commands` can be a single string or array — normalise to array.
