// mangocode-bridge: Remote control bridge implementation.
//
// The bridge connects the local MangoCode CLI to the claude.ai web UI,
...
fn error_body_suffix(body: &str) -> String {
    let body = body.trim();
    if body.is_empty() {
        String::new()
    } else {
...
impl BridgeConfig {
    ...
    /// - `MANGOCODE_BRIDGE_URL` — overrides `server_url` and sets `enabled = true`
    /// - `MANGOCODE_BRIDGE_TOKEN` / `CLAUDE_BRIDGE_OAUTH_TOKEN` — sets `session_token`
    /// - `CLAUDE_BRIDGE_BASE_URL` — alternative URL override (ant-only dev override)
    pub fn from_env() -> Self {
        let mut config = Self::default();

        // URL override (sets enabled implicitly)
        if let Ok(url) = std::env::var("MANGOCODE_BRIDGE_URL")
        ...
        // Token override
        if let Ok(token) = std::env::var("MANGOCODE_BRIDGE_TOKEN")
    ...
    pub fn validate_id<'a>(id: &'a str, label: &str) -> anyhow::Result<&'a str> {
        ...
        let re = RE
            .get_or_init(|| regex::Regex::new(r"^[a-zA-Z0-9_-]+$"))
            .as_ref()
            .map_err(|err| anyhow::anyhow!("Bridge ID validation is unavailable: {err}"))?;
...
impl BridgeSession {
    ...
    /// Create a new bridge session reusing an existing HTTP client.
    pub fn with_http(config: BridgeConfig, http: reqwest::Client) -> Self {
        Self {
            config,
...
async fn start_bridge_with_client(
    ...
    let mut session = BridgeSession::with_http(config, http);
    session
        .register()
        .await
...
/// 1. `MANGOCODE_BRIDGE_TOKEN` environment variable
...
/// match mangocode_bridge::start_bridge_session(None).await {
...
pub async fn start_bridge_session(
    ...
    let token = token_override
        .or_else(|| std::env::var("MANGOCODE_BRIDGE_TOKEN").ok())
        .or_else(|| std::env::var("CLAUDE_BRIDGE_OAUTH_TOKEN").ok())
        .filter(|t| !t.is_empty())
        .ok_or_else(|| {
            anyhow::anyhow!(
                "Remote Control requires a session token.\n\
                 Set MANGOCODE_BRIDGE_TOKEN=<your-token> to enable.\n\
    ...
    let server_url = std::env::var("MANGOCODE_BRIDGE_URL")
        .or_else(|_| std::env::var("CLAUDE_BRIDGE_BASE_URL"))
    ...
    match status {
        ...
        _ => {
            let body_text = response_error_body(resp, "bridge registration").await;
            anyhow::bail!(
...
pub async fn poll_bridge_messages(
    ...
    loop {
        ...
        match status {
            ...
            401 | 403 => {
                let body = response_error_body(resp, "bridge poll").await;
                anyhow::bail!(
                    "poll_bridge_messages: auth error (HTTP {}){}",
                    status,
                    error_body_suffix(&body)
                );
            }
            _ => {
                let body = response_error_body(resp, "bridge poll").await;
...
pub async fn post_bridge_response(
    ...
    if resp.status().is_success() {
        ...
        let body = response_error_body(resp, "bridge response post").await;
        anyhow::bail!(
...
pub async fn post_bridge_event(info: &BridgeSessionInfo, payload: String) -> anyhow::Result<()> {
    ...
    if resp.status().is_success() {
        ...
        let body = response_error_body(resp, "bridge event post").await;
        anyhow::bail!(
...
mod tests {
    ...
    fn test_jwt_decode_invalid() {
        ...
        // Validation behavior is implementation-defined; this call should never panic.
