...
fn transcribe_audio_local(
    ...
    if !output.status.success() {
        cleanup_voice_temp_file(&output_txt);
    ...
    let text = match std::fs::read_to_string(&output_txt) {
        ...
        Err(err) => {
            if err.kind() != std::io::ErrorKind::NotFound {
                tracing::warn!(
                    path = %output_txt.display(),
                    error = %err,
                    "failed to read local voice transcription output file; falling back to stdout"
                );
            }
...
#[cfg(feature = "voice")]
fn cleanup_voice_temp_file(path: &Path) {
    if let Err(err) = std::fs::remove_file(path) {
        if err.kind() != std::io::ErrorKind::NotFound {
            tracing::warn!(
                path = %path.display(),
                error = %err,
                "failed to remove local voice transcription temp file"
            );
        }
    }
}
...
mod tests {
    ...
    #[cfg(feature = "voice")]
    #[test]
    fn test_cleanup_voice_temp_file_removes_files_and_ignores_missing_files() {
        let path = std::env::temp_dir().join(format!(
            "mangocode-voice-cleanup-test-{}",
            uuid::Uuid::new_v4()
        ));

        std::fs::write(&path, b"temporary voice output").unwrap();
