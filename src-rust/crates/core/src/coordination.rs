    ...
    tokio::spawn(async move {
        loop {
            if task_cancel.is_cancelled() {
                ...
                tokio::task::spawn_blocking(move || unregister_heartbeat_state(&state));
                break;
            }
            // The per-tick register/heartbeat opens a fresh SQLite connection and
            // runs schema/migration/prune — blocking work. Offload it to the
            // blocking pool so it never stalls the async runtime. Fire-and-forget:
            // a delayed or skipped tick is harmless. (The register_heartbeat_state
            // function itself stays sync so update_with_parent can call it from a
            // non-runtime context without panicking on spawn_blocking.)
            let state = task_state.clone();
            tokio::task::spawn_blocking(move || register_heartbeat_state(&state));
            tokio::select! {
    ...
    pub fn update_with_parent(
        ...
        if previous_session != session_id {
            if let Ok(store) = CoordinationStore::open_default() {
                if let Err(err) = store.unregister_session(&previous_session) {
                    tracing::warn!(
                        error = %err,
                        session_id = %previous_session,
                        "failed to unregister previous coordination heartbeat session"
                    );
                }
            }
        } else if canonical_project_root(&previous_cwd) != canonical_project_root(&current_cwd) {
            if let Ok(store) = CoordinationStore::open_default() {
                if let Err(err) = store.release_claims(&session_id, None) {
                    tracing::warn!(
                        error = %err,
                        session_id = %session_id,
                        "failed to release coordination claims after project change"
                    );
                }
            }
        }
    ...
    if let Ok(store) = CoordinationStore::open_default() {
        if let Err(err) = store.register_session_with_parent(
            ...
            tracing::warn!(
                ...
                "failed to register coordination heartbeat session"
            );
        }
        if let Err(err) = store.heartbeat(&heartbeat.session_id) {
            tracing::warn!(
                error = %err,
                session_id = %heartbeat.session_id,
                "failed to update coordination heartbeat"
            );
        }
    }
}
...
fn unregister_heartbeat_state(state: &Arc<Mutex<PresenceHeartbeatState>>) {
    ...
    if let Ok(store) = CoordinationStore::open_default() {
        if let Err(err) = store.unregister_session(&heartbeat.session_id) {
            tracing::warn!(
                error = %err,
                session_id = %heartbeat.session_id,
                "failed to unregister coordination heartbeat session"
            );
        }
    }
}
...
impl CoordinationStore {
    ...
    pub fn open(db_path: &Path) -> anyhow::Result<Self> {
        ...
        add_column_if_missing(&conn, "ALTER TABLE coord_messages ADD COLUMN read_at TEXT")?;
        conn.execute(
        ...
        let store = Self { conn };
        if let Err(err) = store.prune_stale_coordination() {
            tracing::warn!(
                error = %err,
                "failed to prune stale coordination records during store open"
            );
        }
        Ok(store)
    }
