...
///
/// The inner `Connection` is wrapped in a `Mutex` so the store is `Send + Sync`
...
impl SqliteSessionStore {
        ...
        let rows = stmt.query_map(
            ...
            |row| {
                ...
                let timestamp = chrono::DateTime::parse_from_rfc3339(&created_at)
                    ...
                    .map_err(|err| from_sql_conversion_error(8, err))?;
                let payload = serde_json::from_str(&payload_json)
                    .map_err(|err| from_sql_conversion_error(7, err))?;
                Ok((
                    row.get::<_, i64>(0)?,
                    HarnessEvent {
                        event_id: row.get(1)?,
    ...
    /// Return the most recent `limit` events for a session, in ascending
    /// sequence order. Unlike `list_harness_events`, which truncates to the
    /// OLDEST `limit` rows, this keeps the newest rows — callers scanning for
    /// the latest run must not lose recent events in long sessions.
    pub fn list_harness_events_latest(
        &self,
        session_id: &str,
        limit: usize,
    ) -> anyhow::Result<Vec<(i64, HarnessEvent)>> {
...
fn from_sql_conversion_error<E>(column: usize, err: E) -> rusqlite::Error
where
...
fn invalid_text_value(column: usize, message: impl Into<String>) -> rusqlite::Error {
    from_sql_conversion_error(
        column,
        std::io::Error::new(std::io::ErrorKind::InvalidData, message.into()),
    )
}

fn parse_checkpoint_kind(value: &str) -> rusqlite::Result<CheckpointKind> {
    match value {
        "before" => Ok(CheckpointKind::Before),
        "after" => Ok(CheckpointKind::After),
        other => Err(invalid_text_value(
            3,
            format!("invalid checkpoint kind '{other}'"),
        )),
    }
}

fn parse_checkpoint_backend(value: &str) -> rusqlite::Result<CheckpointBackend> {
    match value {
        "git_ref" => Ok(CheckpointBackend::GitRef),
        "file_snapshot" => Ok(CheckpointBackend::FileSnapshot),
        "metadata_only" => Ok(CheckpointBackend::MetadataOnly),
        other => Err(invalid_text_value(
            4,
            format!("invalid checkpoint backend '{other}'"),
        )),
    }
}
...
fn checkpoint_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<HarnessCheckpoint> {
    ...
    let snapshot = snapshot_json
        .map(|s| serde_json::from_str(&s).map_err(|err| from_sql_conversion_error(7, err)))
        .transpose()?;
...
fn goal_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<ThreadGoal> {
    ...
    let status = ThreadGoalStatus::try_from(status.as_str())
        .map_err(|err| invalid_text_value(3, err.to_string()))?;
    Ok(ThreadGoal {
...
mod tests {
    ...
    #[test]
    fn list_harness_events_rejects_malformed_payload_json() {
        let dir = tempfile::tempdir().unwrap();
        let db = dir.path().join("sessions.db");
        let store = SqliteSessionStore::open(&db).unwrap();
        let event = HarnessEvent {
        ...
        store.append_harness_event(&event).unwrap();
        store
            .conn
            .lock()
            .unwrap()
            .execute(
                "UPDATE harness_events SET payload_json = ?1 WHERE event_id = ?2",
                rusqlite::params!["{not-json", "event-1"],
            )
            .unwrap();

        assert!(store.list_harness_events("session-1", None, 100).is_err());
    }

    #[test]
    fn session_lists_reject_row_conversion_errors() {
        let dir = tempfile::tempdir().unwrap();
        let db = dir.path().join("sessions.db");
        let store = SqliteSessionStore::open(&db).unwrap();
        store
            .save_session("session-1", Some("Broken session"), "model-a")
            .unwrap();
        store
            .conn
            .lock()
            ...
            .execute(
                ...
                rusqlite::params!["not-a-count", "session-1"],
            )
            .unwrap();

        assert!(store.list_sessions().is_err());
        assert!(store.search_sessions("Broken").is_err());
    }

    #[test]
    fn list_harness_checkpoints_rejects_malformed_snapshot_json() {
        let dir = tempfile::tempdir().unwrap();
        let db = dir.path().join("sessions.db");
        let store = SqliteSessionStore::open(&db).unwrap();
        let checkpoint = HarnessCheckpoint {
            ...
            backend: CheckpointBackend::FileSnapshot,
            repo_root: Some(dir.path().to_path_buf()),
            git_ref: None,
            snapshot: Some(json!({ "files": [] })),
            created_at: Utc::now(),
        };
        store.save_harness_checkpoint(&checkpoint).unwrap();
        store
            .conn
            .lock()
            .unwrap()
            .execute(
                "UPDATE harness_checkpoints SET snapshot_json = ?1 WHERE checkpoint_id = ?2",
                rusqlite::params!["{not-json", "checkpoint-1"],
            )
            .unwrap();

        assert!(store.list_harness_checkpoints("session-1").is_err());
        assert!(store.get_harness_checkpoint("checkpoint-1").is_err());
    }
