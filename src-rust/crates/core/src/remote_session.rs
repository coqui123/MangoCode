...
impl RemoteSessionManager {
    ...
    pub async fn list_sessions(&self) -> Result<Vec<CloudSession>, String> {
        ...
        if !resp.status().is_success() {
            ...
            let body = response_error_body(resp, "remote session list").await;
            return Err(format_http_error("API error", status, body));
        }
    ...
    pub async fn push_transcript_entry(
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let body = response_error_body(resp, "remote session push").await;
            return Err(format_http_error("API error", status, body));
        }
        Ok(())
    }
                ...
                if let Ok(content) = tokio::fs::read_to_string(&transcript_path).await {
                    ...
                    if lines.len() > last_sync_len {
                        for line in &lines[last_sync_len..] {
                            if !line.is_empty() {
                                if let Err(err) =
                                    self.push_transcript_entry(&session_id, line).await
                                {
                                    warn!(
                                        error = %err,
                                        session_id = %session_id,
                                        transcript_path = %transcript_path.display(),
                                        "remote transcript sync push failed"
                                    );
                                }
                            }
                        }
