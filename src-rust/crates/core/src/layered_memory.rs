...
use crate::command_line::split_command_words;
...
impl LayeredMemoryStore {
    ...
    pub fn init(&self) -> anyhow::Result<()> {
        ...
        self.add_column_if_missing("ALTER TABLE memories ADD COLUMN vector TEXT")?;
        self.add_column_if_missing("ALTER TABLE memories ADD COLUMN embedding TEXT")?;
        self.add_column_if_missing("ALTER TABLE memories ADD COLUMN embedding_model TEXT")?;
        Ok(())
    }

    fn add_column_if_missing(&self, sql: &str) -> anyhow::Result<()> {
        match self.conn.execute(sql, []) {
            Ok(_) => Ok(()),
            Err(err) if is_sqlite_duplicate_column_error(&err) => Ok(()),
...
fn is_sqlite_duplicate_column_error(err: &rusqlite::Error) -> bool {
...
pub fn capture_explicit_memories(
    ...
    for (class, content) in extract_explicit_memory_candidates(text) {
        match store.insert(class.clone(), &content, source, project) {
            Ok(id) => ids.push(id),
            Err(err) => {
                let error_message = err.to_string();
                if error_message.contains("privacy filter") {
                    tracing::debug!(
                        class = class.as_str(),
                        source = ?source,
                        project = ?project,
                        "explicit layered memory candidate skipped by privacy filter"
                    );
                } else {
                    tracing::warn!(
                        class = class.as_str(),
                        ...
                        project = ?project,
                        error = %err,
                        "failed to capture explicit layered memory"
                    );
                }
            }
        }
...
/// Embed arbitrary text with the configured local embedding backend, returning a
/// dense vector. `is_query` selects the query- vs passage-side instruction prefix
/// used by instruction-tuned embedders (e5/nomic-style). Returns `None` when no
/// embedding backend is available so callers can fall back to non-semantic
/// ranking. Reuses the same embedding path as layered memory — this is the shared
/// entry point for semantic code retrieval.
pub fn embed_text(text: &str, is_query: bool) -> Option<Vec<f32>> {
    ...
    local_embedding_for_text(text, purpose).ok()
}

/// Cosine similarity between two dense embedding vectors. Returns 0.0 when the
/// shapes differ or either vector is empty/zero-norm.
pub fn embedding_cosine_similarity(a: &[f32], b: &[f32]) -> f32 {
    cosine_similarity_dense(a, b)
}
...
fn command_embedding_for_text(text: &str) -> anyhow::Result<Vec<f32>> {
    ...
    let Some(mut stdin) = child.stdin.take() else {
        terminate_embedding_child(&mut child, "stdin unavailable");
        anyhow::bail!("embedding command stdin is unavailable");
    };
    if let Err(err) = stdin.write_all(text.as_bytes()) {
        drop(stdin);
        terminate_embedding_child(&mut child, "stdin write failed");
        return Err(err.into());
    }
    drop(stdin);
...
fn terminate_embedding_child(child: &mut std::process::Child, reason: &str) {
    if let Err(err) = child.kill() {
        tracing::warn!(
            reason,
            error = %err,
            "failed to kill embedding command after setup failure"
        );
    }
    if let Err(err) = child.wait() {
        tracing::warn!(
            reason,
            error = %err,
            "failed to wait for embedding command after setup failure"
        );
    }
}
