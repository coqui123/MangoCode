...
pub fn load_output_styles_dir(styles_dir: &Path) -> Vec<OutputStyleDef> {
    ...
    for entry in entries {
        let entry = match entry {
            ...
            Err(err) => {
                tracing::warn!(
                    path = %styles_dir.display(),
                    error = %err,
                    "failed to read output style directory entry"
                );
                continue;
            }
        };
        ...
        if ext.eq_ignore_ascii_case("md") || ext.eq_ignore_ascii_case("json") {
            match load_output_style_file_result(&path) {
                Ok(Some(style)) => styles.push(style),
                Ok(None) => {}
                Err(err) => {
                    tracing::warn!(
                        path = %path.display(),
                        error = %err,
                        "failed to load output style file"
                    );
                }
            }
        }
...
fn load_output_style_file_result(path: &Path) -> Result<Option<OutputStyleDef>, String> {
    ...
    if ext.eq_ignore_ascii_case("json") {
        ...
        let mut def: OutputStyleDef = serde_json::from_str(&content)
            .map_err(|err| format!("failed to parse output style JSON: {err}"))?;
        ...
        if def.name.is_empty() {
            return Err("empty output style name".to_string());
        }
        return Ok(Some(def));
    }
