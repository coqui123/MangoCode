...
fn collect_md_files(base: &Path, current_dir: &Path, out: &mut Vec<MemoryFileMeta>) {
    ...
    for entry in entries {
        let entry = match entry {
            ...
            Err(err) => {
                tracing::warn!(
                    path = %current_dir.display(),
                    error = %err,
                    "failed to read memory directory entry"
                );
                continue;
            }
        };
                ...
                .is_some_and(|ext| ext.eq_ignore_ascii_case("md"))
        {
            ...
            let modified_secs = match entry.metadata().and_then(|m| m.modified()) {
                ...
                Err(err) => {
                    tracing::warn!(
                        path = %path.display(),
                        error = %err,
                        "failed to inspect memory file timestamp"
                    );
                    0
                }
            };
            ...
            let (name, description, memory_type) = match std::fs::read_to_string(&path) {
                Ok(content) => parse_frontmatter_quick(&content),
                Err(err) => {
                    tracing::warn!(
                        path = %path.display(),
                        error = %err,
                        "failed to read memory file frontmatter"
                    );
                    (None, None, None)
                }
            };
...
/// 3. `~/.mangocode/projects/<sanitized-root>/memory/` (default).
pub fn auto_memory_path(project_root: &Path) -> PathBuf {
    ...
    let memory_base = std::env::var("MANGOCODE_REMOTE_MEMORY_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            dirs::home_dir()
...
pub fn is_auto_memory_enabled(settings_enabled: Option<bool>) -> bool {
    if let Ok(val) = std::env::var("MANGOCODE_DISABLE_AUTO_MEMORY") {
    ...
    if std::env::var("MANGOCODE_SIMPLE").is_ok() {
        return false;
    }

    if std::env::var("MANGOCODE_REMOTE").is_ok()
        && std::env::var("MANGOCODE_REMOTE_MEMORY_DIR").is_err()
    {
        return false;
    }
...
///
/// The full Sonnet side-query lives in `mangocode-query` (`find_relevant_memories`);
