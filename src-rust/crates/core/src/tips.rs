...
//
// History is persisted to `~/.mangocode/tip_history.json`.
...
static ALL_TIPS: Lazy<Vec<Tip>> = Lazy::new(|| {
    vec![
        Tip {
            id: "new-user-warmup",
            content: "Start with small features or bug fixes, tell MangoCode to propose a plan, and verify its suggested edits",
            cooldown_sessions: 3,
        },
        Tip {
        ...
        Tip {
            id: "git-worktrees",
            content: "Use git worktrees to run multiple MangoCode sessions in parallel.",
            cooldown_sessions: 10,
        },
        Tip {
            id: "color-when-multi-clauding",
            content: "Running multiple MangoCode sessions? Use /color and /rename to tell them apart at a glance.",
            cooldown_sessions: 10,
        },
        Tip {
            id: "shift-enter",
        ...
        Tip {
            id: "memory-command",
            content: "Use /memory to view and manage MangoCode memory",
        ...
        Tip {
            ...
            content: "Send messages to MangoCode while it works to steer MangoCode in real-time",
            cooldown_sessions: 20,
        },
        Tip {
            id: "todo-list",
            content: "Ask MangoCode to create a todo list when working on complex tasks to track progress and remain on track",
            cooldown_sessions: 20,
        },
        Tip {
            id: "permissions",
        ...
        Tip {
            id: "custom-commands",
            content: "Create skills by adding .md files to .mangocode/skills/ in your project or ~/.mangocode/skills/ for skills that work in any project",
            cooldown_sessions: 15,
        },
        Tip {
            id: "shift-tab",
...
impl TipHistory {
    /// Path to the persisted history file: `~/.mangocode/tip_history.json`.
    fn history_path() -> Option<std::path::PathBuf> {
        dirs::home_dir().map(|h| h.join(".mangocode").join("tip_history.json"))
    }
    ...
    /// Load history from `~/.mangocode/tip_history.json`.
    /// Returns an empty `TipHistory` if the file does not exist; malformed files
    /// are logged and ignored because tips are non-critical.
    pub fn load() -> Self {
        ...
        match Self::load_from_path(&path) {
            ...
            Err(err) => {
                tracing::warn!(
                    error = %err,
                    path = %path.display(),
                    "failed to load tip history; using defaults"
                );
                Self::default()
            }
        }
    ...
    /// Persist history to `~/.mangocode/tip_history.json`.
    /// Tip persistence is non-critical, but failures are logged for operators.
    pub fn save(&self) {
        let path = match Self::history_path() {
            Some(p) => p,
            None => return,
        ...
        if let Some(parent) = path.parent() {
            if let Err(err) = std::fs::create_dir_all(parent) {
                tracing::warn!(
                    error = %err,
                    path = %parent.display(),
                    "failed to create tip history directory"
                );
                return;
            }
        }
            ...
            Ok(json) => {
                if let Err(err) = std::fs::write(&path, json) {
                    tracing::warn!(
                        error = %err,
                        path = %path.display(),
                        "failed to write tip history"
                    );
                }
            }
            Err(err) => {
                tracing::warn!(error = %err, "failed to serialize tip history");
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn tip_history_load_from_path_rejects_malformed_json() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("tip_history.json");
        std::fs::write(&path, "{not-json").unwrap();

        let err = TipHistory::load_from_path(&path).unwrap_err();
