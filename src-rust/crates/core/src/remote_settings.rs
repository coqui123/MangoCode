...
// ---------------------------------------------------------------------------
// Remote settings fetch entry point
// ---------------------------------------------------------------------------

/// Fetch remote managed settings when the current auth state is eligible.
/// Returns an empty object when settings are unavailable or the account is not
/// eligible, so startup remains fail-open.
pub async fn fetch_remote_managed_settings() -> Value {
    ...
    match manager.fetch_with_retry(None).await {
        Ok(Some(settings)) => {
            ...
            if let Err(e) = manager.save_cache(&cache).await {
...
/// On-disk cache structure stored in ~/.mangocode/remote-settings.json.
...
impl RemoteSettingsManager {
    ...
    pub fn new(config: RemoteSettingsConfig) -> Self {
            ...
            .build()
        {
            ...
            Err(err) => {
                warn!(
                    error = %err,
                    "failed to build timed remote settings HTTP client; falling back to default client"
                );
                reqwest::Client::new()
            }
        };
        ...
        match tokio::fs::read_to_string(&self.cache_path).await {
            Ok(text) => match serde_json::from_str::<Value>(&text) {
                ...
                Err(err) => {
                    warn!(
                        ...
                        error = %err,
                        "failed to parse cached remote settings"
                    );
                    None
                }
            },
            ...
            Err(err) => {
                warn!(
                    path = %self.cache_path.display(),
                    error = %err,
                    "failed to read cached remote settings"
                );
                None
            }
        }
    ...
    pub async fn clear_cache(&self) {
        match tokio::fs::remove_file(&self.cache_path).await {
            ...
            Err(err) => warn!(
                path = %self.cache_path.display(),
                error = %err,
                "failed to clear cached remote settings"
            ),
        }
    }
    ...
    async fn fetch_once(&self, cached_checksum: Option<&str>) -> Result<Option<Value>> {
        ...
        match status.as_u16() {
            ...
            401 | 403 => {
                ...
                let body = response_error_body(resp, "remote settings fetch").await;
                anyhow::bail!(
        ...
        let body_text = resp
            .text()
            .await
            .context("Remote settings: failed to read response body")?;
        let body: Value = serde_json::from_str(&body_text).with_context(|| {
            format!(
                "Remote settings: failed to parse response JSON: {}",
    ...
    pub async fn fetch_once_and_cache(&self) -> Option<Value> {
        ...
        let cached_settings: Option<Value> =
            cached_raw
                .as_deref()
                .and_then(|t| match serde_json::from_str(t) {
                    ...
                    Err(err) => {
                        warn!(
                            "Remote settings: failed to parse cached settings from {}: {}",
...
/// Return the ~/.mangocode directory, falling back to the current directory.
