...
impl HarnessRecorder {
        ...
        if let Some(parent) = db_path.parent() {
            if let Err(err) = std::fs::create_dir_all(parent) {
                tracing::warn!(
                    error = %err,
                    path = %parent.display(),
                    "failed to create harness database directory"
                );
            }
        }
        let store = match SqliteSessionStore::open(&db_path) {
            ...
            Err(err) => {
                tracing::warn!(
                    ...
                    "failed to open harness database"
                );
                None
            }
        };
    ...
    pub fn record(
        ...
        if let Some(store) = self.store.lock().as_ref() {
            if let Err(err) = store.append_harness_event(&event) {
                tracing::warn!(
                    error = %err,
    ...
    pub fn record_turn_status(
        ...
        if let Some(store) = self.store.lock().as_ref() {
            if let Err(err) =
                store.upsert_harness_turn(&self.session_id, turn_id, status, model, cwd, now)
            {
                tracing::warn!(
                    error = %err,
                    session_id = %self.session_id,
                    turn_id = %turn_id,
                    "failed to persist harness turn status"
                );
            }
        }
        self.record(
    ...
    pub fn capture_checkpoint(
        ...
        if let Some(store) = self.store.lock().as_ref() {
            if let Err(err) = store.save_harness_checkpoint(&checkpoint) {
                tracing::warn!(
                    error = %err,
                    session_id = %self.session_id,
                    checkpoint_id = %checkpoint.checkpoint_id,
                    "failed to persist harness checkpoint"
                );
            }
        }
        self.record(
...
pub fn list_events(session_id: &str, limit: usize) -> anyhow::Result<Vec<HarnessEvent>> {
    let store = SqliteSessionStore::open(&default_db_path())?;
    let limit = limit.min(i64::MAX as usize);
    // Return the NEWEST `limit` events (ascending order). Consumers scan this
    // for the latest work run; truncating to the oldest rows would hide recent
    // runs once a session exceeds `limit` events.
    Ok(store
        .list_harness_events_latest(session_id, limit)?
        .into_iter()
        .map(|(_, event)| event)
        .collect())
}
...
fn capture_git_checkpoint(
    ...
    cleanup_git_index_file(&index_path);
    Ok((git_ref, checkpoint_id))
}

fn cleanup_git_index_file(path: &Path) {
    if let Err(err) = std::fs::remove_file(path) {
        if err.kind() != std::io::ErrorKind::NotFound {
            tracing::warn!(
                path = %path.display(),
                error = %err,
                "failed to remove temporary git index file"
            );
        }
    }
}
...
fn append_trace_jsonl(event: &HarnessEvent) {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        tracing::warn!(
            error = %err,
            path = %dir.display(),
            "failed to create harness trace directory"
        );
        return;
    }
    ...
    let mut file = match OpenOptions::new().create(true).append(true).open(&path) {
        ...
        Err(err) => {
            tracing::warn!(
                error = %err,
                path = %path.display(),
                "failed to open harness trace file"
            );
            return;
        }
    };
        ...
        Ok(line) => {
            if let Err(err) = writeln!(file, "{}", line) {
                tracing::warn!(
                    error = %err,
                    path = %path.display(),
                    "failed to write harness trace event"
                );
            }
        }
        Err(err) => {
            tracing::warn!(
                ...
                "failed to serialize harness trace event"
            );
        }
    }
}
