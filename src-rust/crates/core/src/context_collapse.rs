...
fn get_tokenizer() -> Option<&'static CoreBPE> {
    TOKENIZER
        .get_or_init(|| match tiktoken_rs::cl100k_base() {
            ...
            Err(err) => {
                tracing::warn!(
                    "failed to load cl100k tokenizer; using heuristic token estimates: {err}"
                );
                None
            }
        })
...
fn estimate_tokens(text: &str) -> u64 {
    ...
    get_tokenizer()
        .map(|tokenizer| tokenizer.encode_with_special_tokens(text).len() as u64)
    ...
    let json = match std::fs::read_to_string(&path) {
        ...
        Err(err) => {
            tracing::warn!(
                error = %err,
                path = %path.display(),
                "failed to read context collapse state"
            );
            return None;
        }
    };
    match serde_json::from_str(&json) {
        ...
        Err(err) => {
            tracing::warn!(
                error = %err,
                path = %path.display(),
                "failed to parse context collapse state"
            );
            None
        }
    }
