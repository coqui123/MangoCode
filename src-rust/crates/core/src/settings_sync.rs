...
impl SettingsSyncManager {
    ...
    pub fn new(oauth_token: String, base_url: String) -> Self {
            ...
            .build()
        {
            ...
            Err(err) => {
                warn!(
                    error = %err,
                    "Settings sync: failed to build timeout-configured HTTP client; using default client"
                );
                reqwest::Client::new()
            }
        };
    ...
    pub async fn download(&self, cached_checksum: Option<&str>) -> Result<Option<SyncedData>> {
        ...
        if status.as_u16() != 200 {
            let body = response_error_body(resp, "settings sync download").await;
            anyhow::bail!(
        ...
        let body = resp
            .text()
            .await
            .context("Settings sync download: failed to read response body")?;
        let UserSyncData {
            user_id,
            version,
            last_modified,
            ...
            format!(
                "Settings sync download: failed to parse response JSON: {}",
    ...
    pub async fn apply_to_local(&self, data: &SyncedData, project_id: Option<&str>) -> ApplyResult {
        ...
        if let Some(ref settings_json) = data.settings {
            ...
            match serde_json::to_string_pretty(settings_json) {
                Ok(content) => match write_file_for_sync(&path, &content).await {
                    Ok(()) => {
                ...
                Err(e) => {
                    warn!(
                        error = %e,
                        "Settings sync: failed to serialize downloaded user settings"
                    );
                }
            }
        }
        ...
        if let Some(pid) = project_id {
            let cwd = match std::env::current_dir() {
                ...
                Err(e) => {
                    warn!(
                        error = %e,
                        "Settings sync: failed to resolve current directory; skipping project files"
                    );
                    return result;
                }
            };
            ...
            if let Some(content) = data.memory_files.get(&proj_settings_key) {
                let path = cwd.join(".mangocode").join("settings.local.json");
            ...
            if let Some(content) = data.memory_files.get(&proj_memory_key) {
                let path = cwd.join("AGENTS.local.md");
                match write_file_for_sync(&path, content).await {
                    Ok(()) => {
                        result.memory_written = true;
                        result.applied_count += 1;
    ...
    async fn put_entries(&self, entries: HashMap<String, String>) -> Result<()> {
        ...
        if !(200..300).contains(&status.as_u16()) {
            let body = response_error_body(resp, "settings sync upload").await;
            anyhow::bail!(
        ...
        match resp.json::<UploadResponse>().await {
            ...
            Err(err) => {
                warn!(
                    error = %err,
                    "Settings sync: failed to parse upload response body"
                );
            }
        }
        Ok(())
    ...
    for (key, value) in entries {
        if key == SYNC_KEY_USER_SETTINGS {
            match serde_json::from_str(&value) {
                Ok(settings) => data.settings = Some(settings),
                Err(err) => {
                    warn!(
                        error = %err,
                        "Settings sync: downloaded user settings JSON is invalid"
                    );
                }
            }
        } else {
    ...
    if let Some(pid) = project_id {
        let cwd = match std::env::current_dir() {
            ...
            Err(e) => {
                warn!(
                    error = %e,
                    "Settings sync: failed to resolve current directory; skipping project upload entries"
                );
                return entries;
            }
        };
    ...
    let meta = match tokio::fs::metadata(path).await {
        ...
        Err(e) => {
            warn!(
                path = %path.display(),
                error = %e,
                "Settings sync: failed to inspect local file"
            );
            return None;
        }
    };
    ...
    let content = match tokio::fs::read_to_string(path).await {
        ...
        Err(e) => {
            warn!(
                path = %path.display(),
                error = %e,
                "Settings sync: failed to read local file"
            );
            return None;
        }
    };
