...
//     `~/.mangocode/pastes/<sha256-hex>` and referenced by hash.
        ...
        if let Err(err) = std::fs::remove_file(&self.path) {
            if err.kind() != std::io::ErrorKind::NotFound {
                warn!(
                    ...
                    error = %err,
                    "failed to remove prompt history lock file"
                );
            }
        }
    }
...
/// Persist `text` to `~/.mangocode/pastes/<hash>`.  Fire-and-forget.
...
/// Read text from `~/.mangocode/pastes/<hash>`, returning `None` if missing.
...
/// Append `entry` to `~/.mangocode/history.jsonl`.
///
/// The call is fire-and-forget: it spawns a background Tokio task and returns
/// immediately.  If `MANGOCODE_SKIP_PROMPT_HISTORY` is truthy the call is
/// a no-op.
pub fn add_to_history(entry: HistoryEntry) {
    if std::env::var("MANGOCODE_SKIP_PROMPT_HISTORY")
...
/// Read `~/.mangocode/history.jsonl`, filter by `project`, and return up to
