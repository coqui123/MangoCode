...
///
/// Strips provider prefixes (`openai/...`), collapses variant suffixes
/// (`-low`, `-medium`, ...), and maps legacy labels to canonical Codex IDs.
...
/// Return true when a bare user-facing model name resolves to a Codex OAuth
/// backend model. Provider-prefixed strings are intentionally rejected so
/// `"provider/model"` routing remains explicit.
pub fn is_bare_codex_model_alias(model: &str) -> bool {
    if model.contains('/') {
        return false;
    }
...
mod tests {
    ...
    #[test]
    fn codex_model_alias_check_accepts_only_bare_codex_aliases() {
        assert!(is_bare_codex_model_alias("gpt-5.5"));
        assert!(is_bare_codex_model_alias("gpt-5-codex"));
        assert!(is_bare_codex_model_alias("gpt-5.1-codex-low"));
        assert!(!is_bare_codex_model_alias("gpt-4o"));
        assert!(!is_bare_codex_model_alias("openai-codex/gpt-5.5"));
    }
}
