        ...
        Ok(val) => match serde_json::from_str(&val) {
            ...
            Err(err) => {
                tracing::warn!(
                    ...
                    env_var = %key,
                    error = %err,
                    "failed to parse dynamic config environment value"
                );
                default
            }
        },
...
/// Return `true` when MangoCode should run in "bare" (minimal) mode.
///
/// Bare mode skips LSP, plugin, and MCP startup for a faster, lighter
/// experience.  It is enabled by either:
///   - The `MANGOCODE_SIMPLE=1` environment variable, OR
///   - The `--bare` flag in `std::env::args()`.
pub fn is_bare_mode() -> bool {
    // Check env var
    if is_env_truthy(std::env::var("MANGOCODE_SIMPLE").ok().as_deref()) {
        return true;
    }
...
mod tests {
    ...
    #[test]
    fn dynamic_config_returns_default_for_malformed_json() {
        let _guard = EnvVarGuard::set(
            "MANGOCODE_DYNAMIC_CONFIG___TEST_MALFORMED_KEY_XYZZY__",
            "{bad json",
        );

        let val: u32 = get_dynamic_config("__test_malformed_key_xyzzy__", 42u32);
