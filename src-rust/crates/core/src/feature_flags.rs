...
pub const FLAG_EXECUTION_SCRATCHPAD: &str = "execution_scratchpad";
/// Enable a one-shot self-review (critic) turn after a file-changing run.
/// When the completion gate is about to finalize a run that modified files,
/// the agent is sent a single self-critique prompt to verify its diff against
/// the original request before finishing. Off by default (opt-in); enable with
/// `MANGOCODE_FLAG_SELF_REVIEW=1` or via flags.json.
pub const FLAG_SELF_REVIEW: &str = "self_review";
/// Enable research-grade, frontier-tilted MCTS plan search before executing a
/// turn. Instead of committing to the first plan, the agent grows a tree of
/// candidate plans, scores each via sandboxed rollouts (tests + clippy + critic),
/// and applies the highest-value branch. Expensive (5–50x tokens) and opt-in;
/// enable with `MANGOCODE_FLAG_PLAN_SEARCH=1` or via flags.json. See
/// `crates/query/src/plan_search.rs`.
pub const FLAG_PLAN_SEARCH: &str = "plan_search";
...
impl FeatureFlags {
    ...
    pub fn set(name: &str, value: bool) -> anyhow::Result<()> {
        Self::load_from_disk_result().context(
            "refusing to update feature flags because the existing flags file could not be loaded",
        )?;
    ...
    fn load_from_disk() -> HashMap<String, bool> {
        match Self::load_from_disk_result() {
            Ok(flags) => flags,
            Err(e) => {
                warn!("Failed to load feature flags from disk: {}", e);
                HashMap::new()
            }
        }
    }

    fn load_from_disk_result() -> anyhow::Result<HashMap<String, bool>> {
        ...
        parse_flags_json(&text, &path)
    }

    fn save_to_disk(&self) -> anyhow::Result<()> {
        let path = Self::flags_path().ok_or_else(|| anyhow!("Cannot determine home directory"))?;
...
fn parse_flags_json(text: &str, path: &Path) -> anyhow::Result<HashMap<String, bool>> {
    serde_json::from_str::<HashMap<String, bool>>(text)
...
mod tests {
    ...
    #[test]
    fn parse_flags_json_reports_malformed_files() {
        let path = PathBuf::from("flags.json");

        let err = parse_flags_json("{bad json", &path).unwrap_err();

        assert!(err.to_string().contains("Failed to parse flags.json"));
    }
