...
pub enum MemoryScope {
    /// `~/.mangocode/rules/*.md` — global managed policy.
    Managed,
    /// `~/.mangocode/AGENTS.md` — user-level memory.
    User,
    /// `{project_root}/AGENTS.md` — project-level memory.
    Project,
    /// `{project_root}/.mangocode/AGENTS.md` — local override.
    Local,
}
...
pub fn load_all_memory_files(project_root: &Path) -> Vec<MemoryFileInfo> {
    ...
    if let Some(home) = dirs::home_dir() {
        let rules_dir = home.join(".mangocode/rules");
        ...
        let user_claude = home.join(".mangocode/AGENTS.md");
    ...
    // 4. Local: {project_root}/.mangocode/AGENTS.md
    let local_claude = project_root.join(".mangocode/AGENTS.md");
