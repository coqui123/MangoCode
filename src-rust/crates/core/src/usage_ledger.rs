...
impl UsageLedger {
            ...
            Some(path) => match Self::load_from_path(&path) {
                ...
                Err(err) => {
                    tracing::warn!(
                        error = %err,
                        path = %path.display(),
                        "failed to load usage ledger; using defaults"
                    );
                    Self::default()
                }
            },
    ...
    pub fn record_session(&mut self, record: SessionCostRecord) {
        ...
        if let Err(err) = self.save() {
            tracing::warn!(error = %err, "failed to persist usage ledger");
        }
    }
    ...
    pub fn backfill_from_analytics(&mut self) {
        ...
        if imported > 0 {
            if let Err(err) = self.save() {
                tracing::warn!(error = %err, "failed to persist usage ledger backfill");
            }
        }
    }
}
...
mod tests {
    ...
    fn temp_usage_ledger_path() -> PathBuf {
        std::env::temp_dir()
            .join(format!(
                "mangocode-usage-ledger-test-{}",
                uuid::Uuid::new_v4()
            ))
            .join("usage.json")
    }
    ...
    fn load_from_path_defaults_when_missing() {
        let path = temp_usage_ledger_path();

        let ledger = UsageLedger::load_from_path(&path).unwrap();
    ...
    #[test]
    fn load_from_path_rejects_malformed_usage_json() {
        let path = temp_usage_ledger_path();
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
        std::fs::write(&path, "{not-json").unwrap();

        let err = UsageLedger::load_from_path(&path).unwrap_err();
