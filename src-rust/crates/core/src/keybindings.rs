...
fn parse_user_binding_context(value: &str) -> KeyContext {
    match serde_json::from_value::<KeyContext>(serde_json::Value::String(value.to_string())) {
        Ok(context) => context,
        Err(err) => {
            tracing::warn!(
                context = value,
                error = %err,
                "invalid user keybinding context; using Global"
            );
            KeyContext::Global
        }
    }
        ...
        for user_binding in &user.bindings {
            if let Some(chord) = parse_chord(&user_binding.chord) {
                let context = user_binding
                    .context
                    .as_deref()
                    .map(parse_user_binding_context)
...
mod tests {
  ...
  "bindings": [
    ...
    #[test]
    fn test_user_keybinding_context_parsing_defaults_invalid_values_to_global() {
        assert_eq!(parse_user_binding_context("Chat"), KeyContext::Chat);
        assert_eq!(parse_user_binding_context("Chat\""), KeyContext::Global);
    }
