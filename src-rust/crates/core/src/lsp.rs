...
impl LspClient {
    ...
    pub async fn initialize(&mut self, root_uri: &str) -> anyhow::Result<()> {
        let params = json!({
            ...
            "clientInfo": { "name": "mangocode", "version": "1.0" },
            "rootUri": root_uri,
            "capabilities": {
                "textDocument": {
                    "publishDiagnostics": {
    ...
    pub async fn shutdown(&mut self) -> anyhow::Result<()> {
        ...
        // Attempt graceful shutdown; continue to process termination on failure.
        if let Err(err) = self.send_request_inner("shutdown", json!(null)).await {
            tracing::debug!(
                error = %err,
                server = %self.server_name,
                "failed to send LSP shutdown request"
            );
        }
        if let Err(err) = self.send_notification_inner("exit", json!(null)).await {
            tracing::debug!(
                error = %err,
                server = %self.server_name,
                "failed to send LSP exit notification"
            );
        }
        ...
        if let Some(mut child) = self.process.take() {
            ...
            match tokio::time::timeout(std::time::Duration::from_secs(5), child.wait()).await {
                Ok(Ok(_status)) => {}
                Ok(Err(err)) => {
                    tracing::warn!(error = %err, "failed to wait for LSP server shutdown");
                    if let Err(kill_err) = child.kill().await {
                        tracing::warn!(
                            error = %kill_err,
                            "failed to kill LSP server after wait failure"
                        );
                    }
                    if let Err(wait_err) = child.wait().await {
                        tracing::warn!(
                            error = %wait_err,
                            "failed to wait for killed LSP server after wait failure"
                        );
                    }
                }
                Err(_) => {
                    tracing::warn!("LSP server did not exit within shutdown timeout; killing it");
                    if let Err(kill_err) = child.kill().await {
                        tracing::warn!(
                            error = %kill_err,
                            "failed to kill timed-out LSP server"
                        );
                    }
                    if let Err(wait_err) = child.wait().await {
                        tracing::warn!(
                            error = %wait_err,
                            "failed to wait for killed LSP server"
                        );
                    }
                }
            }
