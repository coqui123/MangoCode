...
pub fn log_event(event_name: &str, metadata: &[(&str, &str)]) {
    ...
    let line = match serde_json::to_string(&serde_json::Value::Object(entry)) {
        Ok(line) => line,
        Err(err) => {
            tracing::warn!(error = %err, "failed to serialize analytics event");
            return;
        }
    };
            ...
            .open(&path)
        {
            ...
            Err(err) => {
                tracing::warn!(
                    error = %err,
                    path = %path.display(),
                    "failed to open analytics event file"
                );
                return;
            }
        };
    ...
    if let Some(dir) = analytics_dir() {
        if let Err(err) = std::fs::create_dir_all(&dir) {
            tracing::warn!(
                error = %err,
                path = %dir.display(),
                "failed to create analytics directory during telemetry initialization"
            );
        }
    }
        ...
        if let Some(provider) = crate::session_tracing::otel_impl::PROVIDER.get() {
            for result in provider.force_flush() {
                if let Err(err) = result {
                    tracing::warn!(
                        error = ?err,
                        "failed to flush OpenTelemetry trace provider"
                    );
                }
            }
        }
...
/// Defaults to off; users can opt-in with MANGOCODE_ENABLE_TELEMETRY=1.
/// The free/OSS build respects this preference but does not phone home.
pub fn is_telemetry_enabled() -> bool {
    std::env::var("MANGOCODE_ENABLE_TELEMETRY")
        .as_deref()
        .unwrap_or("0")
        == "1"
}
