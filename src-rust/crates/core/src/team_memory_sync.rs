...
fn http_error(body: String, fallback: impl AsRef<str>) -> anyhow::Error {
    let body = body.trim();
    if body.is_empty() {
        anyhow::anyhow!("{}", fallback.as_ref())
    } else {
        anyhow::anyhow!("{}: {}", fallback.as_ref(), body)
    }
}
...
pub fn validate_memory_path(path: &str) -> Result<()> {
    ...
    if path.len() >= 2 {
        ...
        if first.is_some_and(|c| c.is_ascii_alphabetic()) && chars.next() == Some(':') {
...
pub struct TeamMemorySync {
    ...
    token: String,
    /// Local directory synced to the server's key namespace.
    team_dir: PathBuf,
}
...
impl TeamMemorySync {
    ...
    pub async fn pull(&self, state: &mut SyncState) -> Result<()> {
        ...
        if !http_status.is_success() {
            let body = response_error_body(response, "team memory pull").await;
            return Err(http_error(
                body,
                format!("team memory pull failed with status {http_status}"),
            ));
        }
    ...
    async fn upload_batch(&self, batch: Vec<TeamMemoryEntry>, state: &mut SyncState) -> Result<()> {
        ...
        match status.as_u16() {
            ...
            412 => {
                let body = response_error_body(response, "team memory upload").await;
                Err(http_error(
                    body,
            ...
            413 => {
                let body = response_error_body(response, "team memory upload").await;
                Err(http_error(body, "Payload too large (413)"))
            }
            401 | 403 => {
                let body = response_error_body(response, "team memory upload").await;
                Err(http_error(body, format!("Authentication error ({status})")))
            }
            _ => {
                let body = response_error_body(response, "team memory upload").await;
                Err(http_error(
            ...
            while let Some(entry) = read_dir.next_entry().await? {
                ...
                if path.is_dir() {
                    ...
                    let key = path
                        ...
                        .with_context(|| {
                            format!(
                                "scan_local_files: path {:?} escaped team dir {:?}",
                                path, self.team_dir
                            )
                        })?
                        .to_string_lossy()
