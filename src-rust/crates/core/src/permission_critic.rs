...
fn parse_critic_response(response: &str) -> (bool, String) {
    ...
    let allowed = first_word.eq_ignore_ascii_case("ALLOW");
    let recognized_decision = allowed || first_word.eq_ignore_ascii_case("DENY");
    let reasoning = if recognized_decision {
        trimmed
            .get(first_word.len()..)
            .unwrap_or("")
            .trim()
...
mod tests {
    ...
    fn test_parse_critic_response_lowercase() {
        let (allowed, reason) = parse_critic_response("allow seems fine");
        assert!(allowed);
        assert_eq!(reason, "seems fine");
    }

    #[test]
    fn test_parse_critic_response_garbage() {
        let (allowed, reason) = parse_critic_response("maybe idk");
        assert!(!allowed); // anything that isn't ALLOW → deny
        assert_eq!(reason, "maybe idk");
    }
