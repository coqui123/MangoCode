...
pub fn get_ide_context() -> Option<String> {
    let lockfile_dir = dirs::home_dir()?.join(".mangocode").join("ide");
    if !lockfile_dir.exists() {
        return None;
    }
    let entries = match std::fs::read_dir(&lockfile_dir) {
        Ok(entries) => entries,
        Err(err) => {
            tracing::warn!(
                path = %lockfile_dir.display(),
                error = %err,
                "failed to read IDE context lockfile directory"
            );
            return None;
        }
    };
        ...
        let entry = match entry {
            ...
            Err(err) => {
                tracing::warn!(
                    path = %lockfile_dir.display(),
                    error = %err,
                    "failed to read IDE context lockfile directory entry"
                );
                continue;
            }
        };
            ...
            let content = match std::fs::read_to_string(&path) {
                ...
                Err(err) => {
                    tracing::warn!(
                        path = %path.display(),
                        error = %err,
                        "failed to read IDE context lockfile"
                    );
                    continue;
                }
            };
            let info = match serde_json::from_str::<serde_json::Value>(&content) {
                Ok(info) => info,
                Err(err) => {
                    tracing::warn!(
                        path = %path.display(),
                        error = %err,
                        "failed to parse IDE context lockfile"
                    );
                    continue;
                }
            };
    ...
    let entries = match std::fs::read_dir(dir) {
        ...
        Err(err) => {
            tracing::warn!(
                path = %dir.display(),
                error = %err,
                "failed to read directory while scanning changed files"
            );
            return;
        }
    };
        ...
        let entry = match entry {
            ...
            Err(err) => {
                tracing::warn!(
                    path = %dir.display(),
                    error = %err,
                    "failed to read directory entry while scanning changed files"
                );
                continue;
            }
        };
            ...
            let meta = match entry.metadata() {
                ...
                Err(err) => {
                    tracing::warn!(
                        path = %path.display(),
                        error = %err,
                        "failed to inspect file while scanning changed files"
                    );
                    continue;
                }
            };
            let modified = match meta.modified() {
                ...
                Err(err) => {
                    tracing::warn!(
                        path = %path.display(),
                        error = %err,
                        "failed to read file timestamp while scanning changed files"
                    );
                    continue;
                }
            };
