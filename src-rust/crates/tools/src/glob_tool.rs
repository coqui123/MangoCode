...
impl Tool for GlobTool {
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> ToolResult {
        ...
        let (entries, skipped_glob_errors): (Vec<PathBuf>, usize) = match glob::glob(&pattern_str) {
            Ok(paths) => {
                ...
                for path in paths {
                    match path {
                        ...
                        Err(err) => {
                            ...
                            debug!(
                                error = %err,
                                "skipping glob match with traversal error"
                            );
                        }
                    }
                }
        ...
        let mut skipped_metadata_errors = 0usize;
        let mut entries_with_time: Vec<(PathBuf, std::time::SystemTime)> = entries
            .into_iter()
            .filter_map(
                |path| match std::fs::metadata(&path).and_then(|m| m.modified()) {
                    Ok(mtime) => Some((path, mtime)),
                    Err(err) => {
                        skipped_metadata_errors += 1;
                        debug!(
                            path = %path.display(),
                            error = %err,
                            "skipping glob match with unreadable metadata"
                        );
                        None
                    }
                },
        ...
        if entries_with_time.is_empty() && (skipped_glob_errors + skipped_metadata_errors) > 0 {
            return ToolResult::error(format!(
                "Matched files for pattern \"{}\" in {}, but could not read any of them ({} skipped)",
                params.pattern,
                base_dir.display(),
                skipped_glob_errors + skipped_metadata_errors
            ));
        }
        ...
        if skipped_glob_errors + skipped_metadata_errors > 0 {
            output.push_str(&format!(
                "\nSkipped {} matched paths due to traversal or metadata errors.\n",
                skipped_glob_errors + skipped_metadata_errors
            ));
        }
