    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> ToolResult {
        ...
        let result = match edit_mode {
            ...
            other => {
                return ToolResult::error(format!(
                    "Invalid edit_mode after normalization: {}",
                    other
                ));
            }
        };
        ...
        match result {
            Ok(msg) => {
                ...
                if let Err(e) = crate::fs_atomic::write_atomic(&path, updated.as_bytes()).await {
                    return ToolResult::error(format!("Failed to write notebook: {}", e));
                }
                // Run any configured formatter, then record the on-disk result
                // (matches FileEdit/FileWrite).
                crate::try_format_file(&path.to_string_lossy(), ctx).await;
                let after = match tokio::fs::read(&path).await {
                    Ok(bytes) => bytes,
                    Err(_) => updated.as_bytes().to_vec(),
                };
                ctx.record_file_change(path.clone(), content.as_bytes(), &after, self.name());
