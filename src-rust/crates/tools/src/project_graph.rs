...
impl Tool for ProjectGraphTool {
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "query": {
                    "type": "string",
                    "description": "Search terms for action=query, action=community, action=neighbors, or action=explain."
                },
                "community": {
                    "type": "integer",
                    "minimum": 0,
                ...
                "graph_path": {
                    "type": "string",
                    "description": "Existing graph JSON to load for report, json, stats, benchmark, god_nodes, surprises, query, community, neighbors, path, explain, html, tree, or callflow. Query and export actions default to <path>/graphify-out/graph.json when present."
                },
                "question": {
                    "type": "string",
                ...
                "limit": {
                    ...
                    "minimum": 1,
                    "maximum": 50,
                    "description": "Maximum report, benchmark, god_nodes, surprises, query, community, neighbors, path, or explain items."
                }
            },
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> ToolResult {
        ...
        match action.as_str() {
            ...
            "stats" | "benchmark" | "god_nodes" | "surprises" | "query" | "community"
                        ...
                        match persist_project_graph(
                            ...
                            Ok(writes) => {
                                ...
                                ToolResult::success(format!(
                                    ...
                                    writes[1].path.display(),
                                    writes[2].path.display(),
                                    writes[3].path.display()
                                ))
                                .with_metadata(json!({
                                    "graph_path": writes[0].path.display().to_string(),
                                    "report_path": writes[1].path.display().to_string(),
                                    "manifest_path": writes[2].path.display().to_string(),
                                    "html_path": writes[3].path.display().to_string(),
                                    "nodes": graph.nodes.len(),
                                    "edges": graph.edges.len(),
                                    "files_scanned": graph.files_scanned,
    ...
    fn capabilities(&self, input: &Value) -> ToolCapabilities {
        ...
        if !matches!(
            ...
            return crate::default_capabilities_for_tool(
                self.name(),
                PermissionLevel::ReadOnly,
                input,
            );
        ...
        let mut capabilities =
            crate::default_capabilities_for_tool(self.name(), PermissionLevel::Write, input);
        ...
        capabilities.affected_paths.push(
            input
                .get(path_key)
                .and_then(Value::as_str)
...
fn build_project_graph(root: &Path, max_files: usize) -> Result<ProjectGraph, String> {
    ...
    let symbol_patterns = [
        Regex::new(r"\b(?P<kind>fn|struct|enum|trait|impl|class|interface|def|function|func|module|namespace)\s+(?P<label>[A-Za-z_][A-Za-z0-9_]*)").unwrap(),
        Regex::new(r"\b(?P<kind>type)\s+(?P<label>[A-Za-z_][A-Za-z0-9_]*)\s*(=|struct|interface)").unwrap(),
        Regex::new(r"(?im)\bcreate\s+(?P<kind>table|view|trigger|procedure|function)\s+(?P<label>[A-Za-z_][A-Za-z0-9_.$]*)").unwrap(),
        Regex::new(r"(?m)^\s*(?P<label>[A-Za-z_][A-Za-z0-9_]*)\s*\(\)\s*\{").unwrap(),
        Regex::new(r"\b(?P<kind>const|let|var)\s+(?P<label>[A-Za-z_][A-Za-z0-9_]*)\s*=\s*(async\s*)?(\([^)]*\)|[A-Za-z_][A-Za-z0-9_]*)\s*=>").unwrap(),
    ];
    let heading_re = Regex::new(r"(?m)^(#{1,6})\s+(.{1,120})$").unwrap();
    let import_re = Regex::new(r#"(?m)^\s*(use|mod|import|from|require)\s+([^;\n]+)"#).unwrap();
    ...
    for path in &files {
        ...
        for (symbol, target) in &known_symbols {
            if target.contains(&format!(":{rel}:")) || !text.contains(symbol) {
                continue;
            }
...
fn global_add_project_graph(
    ...
    if repo_tag.trim().is_empty() {
        return Err("ProjectGraph global_add requires a non-empty repo_tag.".to_string());
    }
...
fn unique_memory_path(memory_dir: &Path, filename: &str) -> PathBuf {
...
aside {{ border-right: 1px solid #374151; padding: 16px; overflow: auto; }}
section {{ padding: 16px; overflow: auto; }}
input {{ width: 100%; box-sizing: border-box; padding: 9px 10px; border-radius: 6px; border: 1px solid #4b5563; background: #0f172a; color: #f9fafb; }}
h1 {{ font-size: 20px; margin: 0 0 4px; }}
h2 {{ font-size: 14px; margin: 18px 0 8px; color: #f9fafb; }}
.muted {{ color: #9ca3af; font-size: 13px; }}
.pill {{ display: inline-block; padding: 2px 7px; border-radius: 999px; background: #374151; font-size: 12px; margin-left: 6px; }}
.row {{ padding: 8px 0; border-bottom: 1px solid #1f2937; cursor: pointer; }}
.row:hover {{ background: #1f2937; }}
.label {{ font-weight: 650; }}
.meta {{ color: #9ca3af; font-size: 12px; margin-top: 2px; }}
.grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(210px, 1fr)); gap: 10px; }}
.card {{ border: 1px solid #374151; border-radius: 8px; padding: 10px; background: #172033; }}
table {{ width: 100%; border-collapse: collapse; font-size: 13px; }}
td, th {{ border-bottom: 1px solid #1f2937; padding: 7px; text-align: left; }}
@media (max-width: 780px) {{ main {{ grid-template-columns: 1fr; }} aside {{ border-right: 0; border-bottom: 1px solid #374151; }} }}
</style>
</head>
<body>
...
<main>
<aside>
<input id="search" placeholder="Search nodes, files, communities">
<h2>Communities</h2>
...
<div id="nodes"></div>
<h2>Edges</h2>
<table><thead><tr><th>Source</th><th>Relation</th><th>Target</th><th>Confidence</th></tr></thead><tbody id="edges"></tbody></table>
</section>
</main>
...
document.getElementById('communities').innerHTML = graph.communities.map(c => `<div class="row" data-community="${{c.id}}"><div class="label">#${{c.id}} ${{esc(c.label)}}</div><div class="meta">${{c.nodes}} nodes · ${{c.edges}} edges · cohesion ${{Number(c.cohesion).toFixed(3)}}</div></div>`).join('') || '<div class="muted">No communities</div>';
...
header {{ padding: 18px 24px; border-bottom: 1px solid #d1d5db; background: #ffffff; }}
main {{ display: grid; grid-template-columns: minmax(260px, 340px) 1fr; min-height: calc(100vh - 76px); }}
aside {{ border-right: 1px solid #d1d5db; padding: 16px 20px; background: #ffffff; }}
section {{ padding: 18px 22px; overflow: auto; }}
h1 {{ margin: 0 0 4px; font-size: 20px; }}
h2 {{ font-size: 14px; margin: 18px 0 10px; }}
.muted, small {{ color: #6b7280; font-size: 12px; display: block; }}
ul {{ margin: 0; padding: 0; list-style: none; }}
li {{ padding: 9px 0; border-bottom: 1px solid #e5e7eb; }}
pre.mermaid {{ background: #ffffff; border: 1px solid #d1d5db; border-radius: 8px; padding: 14px; overflow: auto; }}
...
@media (max-width: 860px) {{ main {{ grid-template-columns: 1fr; }} aside {{ border-right: 0; border-bottom: 1px solid #d1d5db; }} }}
</style>
</head>
<body>
...
<table><thead><tr><th>Source</th><th>Relation</th><th>Target</th><th>Confidence</th><th>File</th></tr></thead><tbody>{}</tbody></table>
</section>
</main>
<script type="module">
...
header {{ padding: 18px 24px; border-bottom: 1px solid #d1d5db; background: #ffffff; position: sticky; top: 0; }}
main {{ padding: 18px 24px; }}
h1 {{ margin: 0 0 4px; font-size: 20px; }}
.muted {{ color: #6b7280; font-size: 12px; }}
details {{ border-left: 3px solid #2563eb; margin: 8px 0; padding: 8px 10px; background: #ffffff; }}
summary {{ cursor: pointer; }}
.file {{ font-weight: 700; }}
ul {{ margin: 8px 0 0 18px; padding: 0; }}
li {{ margin: 4px 0; }}
.kind {{ display: inline-block; min-width: 72px; color: #374151; font-size: 12px; }}
</style>
</head>
<body>
  ...
  "skipped_files": 0
    ...
    fn project_graph_write_actions_are_mutating_capabilities() {
        ...
        assert!(global_add_caps
            .affected_paths
            .iter()
            .any(|path| path.contains("project-graph")));
