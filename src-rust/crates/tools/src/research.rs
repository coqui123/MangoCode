    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> ToolResult {
        ...
        match crate::browser_tool::rendered_extract_for_research(&url).await {
            Ok(content) => {
                ...
                save_cached_document_or_warn(&doc);
                upsert_research_index_or_warn(&doc);
    ...
    if let Some(mut cached) = load_cached_document(url) {
        ...
        if cached.verified_at_read_time {
            if rendered_fallback && should_upgrade_cached_doc(&cached) {
                ...
                {
                    save_cached_document_or_warn(&upgraded);
                    upsert_research_index_or_warn(&upgraded);
                    return Ok(upgraded);
                }
            }
            upsert_research_index_or_warn(&cached);
            return Ok(cached);
        }
    }
...
fn load_cached_document(url: &str) -> Option<ResearchDocument> {
    let path = research_cache_path(url);
    let raw = std::fs::read_to_string(&path).ok()?;
    let mut doc: ResearchDocument = match serde_json::from_str(&raw) {
        Ok(doc) => doc,
        Err(err) => {
            tracing::warn!(
                error = %err,
                path = %path.display(),
                url,
                "failed to parse cached research document"
            );
            return None;
        }
    };
...
fn save_cached_document(doc: &ResearchDocument) -> std::io::Result<()> {
    ...
    crate::fs_atomic::write_atomic_sync(&research_cache_path(&doc.url), json.as_bytes())
}
...
fn load_research_index() -> Vec<ResearchIndexEntry> {
    ...
    let entries = match serde_json::from_str::<Vec<ResearchIndexEntry>>(&raw) {
        Ok(entries) => entries,
        Err(err) => {
            tracing::warn!(
                error = %err,
                path = %path.display(),
                "failed to parse research source index"
            );
            return Vec::new();
        }
    };
...
fn save_research_index(entries: &[ResearchIndexEntry]) -> std::io::Result<()> {
    ...
    crate::fs_atomic::write_atomic_sync(&research_index_path(), json.as_bytes())
}

fn save_cached_document_or_warn(doc: &ResearchDocument) {
    if let Err(err) = save_cached_document(doc) {
        tracing::warn!(
            error = %err,
            url = %doc.url,
            "failed to save research document cache"
        );
    }
}

fn upsert_research_index_or_warn(doc: &ResearchDocument) {
    if let Err(err) = upsert_research_index(doc) {
        tracing::warn!(
            error = %err,
            url = %doc.url,
            "failed to update research source index"
        );
    }
}
