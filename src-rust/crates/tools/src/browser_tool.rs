...
async fn execute_browser_action(params: BrowserInput, ctx: &ToolContext) -> ToolResult {
    ...
    match action_result {
        ...
        Err(e) if is_recoverable_browser_error(&e) => {
            if let Err(reset_error) = reset_browser_session(&ctx.session_id).await {
                return ToolResult::error(format!(
                    "{} (browser session reset failed before retry: {})",
                    e, reset_error
                ));
            }
...
pub async fn rendered_extract_for_research(url: &str) -> Result<String, String> {
    match rendered_extract_for_research_once("__mangocode_research__", url).await {
        ...
        Err(error) if is_recoverable_browser_error(&error) => {
            reset_browser_session("__mangocode_research__")
                .await
                .map_err(|reset_error| {
                    format!(
                        "{} (browser session reset failed before retry: {})",
...
impl BrowserSession {
    ...
    async fn maybe_bypass_tls_interstitial(&self, page: &chromiumoxide::Page) {
        if std::env::var("MANGOCODE_BROWSER_ACCEPT_INSECURE_TLS_UI")
            ...
            .unwrap_or(false)
        {
            if let Err(err) = page.evaluate("void 0").await {
                tracing::debug!(error = %err, "failed to focus page before TLS interstitial bypass");
            }
            if let Err(err) = page.execute(InsertTextParams::new("thisisunsafe")).await {
                tracing::debug!(error = %err, "failed to type TLS interstitial bypass phrase");
            }
        }
    }
        ...
        if ua.contains("Headless") {
                ...
                .build()
            {
                if let Err(err) = page.set_user_agent(params).await {
                    tracing::debug!(
                        error = %err,
                        "failed to strip Headless token from browser user agent"
                    );
                }
            }
        }
    ...
    async fn close(&mut self) -> Result<(), String> {
        ...
        if let Err(err) = self.browser.close().await {
            tracing::warn!(error = %err, "failed to close persistent browser");
            errors.push(format!("close failed: {err}"));
        }
        if let Err(err) = self.browser.wait().await {
            tracing::warn!(error = %err, "failed to wait for persistent browser shutdown");
            errors.push(format!("wait failed: {err}"));
        }
        ...
        if errors.is_empty() {
            ...
            Err(format!(
                "Persistent browser session was removed, but shutdown reported errors: {}",
                errors.join("; ")
            ))
        }
    }
