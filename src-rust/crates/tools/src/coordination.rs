...
pub fn register_current_session(ctx: &ToolContext) {
    match open_store() {
        Ok(store) => {
            register_current_session_with_store(&store, ctx);
        }
        Err(err) => {
            tracing::warn!(error = %err, "failed to open coordination store");
        }
    }
}
...
fn register_current_session_with_store(store: &CoordinationStore, ctx: &ToolContext) -> String {
    ...
    if let Err(err) = store.register_session_with_parent(
        ...
        tracing::warn!(
            error = %err,
            actor_id = %session_id,
            "failed to register current actor in coordination store"
        );
    }
    session_id
}
...
pub fn execution_claim_notice(ctx: &ToolContext, command: &str) -> Option<String> {
    ...
    let store = match open_store() {
        ...
        Err(err) => {
            tracing::warn!(
                error = %err,
                "failed to open coordination store for execution notice"
            );
            return None;
        }
    };
    let session_id = register_current_session_with_store(&store, ctx);
    let claims = match store.list_claims(Some(&ctx.working_dir)) {
        Ok(claims) => claims,
        Err(err) => {
            tracing::warn!(
                error = %err,
                "failed to list coordination claims for execution notice"
            );
            return None;
        }
    };
...
impl Drop for TransientWriteClaimGuard {
    fn drop(&mut self) {
        ...
        let store = match CoordinationStore::open(&default_db_path()) {
            Ok(store) => store,
            Err(err) => {
                tracing::warn!(
                    error = %err,
                    "failed to open coordination store to release transient write claims"
                );
                return;
            }
        };
        for claim_id in &self.claim_ids {
            if let Err(err) = store.release_claims(&self.session_id, Some(claim_id)) {
                tracing::warn!(
                    error = %err,
                    claim_id = %claim_id,
                    session_id = %self.session_id,
                    "failed to release transient write claim"
                );
            }
        }
    }
