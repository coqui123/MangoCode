            ...
            let entry = match entry {
                ...
                Err(err) => {
                    ...
                    debug!(
                        ...
                        error = %err,
                        "Grep skipped path during traversal"
                    );
                    continue;
                }
            };
            ...
            let content = match std::fs::read_to_string(path) {
                ...
                Err(err) => {
                    ...
                    debug!(
                        path = %path.display(),
                        error = %err,
                        "Grep skipped unreadable or non-UTF-8 file"
                    );
                    continue;
                }
            };
            ...
            match output_mode {
                ...
                other => {
                    return ToolResult::error(format!(
                        "Invalid output_mode after normalization: {}",
                        other
                    ));
                }
            }
        ...
        if results.is_empty() {
            ...
            if skipped_paths > 0 {
                output.push_str(&format!(
                    "\nSkipped {} paths due to traversal, read, or encoding errors.",
                    skipped_paths
                ));
            }
