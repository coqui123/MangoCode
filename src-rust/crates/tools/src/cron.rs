...
/// Path to `~/.mangocode/scheduled_tasks.json`.
fn scheduled_tasks_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".mangocode").join("scheduled_tasks.json"))
}
...
async fn ensure_store_loaded() {
    ...
    *init = true;
    ...
    // Load from ~/.mangocode/scheduled_tasks.json if it exists.
...
// ---------------------------------------------------------------------------
// Public scheduler API (used by mangocode-query cron_scheduler)
// ---------------------------------------------------------------------------
...
fn cron_range_matches(part: &str, value: u32) -> bool {
    if let Some((lo_str, hi_str)) = part.split_once('-') {
        let lo: u32 = lo_str.parse().unwrap_or(u32::MAX);
        let hi: u32 = hi_str.parse().unwrap_or(0);
        value >= lo && value <= hi
    } else {
        part.parse::<u32>()
...
fn validate_cron(expr: &str) -> bool {
    ...
    for (i, field) in fields.iter().enumerate() {
        ...
        // Handle comma-separated lists; each item is a range (N-M) or number N.
...
impl Tool for CronCreateTool {
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "durable": {
                    "type": "boolean",
                    "description": "true = persist to .mangocode/scheduled_tasks.json; false (default) = session only"
                }
            },
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> ToolResult {
        ...
        // Persist to ~/.mangocode/scheduled_tasks.json for durable tasks.
        if params.durable {
