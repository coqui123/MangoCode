...
# Keep native-command stderr (e.g. cargo's progress) as plain text instead of
# wrapping it in NativeCommandError records on PowerShell 7.3+, which made the
# model misread successful builds (exit 0) as failures. Harmless on older
# versions where this automatic variable does not exist.
            ...
            Err(_) => {
                if let Err(err) = child.kill().await {
                    tracing::warn!(
                        error = %err,
                        timeout_ms,
                        "failed to kill timed-out PowerShell command"
                    );
                }
...
mod tests {
    ...
    #[test]
    fn wrapper_disables_native_command_error_promotion() {
        let script = build_ps_wrapper_script(
            "cargo check",
            &ShellState::new(),
            &PathBuf::from("C:\\Users\\test"),
        );
        // Native stderr (cargo progress) must stay plain text, not become
        // NativeCommandError records that the model misreads as build failures.
        assert!(script.contains("$PSNativeCommandUseErrorActionPreference = $false"));
    }
}
