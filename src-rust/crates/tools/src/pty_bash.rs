...
fn extract_exports_from_command(command: &str) -> HashMap<String, String> {
    ...
    match Regex::new(r#"(?m)^\s*export\s+([A-Za-z_][A-Za-z0-9_]*)=(?:"([^"]*)"|'([^']*)'|(\S*))"#) {
        Ok(re) => {
        ...
        Err(err) => {
            tracing::warn!("export parsing regex failed to compile; using fallback parser: {err}");
...
fn normalize_windows_paths_for_bash(command: &str) -> String {
    let command = if let Some(double_quoted_re) =
        compile_windows_path_regex("double-quoted", r#""(?P<path>[A-Za-z]:[\\/][^"]*)""#)
    {
        double_quoted_re
    ...
    let command = if let Some(single_quoted_re) =
        compile_windows_path_regex("single-quoted", r#"'(?P<path>[A-Za-z]:[\\/][^']*)'"#)
    {
        single_quoted_re
            .replace_all(&command, |caps: &regex::Captures<'_>| {
                format!(
    ...
    if let Some(bare_re) =
        compile_windows_path_regex("bare", r#"(?P<path>[A-Za-z]:[\\/][^\s"'`;&|<>)]*)"#)
    {
        bare_re
            .replace_all(&command, |caps: &regex::Captures<'_>| {
...
#[cfg(windows)]
fn compile_windows_path_regex(label: &str, pattern: &str) -> Option<Regex> {
    match Regex::new(pattern) {
        Ok(regex) => Some(regex),
        Err(err) => {
            tracing::warn!("Windows path normalization regex '{label}' failed to compile: {err}");
            None
        }
    }
}
...
///
/// Uses the shared [`crate::ansi::strip_ansi`] for escape handling, then drops
/// carriage returns (PTYs emit bare `\r` for line resets; `\n` is preserved).
#[cfg(unix)]
fn strip_ansi(s: &str) -> String {
    crate::ansi::strip_ansi(s).replace('\r', "")
}
        ...
        Err(_) => {
            if let Err(err) = child.kill().await {
                tracing::warn!(
                    error = %err,
                    timeout_ms,
                    "failed to kill timed-out PTY command"
                );
            }
        ...
        Err(_) => {
            if let Err(err) = child.kill().await {
                tracing::warn!(
                    error = %err,
                    timeout_ms,
                    "failed to kill timed-out cmd.exe command"
                );
            }
            Err(format!("Command timed out after {}ms", timeout_ms))
        }
...
mod tests {
    ...
    #[test]
    fn extracts_simple_export_assignments_with_whitespace() {
        let exports = extract_exports_from_command(
            "export FOO=bar\nexport\tQUOTED='two words'\nexport   DOUBLE=\"three words\"",
        );

        assert_eq!(exports.get("FOO").map(String::as_str), Some("bar"));
        assert_eq!(exports.get("QUOTED").map(String::as_str), Some("two words"));
        assert_eq!(
            exports.get("DOUBLE").map(String::as_str),
            Some("three words")
        );
        assert!(parse_simple_export_line("exported FOO=bar").is_none());
    }
