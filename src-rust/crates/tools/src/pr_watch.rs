...
async fn load_watched_prs() -> Result<Vec<WatchedPr>, String> {
    let path = watched_prs_path().ok_or_else(|| "Cannot determine home directory".to_string())?;
    load_watched_prs_from_path(&path).await
}

async fn load_watched_prs_from_path(path: &Path) -> Result<Vec<WatchedPr>, String> {
...
async fn save_watched_prs(items: &[WatchedPr]) -> Result<(), String> {
    ...
    crate::fs_atomic::write_atomic(&path, data.as_bytes())
        .await
...
fn parse_pr_number(raw: &str) -> Option<u64> {
    ...
    static PULL_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"/pull/(\d+)").unwrap());
...
async fn watch_pr(params: PrWatchInput, _ctx: &ToolContext) -> ToolResult {
    ...
    let mut items = match load_watched_prs().await {
        Ok(items) => items,
        Err(e) => return ToolResult::error(e),
    };
    ...
    match load_watched_prs().await {
        Ok(mut watched) => {
            if let Some(item) = watched.iter_mut().find(|w| w.pr == pr) {
                ...
                if let Err(e) = save_watched_prs(&watched).await {
                    out.push(String::new());
                    out.push(format!("Warning: failed to update watched PR state: {}", e));
                }
            }
        }
        Err(e) => {
            out.push(String::new());
            out.push(format!("Warning: failed to load watched PR state: {}", e));
        }
    }
...
pub async fn heartbeat_scan_watched_prs(
    ...
    let mut watched = match load_watched_prs().await {
        Ok(watched) => watched,
        Err(e) => return vec![format!("PR watch state error: {}", e)],
    };
    if watched.is_empty() {
        return Vec::new();
    }
    ...
    if let Err(e) = save_watched_prs(&watched).await {
        alerts.push(format!("PR watch state update failed: {}", e));
    }
    alerts
}
...
mod tests {
    ...
    #[tokio::test]
    async fn load_watched_prs_missing_file_returns_empty_list() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("watched_prs.json");

        let watched = load_watched_prs_from_path(&path).await.unwrap();

        assert!(watched.is_empty());
    }

    #[tokio::test]
    async fn load_watched_prs_reports_parse_errors() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("watched_prs.json");
        tokio::fs::write(&path, "{not json").await.unwrap();

        let err = load_watched_prs_from_path(&path).await.unwrap_err();

        assert!(err.contains("Failed to parse"));
        assert!(err.contains("watched_prs.json"));
    }
}
