// mangocode-mcp: Model Context Protocol (MCP) client implementation.
//
...
/// (unknown variables without a default are reported but don't crash).
...
pub mod transport {
            ...
            match child.try_wait() {
                ...
                Ok(None) => {
                    if let Err(err) = child.kill().await {
                        warn!(error = %err, "failed to kill MCP stdio child during close");
                    }
                }
                Err(err) => {
                    warn!(error = %err, "failed to inspect MCP stdio child during close");
                    if let Err(kill_err) = child.kill().await {
                        warn!(
                            error = %kill_err,
                            "failed to kill MCP stdio child after status check error"
                        );
                    }
                }
            }
    ...
    impl HttpTransport {
        ...
        pub async fn connect(config: &McpServerConfig) -> anyhow::Result<Self> {
            let url = if config.server_type == "pipedream" {
                    ...
                    Self::read_vault_secret(Self::pipedream_vault_keys("mcp_url"))
                {
                    ...
                    env_url
                } else if let Some(file_url) = PipedreamConfig::load_result()?
                    .map(|c| c.mcp_url())
                    .filter(|s| !s.is_empty())
                {
                    file_url
        ...
        fn pipedream_config(
            ...
            let file_config = PipedreamConfig::load_result()?;
        ...
        async fn pipedream_access_token(
            ...
            if !response.status().is_success() {
                ...
                let body = response_error_body(response, "Pipedream token request").await;
    ...
    impl McpTransport for HttpTransport {
        async fn send(&self, message: &JsonRpcRequest) -> anyhow::Result<()> {
            ...
            if !response.status().is_success() {
                let status = response.status();
                let body = response_error_body(response, "MCP HTTP request").await;
                ...
                while let Some(result) = stream.next().await {
                    ...
                    if has_id {
                        if let Some(id) = raw.get("id").and_then(|v| v.as_u64()) {
                                ...
                                match serde_json::from_value::<JsonRpcResponse>(raw.clone()) {
                                    Ok(response) => {
                                        if sender.send(response).is_err() {
                                            debug!(server = %server_name, response_id = id, "MCP response receiver dropped before delivery");
                                        }
                                    }
                                    Err(e) => {
                                        debug!(
                            ...
                            debug!(server = %server_name, raw_id = ?raw.get("id"), "MCP response has non-u64 id; dropping");
                        }
                        continue;
                    }
...
pub async fn unsubscribe_resource(
    ...
    let params = serde_json::json!({ "uri": uri });
    client
        .call::<serde_json::Value>("resources/unsubscribe", Some(params))
        .await
        .map_err(|e| format!("unsubscribe_resource failed: {e}"))?;

    manager
        .resource_subscriptions
        .remove(&(server_name.to_string(), uri.to_string()));
    Ok(())
}
