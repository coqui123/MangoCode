    ...
    let bytes = match std::fs::read(&path) {
        ...
        Err(err) => {
            tracing::warn!(
                ...
                path = %path.display(),
                error = %err,
                "failed to read stored MCP OAuth token"
            );
            return None;
        }
    };
    ...
    match serde_json::from_slice(&bytes) {
        ...
        Err(err) => {
            tracing::warn!(
                ...
                path = %path.display(),
                error = %err,
                "failed to parse stored MCP OAuth token"
            );
            None
        }
    }
...
pub fn initiate_xaa_login(server_name: &str, idp_url: &str) -> std::io::Result<XaaLoginState> {
    ...
    if let Err(err) = open_browser(&auth_url) {
        tracing::warn!(
            error = %err,
            server = %server_name,
            "failed to open XAA OAuth URL in browser"
        );
    }
...
pub async fn exchange_code(
    ...
    if !resp.status().is_success() {
        ...
        let body = response_error_body(resp, "exchange_code").await;
...
pub async fn refresh_mcp_token(
    ...
    if !resp.status().is_success() {
        let status = resp.status();
        let body = response_error_body(resp, "refresh").await;
