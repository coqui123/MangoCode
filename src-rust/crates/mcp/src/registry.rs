// mangocode-mcp: Official MCP server registry.
//
...
///
/// Skipped when `MANGOCODE_DISABLE_NONESSENTIAL_TRAFFIC` is set.
pub async fn prefetch_official_mcp_urls() {
    if std::env::var("MANGOCODE_DISABLE_NONESSENTIAL_TRAFFIC").is_ok() {
        return;
    }
    ...
    match result {
        ...
        Err(e) => {
            // Seed an empty set so the "only fetch once" guard trips and we
            // don't re-issue the (5 s-timeout) request on every later call.
