...
pub fn plan_execution_batches(calls: &[ToolCallPlan]) -> Vec<Vec<usize>> {
    ...
    let mut current_conflict_keys = HashSet::new();
    ...
    for (idx, call) in calls.iter().enumerate() {
        if call.blocked || call.capabilities.mutating || !call.capabilities.parallel_safe {
            if !current_parallel.is_empty() {
                ...
                current_conflict_keys.clear();
            }
            batches.push(vec![idx]);
        } else {
            let conflict_keys = tool_conflict_keys(call);
            let conflicts = conflict_keys
                .iter()
                .any(|key| current_conflict_keys.contains(key));
            if conflicts && !current_parallel.is_empty() {
                batches.push(std::mem::take(&mut current_parallel));
                current_conflict_keys.clear();
            }
            current_conflict_keys.extend(conflict_keys);
...
fn tool_conflict_keys(call: &ToolCallPlan) -> Vec<String> {
    call.capabilities
        .affected_paths
        .iter()
        .map(|path| format!("path:{}", normalize_conflict_key(path)))
        .chain(
            call.capabilities
                .network_targets
                .iter()
                .map(|target| format!("host:{}", normalize_conflict_key(target))),
        )
        .collect()
}

fn normalize_conflict_key(value: &str) -> String {
...
fn score_name(requested: &str, candidate: &str) -> usize {
    ...
    // Fuzzy fallback: rank by edit distance so typos and adjacent transpositions
    // (e.g. "bsah" -> "bash") still surface a suggestion, which the previous
    // subsequence heuristic missed.
    let req_len = requested.chars().count();
    let cand_len = candidate.chars().count();
    let max_len = req_len.max(cand_len);
    if max_len == 0 {
        return 0;
    }
    let dist = fuzzy::damerau_levenshtein(requested, candidate);
    // Only suggest reasonably-close names: edit distance at most ~40% of the
    // longer length. Closer = higher score, kept well below the substring tier
    // (5_000) so exact and substring matches always rank first.
    if dist * 5 > max_len * 2 {
        0
    } else {
        ((max_len - dist) * 100).min(4_000)
    }
}
...
mod tests {
    ...
    #[test]
    fn suggests_name_with_a_typo_or_transposition() {
        let plan = ToolRegistryPlan::from_specs(vec![
            spec("Bash", &[], ToolCapabilities::mutating()),
            spec("ToolSearch", &["tool_search"], ToolCapabilities::read_only()),
        ]);

        // Adjacent transposition ("bsah") and a single substitution ("toolsaerch")
        // — both missed by the old subsequence heuristic — now resolve via the
        // edit-distance fallback.
        assert_eq!(plan.suggestions_for("bsah", 1), vec!["Bash"]);
        assert_eq!(plan.suggestions_for("toolsaerch", 1), vec!["ToolSearch"]);
    }
    ...
    #[test]
    fn batches_serialize_parallel_safe_calls_with_same_path_or_host() {
        let calls = vec![
            ToolCallPlan::new(
                "Read",
                ToolCapabilities::read_only().with_affected_paths(vec!["src/lib.rs".to_string()]),
            ),
            ToolCallPlan::new(
                "Grep",
                ToolCapabilities::read_only().with_affected_paths(vec!["SRC\\lib.rs".to_string()]),
            ...
            ToolCallPlan::new(
                "WebFetch",
                ToolCapabilities::read_only()
                    .with_network_targets(vec!["https://docs.example.com".to_string()]),
            ),
            ToolCallPlan::new(
                "DocRead",
                ToolCapabilities::read_only()
    ...
    #[test]
    fn batches_keep_independent_parallel_safe_calls_together() {
        let calls = vec![
            ToolCallPlan::new(
                "Read",
                ToolCapabilities::read_only().with_affected_paths(vec!["src/lib.rs".to_string()]),
            ...
            ToolCallPlan::new(
                "Grep",
                ToolCapabilities::read_only().with_affected_paths(vec!["src/main.rs".to_string()]),
            ),
            ToolCallPlan::new("Glob", ToolCapabilities::read_only()),
        ];

        assert_eq!(plan_execution_batches(&calls), vec![vec![0, 1, 2]]);
    }
}
