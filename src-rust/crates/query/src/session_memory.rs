// Session Memory Extraction for mangocode-query.
//
...
pub async fn migrate_legacy_agents_if_needed(memory_dir: &Path) -> anyhow::Result<()> {
    ...
    let content = fs::read_to_string(&agents_path).await.map_err(|e| {
        anyhow::anyhow!(
            "Failed to read legacy AGENTS.md at {}: {}",
            agents_path.display(),
            e
        )
    })?;
    ...
    match fs::rename(&agents_path, &backup_path).await {
        Ok(()) => info!(
            path = %agents_path.display(),
        ...
        Err(e) => warn!(
            ...
            backup = %backup_path.display(),
            error = %e,
            "Migrated legacy AGENTS.md content but failed to move original file to backup"
        ),
    }
