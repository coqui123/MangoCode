...
pub struct ScratchpadState {
    ...
    /// Compact agent-owned lifecycle summary for the current work run.
    pub work_run_summary: Option<String>,
...
impl ScratchpadState {
    ...
    /// Update the compact run lifecycle summary injected alongside the plan.
    pub fn set_work_run_summary(&mut self, summary: impl Into<String>) {
        let summary = summary.into();
        if summary.trim().is_empty() {
            self.work_run_summary = None;
        } else {
            self.work_run_summary =
                Some(truncate_bytes_with_ellipsis(&summary, RESULT_SUMMARY_CHARS).into_owned());
        }
    }
    ...
    pub fn render(&self) -> Option<String> {
        ...
        if let Some(ref work_run) = self.work_run_summary {
            lines.push(format!("Run: {}", work_run));
        }
...
mod tests {
    ...
    fn render_includes_all_fields() {
        ...
        state.set_work_run_summary("executing_tools; 2 changed files; verification pending");
