...
impl Drop for OllamaProcess {
    fn drop(&mut self) {
        ...
        if let Err(err) = self.child.wait() {
            warn!(error = %err, "Failed to wait for local Ollama process on shutdown");
        }
    }
}
...
fn local_ollama_process() -> MutexGuard<'static, Option<Arc<OllamaProcess>>> {
    match LOCAL_OLLAMA_PROCESS.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
            warn!("Local Ollama process mutex was poisoned; recovering state");
            poisoned.into_inner()
        }
    }
}
...
pub fn ensure_local_ollama_server() {
    ...
    let mut guard = local_ollama_process();
    if guard.is_some() {
        return;
    }
...
mod tests {
    ...
    #[test]
    fn local_ollama_process_lock_recovers_from_poison() {
        let result = std::panic::catch_unwind(|| {
            let _guard = LOCAL_OLLAMA_PROCESS.lock().unwrap();
            panic!("poison ollama process mutex");
        });
        assert!(result.is_err());

        drop(local_ollama_process());
    }
}
