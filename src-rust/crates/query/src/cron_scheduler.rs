...
// (in mangocode-tools) for tasks whose cron expression matches the current wall-clock
...
async fn run_scheduler_loop(
    ...
    loop {
        ...
        let due = mangocode_tools::cron::pop_due_tasks(&tick_time).await;
        ...
        for task in due {
            ...
            tokio::spawn(async move {
                ...
                let outcome = run_query_loop(
                    ...
                    None, // no HITL permission prompts for cron tasks
                )
                .await;
