...
use mangocode_api::{AnthropicStreamEvent, ApiMessage, CreateMessageRequest, StreamAccumulator, StreamHandler, SystemPrompt};
                ...
                match block {
                    ...
                    ContentBlock::ToolResult { tool_use_id, content, is_error } => {
                        let result_text = match content {
                            mangocode_core::types::ToolResultContent::Text(t) => t.as_str().to_string(),
                            mangocode_core::types::ToolResultContent::Blocks(_) => "[complex content]".to_string(),
                        };
...
pub struct CompactResult {
    ...
    pub messages: Vec<mangocode_core::types::Message>,
...
pub fn snip_compact(messages: Vec<mangocode_core::types::Message>, keep_n_newest: usize) -> (Vec<mangocode_core::types::Message>, u64) {
    let total = messages.len();
...
pub fn calculate_messages_to_keep_index(messages: &[mangocode_core::types::Message], token_budget: u64) -> usize {
...
/// `reactiveCompact.ts`.
fn strip_images(messages: Vec<mangocode_core::types::Message>) -> Vec<mangocode_core::types::Message> {
...
/// `mangocode_core::feature_gates::is_feature_enabled("reactive_compact")` is true.
///
...
pub async fn reactive_compact(
...
) -> Result<CompactResult, mangocode_core::error::ClaudeError> {
    if cancel.is_cancelled() {
        return Err(mangocode_core::error::ClaudeError::Cancelled);
    }
    ...
    for path in recently_modified.iter().take(MAX_FILES * 3) {
        ...
        new_messages.push(mangocode_core::types::Message::user(text));
        injected += 1;
    }
...
pub async fn context_collapse(
    ...
    for msg in &messages {
        let role = match msg.role {
            mangocode_core::types::Role::User => "Human",
    ...
    if summary_text.is_empty() {
        return Err(mangocode_core::error::ClaudeError::Other(
            "Context-collapse summary was empty".to_string(),
        ));
    }

    // Keep only: the synthetic summary + the most recent user turn.
    let collapse_notice = mangocode_core::types::Message::user(format!(
...
pub fn collapse_read_tool_results(messages: Vec<mangocode_core::types::Message>) -> Vec<mangocode_core::types::Message> {
    use mangocode_core::types::{ContentBlock, MessageContent, ToolResultContent};
...
pub fn collapse_search_results(messages: Vec<mangocode_core::types::Message>) -> Vec<mangocode_core::types::Message> {
    use mangocode_core::types::{ContentBlock, MessageContent, ToolResultContent};
    ...
    let mut result: Vec<mangocode_core::types::Message> = messages
        .into_iter()
        .rev()
        .map(|mut msg| {
            if let MessageContent::Blocks(ref mut blocks) = msg.content {
