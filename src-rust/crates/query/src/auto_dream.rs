...
impl AutoDream {
        ...
        match fs::metadata(&self.lock_file).await {
            ...
            Err(err) => {
                tracing::warn!(
                    ...
                    "failed to inspect AutoDream lock file; allowing consolidation"
                );
                Ok(true)
            }
        }
    ...
    /// Load persisted state; returns `Default` on missing or malformed state,
    /// logging failures that are likely to affect consolidation timing.
            ...
            Ok(data) => match serde_json::from_str(&data) {
                ...
                Err(err) => {
                    tracing::warn!(
                        ...
                        "failed to parse AutoDream state; using default state"
                    );
                    ConsolidationState::default()
                }
            },
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => ConsolidationState::default(),
            Err(err) => {
                tracing::warn!(
                    error = %err,
                    path = %self.state_file.display(),
                    "failed to read AutoDream state; using default state"
                );
                ConsolidationState::default()
            }
        }
            ...
            if let Some(parent) = task.state_file.parent() {
                if let Err(err) = tokio::fs::create_dir_all(parent).await {
                    tracing::warn!(
                        error = %err,
                        path = %parent.display(),
                        "failed to create AutoDream state directory"
                    );
                }
            }
            if let Err(err) = tokio::fs::write(&task.state_file, json).await {
                tracing::warn!(
                    error = %err,
                    path = %task.state_file.display(),
                    "failed to write AutoDream state"
                );
            }
        } else {
            tracing::warn!("failed to serialize AutoDream state");
        }
        if task.lock_file.exists() {
            if let Err(err) = tokio::fs::remove_file(&task.lock_file).await {
                tracing::warn!(
                    error = %err,
                    path = %task.lock_file.display(),
                    "failed to remove AutoDream lock file"
                );
            }
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_malformed_state_defaults_without_overwriting_file() {
        let tmp = TempDir::new().unwrap();
        let dream = make_dream(&tmp);
        std::fs::create_dir_all(&dream.memory_dir).unwrap();
        std::fs::write(&dream.state_file, "{not-json").unwrap();

        let loaded = dream.load_state().await;

        assert_eq!(loaded.last_consolidated_at, None);
        assert_eq!(loaded.lock_etag, None);
        assert_eq!(
            std::fs::read_to_string(&dream.state_file).unwrap(),
            "{not-json"
        );
    }
