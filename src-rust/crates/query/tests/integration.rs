...
use mangocode_tools::{FileEditTool, FileReadTool, PermissionLevel, Tool, ToolContext, ToolResult};
use serde_json::json;
...
async fn simple_text_response() {
    ...
    let outcome = run_query_loop(
        ...
        None,
        None, // no HITL permission prompts in tests
    )
    .await;
...
#[tokio::test]
async fn enforce_completion_gate_fails_closed_after_repeated_unready_final_answers() {
    let temp = tempfile::tempdir().expect("tempdir");
    let src_dir = temp.path().join("src");
    std::fs::create_dir_all(&src_dir).expect("create src dir");
    std::fs::write(src_dir.join("lib.rs"), "pub fn value() -> u32 { 1 }\n").expect("write file");

    let provider = MockProvider::with_responses(vec![
        "All done without verification.",
        "Still done without verification.",
        "Still not running verification.",
        "Final answer without verification.",
    ])
    .with_tool_sequence(vec![
        ...
        vec![ToolCall::new(
            ...
            json!({
                ...
                "old_string": "pub fn value() -> u32 { 1 }\n",
                "new_string": "pub fn value() -> u32 { 2 }\n"
            }),
        )],
        vec![],
        vec![],
    ...
    let mut config = test_config(make_registry(provider.clone()));
    config.max_turns = 10;
    let mut ctx = test_tool_ctx("mock");
    ctx.working_dir = temp.path().to_path_buf();
    ctx.session_id = format!("completion-gate-{}", uuid::Uuid::new_v4());
    let mut messages = vec![Message::user("Change src/lib.rs and finish")];
    let tools: Vec<Box<dyn Tool>> = vec![Box::new(FileReadTool), Box::new(FileEditTool)];
    ...
    match outcome {
        QueryOutcome::Error(err) => {
            ...
            assert!(
                text.contains("Completion gate blocked finalization"),
                "{text}"
            );
            assert!(text.contains("needs_verification"), "{text}");
        }
        other => panic!(
            "Expected completion-gate Error, got {:?}; file={:?}; events={:#?}; messages={:#?}",
            other,
            std::fs::read_to_string(src_dir.join("lib.rs")),
            mangocode_core::harness::list_events(&ctx.session_id, 100),
            messages
        ),
    }

    let gate_prompts = messages
        .iter()
        .filter(|msg| {
            msg.role == Role::User && msg.get_all_text().contains("MangoCode completion gate")
        })
        .count();
    assert_eq!(gate_prompts, 3);
    assert_eq!(provider.request_count(), 6);
    assert_eq!(
        std::fs::read_to_string(src_dir.join("lib.rs")).expect("read edited file"),
        "pub fn value() -> u32 {\n    2\n}\n"
    );
}
